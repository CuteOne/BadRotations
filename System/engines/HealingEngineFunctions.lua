...
function br.getFocusedTank()
	...
	if #tanks > 0 and br.GetUnitExists("target") and br.GetUnitIsVisible("target") and br.GetUnitExists("targettarget")
		and br.GetUnitIsVisible("targettarget") then
...
function br.castWiseAoEHeal(unitTable,spell,radius,health,minCount,maxCount,facingCheck,movementCheck)
	if movementCheck ~= true or not br.isMoving("player") then
		local bestCandidate = nil
		-- find best candidate with list of units
		for i = 1, #unitTable do
			-- added a visible check as its not in healing engine.
			if (br.GetUnitIsVisible(unitTable[i].unit) and (facingCheck ~= true or br.getFacing("player",unitTable[i].unit)) or br.GetUnitIsUnit(unitTable[i].unit,"player")) then
				local candidate = br.getUnitsToHealAround(unitTable[i].unit,radius,health,maxCount)
...
function br.getNovaDistance(Unit1,Unit2)
...
function br.getUnitsToHealAround(UnitID,radius,health,count)
	...
	if type(UnitID) == "string" then
		X1,Y1,Z1 = br.GetObjectPosition(UnitID)
	...
	lowHealthCandidates[0] = { unit = UnitID, coef = br.getLowHealthCoeficient(lowHealthCandidates) }
...
local function chainJumps(unit,hp,range)
	for i = 1, #br.friend do
		...
		if newUnit.hp <= hp and newUnit.guid ~= unit.guid and br.getNovaDistance(unit,newUnit) <= range then
...
function br.chainHealUnits(spell,range,hp,count)
	jumpFound = true
	for i = 1, #br.friend do
		local thisUnit = br.friend[i]
		if thisUnit.hp <= hp then
			...
			elseif #totalUnits >= count then
				totalUnits= {}
				if br.castSpell(startUnit,spell,false,true) then
					return true
				end
			end
		end
...
function br.getLowHealthCoeficient(lowHealthCandidates)
	local tableCoef = 0
...
function br.getAllies(Unit,Radius)
	local alliesTable = {}
	for i=1,#br.friend do
...
function br.getAlliesInLocation(myX,myY,myZ,Radius)
	local alliesTable = {}
	for i=1,#br.friend do
		if not br.GetUnitIsDeadOrGhost(br.friend[i].unit) then
...
function br.isBadlyDeBuffed(Unit)
	if Unit == nil then
		return false
	end

	for i=1,#br.novaEngineTables.BadlyDeBuffed do
		if br.UnitDebuffID(Unit,br.novaEngineTables.BadlyDeBuffed[i])~=nil then
			return true
		end
	end
	return false
...
function br.inLoSHealer()
	...
	for i = 1, #br.friend do
		...
		if not br.GetUnitIsUnit(thisUnit,"player") and br._G.UnitGroupRolesAssigned(thisUnit) == "HEALER" then
...
function br.isInside(x,y,ax,ay,bx,by,dx,dy)
	local bax = bx - ax
	local bay = by - ay
	local dax = dx - ax
	local day = dy - ay
...
function br.getUnitsInRect(width,length, showLines, hp)
	...
	for i = 1, #br.friend do
		local thisUnit = br.friend[i]
		if br.GetUnitExists(thisUnit.unit) and thisUnit.hp <= hp and not br.GetUnitIsDeadOrGhost(thisUnit.unit) then
			local tX, tY = thisUnit.x, thisUnit.y
			if tX and tY then
				if br.isInside(tX,tY,nlX,nlY,nrX,nrY,frX,frY) then
					if showLines then
...
function br.getAngles(X1,Y1,Z1,X2,Y2,Z2)
	return math.atan2(Y2-Y1,X2-X1)
end

function br.getUnitsInCone(length,angle,hp)
    local playerX, playerY, playerZ = br.GetObjectPosition("player")
    local facing = br.GetObjectFacing("player")
    local units = {};
    ...
    for i = 1, #br.friend do
		...
		if thisUnit.hp <= hp then
			if br.friend[i].distance <= length then
		        if not br.GetUnitIsUnit(thisUnit,"player") and (br.isDummy(thisUnit) or br.GetUnitIsFriend(thisUnit,"player")) then
		            local unitX, unitY, unitZ = br.GetObjectPosition(thisUnit)
...
function br.healConeAround(numUnitsp, healthp, anglep, rangeInfrontp, rangeAroundp)
    local total         = 0
...
function br.Factorial(n)
	if n < 0 then return nil end
	if n >= 2 then return n * br.Factorial(n - 1) end
	return 1
end
...
function br.GetCombinations(points, choose)
...
function br.GetDistanceToClosestNeighbor(p,points)
	local minDist = 999
...
function br.GetNumPointsInCircle(center,radius,points)
...
function br.GetCentroidOfPoints(points)
...
function br.getBestGroundCircleLocation(unitTable,minTargets,maxHealTargets,radius)
	...
	for i=1,#unitTable do
		...
		if type(thisUnit) == "string" then
			X1,Y1,Z1 = br.GetObjectPosition(thisUnit)
			-- if we provide it a table, take that object position(accepts br.enemy[i] or br.friend[i])
	...
	local center = br.GetCentroidOfPoints(pointsInRange)
	if center == nil then return nil end
	local numInside = br.GetNumPointsInCircle(center,radius,pointsInRange)
    ...
    while groupSize >= minTargets do
		local allCombinations = br.GetCombinations(pointsInRange,groupSize)
		for i=1, #allCombinations do
			-- get the centroid of this list of points
			local c = br.GetCentroidOfPoints(allCombinations[i])
    ...
    return br.GetCentroidOfPoints(bestGroup)
end
