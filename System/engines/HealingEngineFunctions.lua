local _, br = ...
br.engines.healingEngineFunctions = br.engines.healingEngineFunctions or {}
local healingEngineFunctions = br.engines.healingEngineFunctions

-- here we want to define functions to use with the healing profiles
-- find best tank to put our lb/beacon/earth shield on
function healingEngineFunctions:getFocusedTank()
	local tanks = br.engines.healingEngineFunctions:getTanksTable()
	-- if we are targetting a mob and its targetting a tank we want to define which tank it is.
	if #tanks > 0 and br.functions.unit:GetUnitExists("target") and br.functions.unit:GetUnitIsVisible("target") and br.functions.unit:GetUnitExists("targettarget")
		and br.functions.unit:GetUnitIsVisible("targettarget") then
		local targetTargetGUID = br._G.UnitGUID("targettarget")
		for i = 1,#tanks do
	...
	elseif #tanks > 0 then
		...
		for i = 1,#tanks do
			...
			if br._G.UnitGUID(tanks[i].target) == tanks[i].guid then
				-- mob is on this unit
				br.oldTank = tanks[i]
				return tanks[i]
			end
...
-- find tanks
function healingEngineFunctions:getTanksTable()
	local tanksTable = {}
	for i = 1, #br.engines.healingEngine.friend do
		if br.engines.healingEngine.friend[i] then
			if br.engines.healingEngine.friend[i].role == "TANK" then
				br._G.tinsert(tanksTable, br.engines.healingEngine.friend[i])
			end
		end
	end
...
function healingEngineFunctions:castWiseAoEHeal(unitTable,spell,radius,health,minCount,maxCount,facingCheck,movementCheck)
	if movementCheck ~= true or not br.functions.misc:isMoving("player") then
		local bestCandidate = nil
		-- find best candidate with list of units
		for i = 1, #unitTable do
			-- added a visible check as its not in healing engine.
			if (br.functions.unit:GetUnitIsVisible(unitTable[i].unit) and (facingCheck ~= true or br.functions.unit:getFacing("player",unitTable[i].unit)) or br.functions.unit:GetUnitIsUnit(unitTable[i].unit,"player")) then
				local candidate = br.engines.healingEngineFunctions:getUnitsToHealAround(unitTable[i].unit,radius,health,maxCount)
		...
		if bestCandidate ~= nil and #bestCandidate >= minCount and br.functions.misc:getLineOfSight("player",bestCandidate[0].unit) and br.functions.range:getDistance("player",bestCandidate[0].unit) <= 40 then
			-- here we would like instead to cast on unit
			if br.functions.cast:castSpell(bestCandidate[0].unit,spell,facingCheck,movementCheck) then
				if br._G.IsAoEPending() then br._G.SpellStopTargeting() br.functions.misc:addonDebug("Canceling Spell", true) end
				return true
			end
		end
	end
...
function healingEngineFunctions:getNovaDistance(Unit1,Unit2)
...
-- now that we have our units in br.engines.healingEngine.friend with their positions, we can compare them to our enemies
...
function healingEngineFunctions:getUnitsToHealAround(UnitID,radius,health,count)
	...
	if type(UnitID) == "string" then
		X1,Y1,Z1 = br.functions.unit:GetObjectPosition(UnitID)
		-- if we provide it a table, take that object position(accepts br.engines.enemiesEngine.enemy[i] or br.engines.healingEngine.friend[i])
	elseif UnitID and UnitID.x ~= 0 then
		X1,Y1,Z1 = UnitID.x,UnitID.y,UnitID.z
	end
	local unit = {x = X1,y = Y1,z = Z1,guid = br._G.UnitGUID(UnitID),name = br._G.UnitName(UnitID)}
	...
	for i = 1, #br.engines.healingEngine.friend do
		...
		-- if in given radius
		if thisUnit.hp <= health and br.engines.healingEngineFunctions:getNovaDistance(unit,thisUnit) < radius then
	...
	lowHealthCandidates[0] = { unit = UnitID, coef = br.engines.healingEngineFunctions:getLowHealthCoeficient(lowHealthCandidates) }
...
local function chainJumps(unit,hp,range)
	for i = 1, #br.engines.healingEngine.friend do
		local newUnit = br.engines.healingEngine.friend[i]
		local unitFound = false
		if newUnit.hp <= hp and newUnit.guid ~= unit.guid and br.engines.healingEngineFunctions:getNovaDistance(unit,newUnit) <= range then
...
function healingEngineFunctions:chainHealUnits(spell,range,hp,count)
	jumpFound = true
	for i = 1, #br.engines.healingEngine.friend do
		local thisUnit = br.engines.healingEngine.friend[i]
		if thisUnit.hp <= hp then
			...
			elseif #totalUnits >= count then
				totalUnits= {}
				if br.functions.cast:castSpell(startUnit,spell,false,true) then
					return true
				end
			end
		end
...
function healingEngineFunctions:getLowHealthCoeficient(lowHealthCandidates)
	local tableCoef = 0
...
function healingEngineFunctions:getAllies(Unit,Radius)
	local alliesTable = {}
	for i=1,#br.engines.healingEngine.friend do
		if not br.functions.unit:GetUnitIsDeadOrGhost(br.engines.healingEngine.friend[i].unit) then
			if br.functions.range:getDistance(Unit,br.engines.healingEngine.friend[i].unit) <= Radius then
				br._G.tinsert(alliesTable,br.engines.healingEngine.friend[i])
			end
		end
	end
	return alliesTable
...
function healingEngineFunctions:getAlliesInLocation(myX,myY,myZ,Radius)
	local alliesTable = {}
	for i=1,#br.engines.healingEngine.friend do
		if not br.functions.unit:GetUnitIsDeadOrGhost(br.engines.healingEngine.friend[i].unit) then
			if br.functions.range:getDistanceToLocation(br.engines.healingEngine.friend[i].unit,myX,myY,myZ) <= Radius then
				br._G.tinsert(alliesTable,br.engines.healingEngine.friend[i].unit)
			end
		end
	end
	return alliesTable
...
function healingEngineFunctions:isBadlyDeBuffed(Unit)
	if Unit == nil then
		return false
	end

	for i=1,#br.engines.healingEngineCollections.BadlyDeBuffed do
		if br.functions.aura:UnitDebuffID(Unit,br.engines.healingEngineCollections.BadlyDeBuffed[i])~=nil then
			return true
		end
	end
	return false
...
function healingEngineFunctions:inLoSHealer()
	local function drawHealers(healer)
		...
		-- local locateX, locateY, locateZ = br.functions.unit:GetObjectPosition(healer)
		local healerX, healerY, healerZ = br.functions.unit:GetObjectPosition(healer)
		if br.functions.misc:getLineOfSight("player",healer) then
	...
	for i = 1, #br.engines.healingEngine.friend do
		...
		if not br.functions.unit:GetUnitIsUnit(thisUnit,"player") and br._G.UnitGroupRolesAssigned(thisUnit) == "HEALER" then
...
function healingEngineFunctions:isInside(x,y,ax,ay,bx,by,dx,dy)
	local bax = bx - ax
	local bay = by - ay
	local dax = dx - ax
	local day = dy - ay
...
function healingEngineFunctions:getUnitsInRect(width,length, showLines, hp)
	local LibDraw = br._G.LibStub("LibDraw-BR")
	local playerX, playerY, playerZ = br.functions.unit:GetObjectPosition("player")
	local facing = br.functions.unit:GetObjectFacing("player")
	-- Near Left
	local nlX, nlY, nlZ = br._G.GetPositionFromPosition(playerX, playerY, playerZ, width/2, facing + math.rad(90), 0)
	-- Near Right
	local nrX, nrY, nrZ = br._G.GetPositionFromPosition(playerX, playerY, playerZ, width/2, facing + math.rad(270), 0)
	-- Far Left
	local flX, flY, flZ = br._G.GetPositionFromPosition(nlX, nlY, nlZ, length, facing + math.rad(0), 0)
	-- Far Right
	local frX, frY, frZ = br._G.GetPositionFromPosition(nrX, nrY, nrZ, length, facing + math.rad(0), 0)
	...
	local unitCounter = 0
	local UnitsInRect = {}
	_G.table.wipe(UnitsInRect)
	for i = 1, #br.engines.healingEngine.friend do
		local thisUnit = br.engines.healingEngine.friend[i]
		if br.functions.unit:GetUnitExists(thisUnit.unit) and thisUnit.hp <= hp and not br.functions.unit:GetUnitIsDeadOrGhost(thisUnit.unit) then
			local tX, tY = thisUnit.x, thisUnit.y
			if tX and tY then
				if br.engines.healingEngineFunctions:isInside(tX,tY,nlX,nlY,nrX,nrY,frX,frY) then
					if showLines then
						LibDraw.Circle(tX, tY, playerZ, br._G.UnitBoundingRadius(thisUnit.unit))
					end
...
function healingEngineFunctions:getAngles(X1,Y1,Z1,X2,Y2,Z2)
	return math.atan2(Y2-Y1,X2-X1)
end

function healingEngineFunctions:getUnitsInCone(length,angle,hp)
    local playerX, playerY, playerZ = br.functions.unit:GetObjectPosition("player")
    local facing = br.functions.unit:GetObjectFacing("player")
    local units = {};
    ...
    for i = 1, #br.engines.healingEngine.friend do
        local thisUnit = br.engines.healingEngine.friend[i].unit
		if thisUnit.hp <= hp then
			if br.engines.healingEngine.friend[i].distance <= length then
		        if not br.functions.unit:GetUnitIsUnit(thisUnit,"player") and (br.functions.unit:isDummy(thisUnit) or br.functions.unit:GetUnitIsFriend(thisUnit,"player")) then
		            local unitX, unitY, unitZ = br.functions.unit:GetObjectPosition(thisUnit)
...
function healingEngineFunctions:healConeAround(numUnitsp, healthp, anglep, rangeInfrontp, rangeAroundp)
    local total         = 0
    ...
    for i=1,#br.engines.healingEngine.friend do
	local thisUnit = br.engines.healingEngine.friend[i].unit
        if br.engines.healingEngine.friend[i].hp < health then
            -- First check around us, light of dawn do heal 5 yards around
            if br.engines.healingEngine.friend[i].distance < raround then
                total = total +1
            else --dont doubledipp so an else
				if br.engines.healingEngine.friend[i].distance < rinfront then --only if they are in range
					local unitX, unitY, unitZ = br.functions.unit:GetObjectPosition(thisUnit)
					if playerX and unitX then
						local angleToUnit = br.engines.healingEngineFunctions:getAngles(playerX,playerY,playerZ,unitX,unitY,unitZ)
						local angleDifference = facing > angleToUnit and facing - angleToUnit or angleToUnit - facing
...
function healingEngineFunctions:Factorial(n)
	if n < 0 then return nil end
	if n >= 2 then return n * br.engines.healingEngineFunctions:Factorial(n - 1) end
	return 1
end
...
function healingEngineFunctions:GetCombinations(points, choose)
	local function fOfN(n, src, got, all)
		...
		local function tableSlice(tbl, first)
		  local sliced = {}
		  for i = first, #tbl do
...
function healingEngineFunctions:GetDistanceToClosestNeighbor(p,points)
	local minDist = 999
...
function healingEngineFunctions:GetNumPointsInCircle(center,radius,points)
...
function healingEngineFunctions:GetCentroidOfPoints(points)
...
function healingEngineFunctions:castGroundAtLocation(loc, SpellID)
	...
	if hit1z ~= nil and
		...
		if br._G.SpellIsTargeting() then
			...
			br.functions.misc:addonDebug("Canceling Spell", true)
		end
		return true
	end
end
...
function healingEngineFunctions:getBestGroundCircleLocation(unitTable,minTargets,maxHealTargets,radius)
	...
	for i=1,#unitTable do
		...
		if type(thisUnit) == "string" then
			X1,Y1,Z1 = br.functions.unit:GetObjectPosition(thisUnit)
			-- if we provide it a table, take that object position(accepts br.engines.enemiesEngine.enemy[i] or br.engines.healingEngine.friend[i])
    ...
    local pointsInRange = {}
    for i=1,#points do
		if br.engines.healingEngineFunctions:GetDistanceToClosestNeighbor(points[i],points) <= 2 * radius then
	...
	local center = br.engines.healingEngineFunctions:GetCentroidOfPoints(pointsInRange)
	if center == nil then return nil end
	local numInside = br.engines.healingEngineFunctions:GetNumPointsInCircle(center,radius,pointsInRange)
	X1,Y1,Z1 = br.functions.unit:GetObjectPosition("player")
	local X2,Y2,Z2 = center.x,center.y,center.z
	local LoS = br._G.TraceLine(X1, Y1, Z1 + 2, X2, Y2, Z2 + 2, 0x10) == nil
	local distance = br.functions.range:getDistanceToLocation("player",X2,Y2,Z2) <= 40
    ...
    while groupSize >= minTargets do
		local allCombinations = br.engines.healingEngineFunctions:GetCombinations(pointsInRange,groupSize)
		for i=1, #allCombinations do
			-- get the centroid of this list of points
			local c = br.engines.healingEngineFunctions:GetCentroidOfPoints(allCombinations[i])
			if c == nil then return nil end
			local tx,ty,tz = c.x,c.y,c.z
			local LoS = br._G.TraceLine(X1, Y1, Z1 + 2, tx, ty, tz + 2, 0x10) == nil
			local tdistance = br.functions.range:getDistanceToLocation("player",tx,ty,tz) <= 40
			if LoS and tdistance then
				-- how many of the points are inside the circle?
				local n = br.engines.healingEngineFunctions:GetNumPointsInCircle(c,radius,allCombinations[i])
    ...
    return br.engines.healingEngineFunctions:GetCentroidOfPoints(bestGroup)
end
