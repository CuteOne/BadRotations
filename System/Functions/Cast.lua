...
function createCastFunction(thisUnit,debug,minUnits,effectRng,spellID,index,predict,predictPad)
	...
	if IsUsableSpell(spellID) --[[not select(2,IsUsableSpell(spellID))]] and getSpellCD(spellID) == 0 and (isKnown(spellID) or debug == "known") and not IsCurrentSpell(spellID)
