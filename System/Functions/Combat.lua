...
function br.canDisarm(Unit)
	if br.DisarmedTarget == nil then br.DisarmedTarget = 0 end
...
function br.isIncapacitated(spellID)
	...
	if eventIndex > 0 then
		for i=0,eventIndex do
			...
			if event then
				if not br.canRegainControl(spellID,event.locType) and (event.locType ~= "DISARM" and event.locType ~= "ROOT" or event.locType == "SNARE")
				-- (event.locType == "NONE"
	...
	if eventIndex > 0 then
		for i=0,eventIndex do
			...
			if event then
				...
				if br.canRegainControl(spellID,event.locType) then return true end
			end
		end
	end
	return false
...
function br.hasThreat(unit,playerUnit)
	...
	else targetFriend = (br.isTargeting(unit) or br._G.UnitName(targetUnit) == br._G.UnitName("player") or (br.GetUnitExists("pet") and br._G.UnitName(targetUnit) == br._G.UnitName("pet")) or br._G.UnitInParty(targetUnit) or br._G.UnitInRaid(targetUnit))
	end
	...
	local function threatSituation(friendlyUnit,enemyUnit)
		...
		if threatPct ~= nil then
			if threatPct > 0 then
				if br.isChecked("Cast Debug") and not br.GetUnitExists("target") then br._G.print(br._G.UnitName(enemyUnit).." is threatening "..br._G.UnitName(friendlyUnit).."."); end
				return true
			end
		end
		return false
	...
	if targetFriend then
		if br.isChecked("Cast Debug") and not br.GetObjectExists("target") then br._G.print(br._G.UnitName(br.GetUnit(unit)).." is targetting "..br._G.UnitName(targetUnit)) end
	...
	if instance == "none" and playerInCombat and br.enemy[br._G.ObjectPointer("target")] ~= nil and br.enemy[objectUnit] == nil and getDistance("target",unitObject) < 8 then
