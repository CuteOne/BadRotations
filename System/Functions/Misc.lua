        ...
        if tentX ~= nil then
            for i = 1, #br.tentCache do
                if br.tentCache[i]["tentX"] ~= nil then
                    local a = {
                        x = br.tentCache[i]["tentX"],
                        y = br.tentCache[i]["tentY"]
                    }
                    local b = {
                        x = br.tentCache[i]["tentX2"],
                        y = br.tentCache[i]["tentY2"]
                    }
                    local c = {
                        x = tX,
                        y = tY
...
function br.getLineOfSight(Unit1, Unit2)
    ...
    if skipLoSTable[br.GetObjectID(Unit1)] or skipLoSTable[br.GetObjectID(Unit2)] or -- Kyrian Hunter Ability
        ...
        (Unit2 and Unit2 ~= "player" and br.getDebuffRemain(Unit2, 308498) > 0) then
        return true
    end
    if br.GetObjectExists(Unit1) and br.GetUnitIsVisible(Unit1) and br.GetObjectExists(Unit2) and
        br.GetUnitIsVisible(Unit2) then
        ...
        if (X1 == nil or X2 == nil or pX == nil) then
            return false
        end
...
function br.getGround(Unit)
    if br.GetObjectExists(Unit) and br.GetUnitIsVisible(Unit) then
        ...
        if br._G.TraceLine(X1, Y1, Z1, X1, Y1, Z1 - 2, 0x10) == nil and
            br._G.TraceLine(X1, Y1, Z1, X1, Y1, Z1 - 2, 0x100) == nil then
            return false
        else
            return true
        end
...
function br.getGroundDistance(Unit)
    if br.GetObjectExists(Unit) and br.GetUnitIsVisible(Unit) then
        local X1, Y1, Z1 = br.GetObjectPosition(Unit)
        for i = 1, 100 do
            if br._G.TraceLine(X1, Y1, Z1, X1, Y1, Z1 - i / 10, 0x10) ~= nil or
                br._G.TraceLine(X1, Y1, Z1, X1, Y1, Z1 - i / 10, 0x100) ~= nil then
                return i / 10
            end
        end
    else
...
function br.hasTank()
    ...
    for i = 1, #br.friend do
        ...
        if br._G.UnitGroupRolesAssigned(thisUnit) == "TANK" and
            br._G.UnitIsPlayer(thisUnit) and br.getDistance(thisUnit) < 40 then
            return true
        end
    end
    return false
...
function br.enemyListCheck(Unit)
    ...
    local mcCheck = (br.isChecked("Attack MC Targets") and
                        (not br.GetUnitIsFriend(Unit, "player") or br._G.UnitIsCharmed(Unit))) or
    ...
    return br.GetObjectExists(Unit) and not br.GetUnitIsDeadOrGhost(Unit) and inPhase and
               br._G.UnitCanAttack("player", Unit) and br._G.UnitHealth(Unit) > 0 and not br.isCritter(Unit) and mcCheck and
               not br.GetUnitIsUnit(Unit, "pet") and br._G.UnitCreator(Unit) ~= br._G.ObjectPointer("player") and
               br.GetObjectID(Unit) ~= 11492 and br.getLineOfSight("player", Unit) and
               ((Unit ~= 131824 and Unit ~= 131823 and Unit ~= 131825) or
                   ((br.UnitBuffID(Unit, 260805) or br.GetUnitIsUnit(Unit, "target")) and
                       (Unit == 131824 or Unit == 131823 or Unit == 131825))) and br.getDistance(Unit, "player") < 50
end
...
function br.isValidUnit(Unit)
    ...
    local mcCheck = (br.isChecked("Attack MC Targets") and
                        (not br.GetUnitIsFriend(Unit, "player") or
                            (br._G.UnitIsCharmed(Unit) and br._G.UnitCanAttack("player", Unit)))) or
                        not br.GetUnitIsFriend(Unit, "player")
    ...
    if not br.pause(true) and Unit ~= nil and (br.units[Unit] ~= nil or Unit == "target" or burnUnit) and mcCheck and
        not isCC and (dummy or burnUnit or (not br._G.UnitIsTapDenied(Unit) and br.isSafeToAttack(Unit) and
        ((hostileOnly and reaction < 4) or (not hostileOnly and reaction < 5) or playerTarget or targeting))) then
        return (playerTarget and (not inInstance or (inInstance and (#br.friend == 1 or not br.hasTank())))) or
                   targeting or burnUnit or br.isInProvingGround() or br.hasThreat(Unit)
    end
    return false
end
...
function br.UpdateToggle(toggle, delay)
    ...
    if (br.SpecificToggle(toggle .. " Mode") or br.customToggle) and not br._G.GetCurrentKeyBoardFocus() and
        br._G.GetTime() - _G[toggle .. "Timer"] > delay then
...
function br.pause(skipCastingCheck)
    ...
    -- Pause Hold/Auto
    if (pausekey and br._G.GetCurrentKeyBoardFocus() == nil and br.isChecked("Pause Mode")) or br.profileStop or
        (br._G.IsMounted() or br._G.IsFlying() or br._G.UnitOnTaxi("player") or
            (br._G.UnitInVehicle("player") and not br.isChecked("Bypass Vehicle Check") and
                (not br._G.UnitExists("target") or
                    (br._G.UnitExists("target") and not br._G.UnitCanAttack("player", "target")))) and
            not (br.UnitBuffID("player", 190784) or br.UnitBuffID("player", 164222) or br.UnitBuffID("player", 165803) or
                br.UnitBuffID("player", 157059))) or br._G.SpellIsTargeting() or
        (br._G.UnitCastingInfo("player") and not skipCastingCheck) or
        (br._G.UnitChannelInfo("player") and not skipCastingCheck) or br._G.UnitIsDeadOrGhost("player") or eating or
        br.UnitDebuffID("player", 252753) or -- Potion of Replenishment (BFA Mana channel) Apparently a debuff
    br.UnitBuffID("player", 114018) then
...
function br.isChecked(Value)
    if br.data ~= nil and br.data.settings ~= nil then
        ...
        if br.data.settings[br.selectedSpec] and
            (br.data.settings[br.selectedSpec][br.selectedProfile][Value .. "Check"] == 1 or
                br.data.settings[br.selectedSpec][br.selectedProfile][Value .. "Check"] == true) then
            return true
        end
    end
    return false
...
function br.isSelected(Value)
    if br.data.settings ~= nil and
        (br.data.settings[br.selectedSpec].toggles["Cooldowns"] == 3 or (br.isChecked(Value) and
            (br.getValue(Value) == 3 or
                (br.getValue(Value) == 2 and br.data.settings[br.selectedSpec].toggles["Cooldowns"] == 2)))) then
        return true
    end
    return false
end
...
function br.talentAnywhere()
    ...
    local function talentSelection(row)
        for column = 1, 3 do
            if br._G.IsMouseButtonDown(1) and br.newTalent == nil and
                br._G.MouseIsOver(_G["PlayerTalentFrameTalentsTalentRow" .. row .. "Talent" .. column]) and
                not select(4, br._G.GetTalentInfoByID(br._G.GetTalentInfo(row, column, 1), 1)) then
    ...
    if br._G.PlayerTalentFrame and br._G.PlayerTalentFrame:IsVisible() and not br._G.IsResting() then
        ...
        if br.newTalent ~= nil then
            if br.selectedTalent ~= nil and br.selectedTalent ~= br.newTalent and not br.selectedNew and
                br.timer:useTimer("RemoveTalent", 0.1) then
...
function br.getEssenceRank(essenceName)
    ...
    for i = 1, #essenceTable do
        ...
        if milestone.slot ~= nil and milestone.unlocked == true then
            local eRank = br._G.C_AzeriteEssence
                              .GetEssenceInfo(br._G.C_AzeriteEssence.GetMilestoneEssence(milestone.ID)).rank
            local eIcon = br._G.C_AzeriteEssence
                              .GetEssenceInfo(br._G.C_AzeriteEssence.GetMilestoneEssence(milestone.ID)).icon
            if icon == eIcon then
                essenceRank = eRank
            end
        end
