...
function br.getLatency()
	...
	local lag = select(4, _G.GetNetStats()) / 1000
	if lag < .05 then
		lag = .05
	elseif lag > .4 then
		lag = .4
...
function br.getFallDistance()
	...
	if br.zCoord == nil then
		br.zCoord = position
	end
	if not _G.IsFalling() or _G.IsFlying() then
		br.zCoord = position
	end
	if position - br.zCoord < 0 then
		zDist = math.sqrt(((position - br.zCoord) ^ 2))
	else
		zDist = 0
	end
...
function br.getFallTime()
	...
	if _G.IsFalling() and br.getFallDistance() < 0 then
		if br.fallStarted == 0 then
			br.fallStarted = _G.GetTime()
		end
		if br.fallStarted ~= 0 then
			br.fallTime = (math.floor((_G.GetTime() - br.fallStarted) * 1000) / 1000)
		end
	end
...
_G.math.doLinesIntersect = function(a, b, c, d)
...
function br.carapaceMath(Unit1, Unit2)
	...
	if (br.player and br.player.eID and br.player.eID == 2337) then
		local pX, pY, pZ = br.GetObjectPosition(Unit1)
		local tX, tY, tZ = br.GetObjectPosition(Unit2)
		...
		for i = 1, br._G.GetObjectCount() do
			...
			if objectid == 157485 then
				...
				table.insert(
					tentCache,
					{
						["tentFacing"] = br.GetObjectFacing(object),
				...
				local tentfinal = {}
				if tentX ~= nil then
					for j = 1, #tentCache do
						if tentCache[j]["tentX"] ~= nil then
							local a = {x = tentCache[j]["tentX"], y = tentCache[j]["tentY"]}
							local b = {x = tentCache[j]["tentX2"], y = tentCache[j]["tentY2"]}
							local c = {x = tX, y = tY}
							local d = {x = pX, y = pY}
							if tX ~= nil then
								if _G.math.doLinesIntersect(a, b, c, d) then
...
function br.getTalent(Row, Column, specGroup)
	if specGroup == nil then
		specGroup = _G.GetActiveSpecGroup()
	end
	local _, _, _, selected = _G.GetTalentInfo(Row, Column, specGroup)
...
function br.hasEmptySlots()
	...
	for i = 0, 4 do --Let's look at each bag
		...
		if numBagSlots > 0 then -- Only look for slots if bag present
			openSlots = openSlots + select(1, _G.GetContainerNumFreeSlots(i))
		end
	end
	if openSlots > 0 then
		return true
...
function br.hasGlyph(glyphid)
	for i = 1, 6 do
		if select(4, _G.GetGlyphSocketInfo(i)) == glyphid or select(6, _G.GetGlyphSocketInfo(i)) == glyphid then
			return true
		end
	end
	return false
...
function br.isMoving(Unit)
	...
	return br._G.br._G.GetUnitSpeed(Unit) > 0
end
...
function br.IsMovingTime(time)
	...
	if br._G.br._G.GetUnitSpeed("player") > 0 then
		if br.IsRunning == nil then
			br.IsRunning = _G.GetTime()
			br.IsStanding = nil
		end
		if _G.GetTime() - br.IsRunning > time then
			return true
		end
	else
		if br.IsStanding == nil then
			br.IsStanding = _G.GetTime()
			br.IsRunning = nil
		end
		if _G.GetTime() - br.IsStanding > time then
			return false
		end
	end
end
...
function br.getStandingTime()
	return br.DontMoveStartTime and _G.GetTime() - br.DontMoveStartTime or nil
end
--
function br.isStanding(Seconds)
	return _G.IsFalling() == false and br.DontMoveStartTime and br.getStandingTime() >= Seconds or false
end
-- if br.IsStandingTime(5) then
function br.IsStandingTime(time, unit)
	...
	if not _G.IsFalling() and br._G.GetUnitSpeed(unit) == 0 then
		if br.IsStanding == nil then
			br.IsStanding = _G.GetTime()
			br.IsRunning = nil
		end
...
-- if br.isValidTarget("target") then
...
function br.enemyListCheck(Unit)
	...
	return br.GetObjectExists(Unit) and not br.GetUnitIsDeadOrGhost(Unit) and inPhase and br._G.UnitCanAttack("player", Unit) and br._G.UnitHealth(Unit) > 0 and distance < 50 and not br.isCritter(Unit) and
		mcCheck and
...
function br.isValidUnit(Unit)
	...
	local mcCheck =
		(br.isChecked("Attack MC Targets") and (not br.GetUnitIsFriend(Unit, "player") or (br._G.UnitIsCharmed(Unit) and br._G.UnitCanAttack("player", Unit)))) or not br.GetUnitIsFriend(Unit, "player")
...
function br.UpdateToggle(toggle, delay)
	...
	if (br.SpecificToggle(toggle .. " Mode") or br.customToggle) and not _G.GetCurrentKeyBoardFocus() and _G.GetTime() - _G[toggle .. "Timer"] > delay then
		_G[toggle .. "Timer"] = _G.GetTime()
...
function br.BurstToggle(toggle, delay)
	...
	if br.burst and not _G.GetCurrentKeyBoardFocus() and _G.GetTime() - _G[toggle .. "Timer"] > delay then
...
function br.pause(skipCastingCheck)
	...
	if not skipCastingCheck then
		...
		if br.pauseCast - _G.GetTime() <= 0 then
			local hasted = (1 - br._G.UnitSpellHaste("player") / 100)
			-- Focused Azerite Beam
			if lastCast == 295258 and br.getSpellCD(295258) == 0 then
				br.pauseCast = _G.GetTime() + br.getCastTime(295258) + (br.getCastTime(295261) * hasted)
			end
			-- Cyclotronic Blast
			if lastCast == 293491 and _G.GetItemCooldown(167555) == 0 then
				br.pauseCast = _G.GetTime() + br.getCastTime(293491) + (2.5 * hasted) + br.getGlobalCD(true)
			end
			-- Azshara's Font of Power - Latent Arcana
			if lastCast == 296962 and _G.GetItemCooldown(169314) == 0 then
				br.pauseCast = _G.GetTime() + br.getCastTime(296962) + (2.5 * hasted)
			end
		end
		if _G.GetTime() < br.pauseCast then
			return true
		elseif _G.GetTime() >= br.pauseCast then
	...
	-- Pause Hold/Auto
	if
		(pausekey and _G.GetCurrentKeyBoardFocus() == nil and br.isChecked("Pause Mode")) or br.profileStop or
			(((_G.IsMounted() or _G.IsFlying() or br._G.UnitOnTaxi("player") or br._G.UnitInVehicle("player")) and not br.isChecked("Bypass Vehicle Check")) and
	 ...
	 then
		if (br._G.UnitCastingInfo("player") and not skipCastingCheck) then
			...
			local finish = endTime / 1000 - _G.GetTime()
			if finish > 0.1 then
				return true
			end
...
function br.getEssenceRank(essenceName)
	if _G.GetSpellInfo(essenceName) == nil then
		return 0
	end
	local essenceRank = 0
	local essenceTable = _G.C_AzeriteEssence.GetMilestones()
	local icon = select(3, _G.GetSpellInfo(essenceName))
	for i = 1, #essenceTable do
		local milestone = essenceTable[i]
		if milestone.slot ~= nil and milestone.unlocked == true then
			local eRank = _G.C_AzeriteEssence.GetEssenceInfo(_G.C_AzeriteEssence.GetMilestoneEssence(milestone.ID)).rank
			local eIcon = _G.C_AzeriteEssence.GetEssenceInfo(_G.C_AzeriteEssence.GetMilestoneEssence(milestone.ID)).icon
			if icon == eIcon then
				essenceRank = eRank
			end
		end
...
function br.sanguineCheck(unit)
	if br.sanguine then
		...
		for _, v in pairs(br.sanguine) do
			if _G.sqrt(((x - v.posX) ^ 2) + ((y - v.posY) ^ 2) + ((z - v.posZ) ^ 2)) < 5 then
				return true
			end
		end
	end
