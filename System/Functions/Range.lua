...
function br.getDistanceCalc(Unit1,Unit2,option)
    ...
    elseif select(1,_G.GetSpecializationInfo(_G.GetSpecialization())) == 255 then
        meleeSpell = testSpell["SHUNTER"]
    elseif select(1,_G.GetSpecializationInfo(_G.GetSpecialization())) == 263 then
        meleeSpell = testSpell["SHAMAN"]
    elseif select(1,_G.GetSpecializationInfo(_G.GetSpecialization())) == 577 then
        meleeSpell = testSpell["DHH"]
    elseif select(1,_G.GetSpecializationInfo(_G.GetSpecialization())) == 581 then
    ...
    then
        ...
        if br.player ~= nil then
            if br.player.talent.acrobaticStrikes ~= nil and meleeSpell ~= nil then
                if br.player.talent.acrobaticStrikes and option ~= "noMod" and _G.IsSpellInRange(select(1,_G.GetSpellInfo(meleeSpell)),Unit2) == 1 then
                    rangeMod = 3
                end
            end
        end
        ...
        if _G.GetSpecializationInfo(_G.GetSpecialization()) == 255 then
        ...
        if meleeSpell ~= nil then
            if _G.IsSpellInRange(select(1,_G.GetSpellInfo(meleeSpell)),Unit2) == 1 then
                currentDist = 0
            end
        end
    end
...
function br.getFacingDistance()
    if br.GetUnitIsVisible("player") and br.GetUnitIsVisible("target") then
        ...
        if br.GetObjectIsFacing("player","target") then
            return Dist
        else
            return -(abs(Dist))
        end
  ...
  if br.GetUnitIsVisible(Unit1) then
    ...
    for i = 1,br._G.GetObjectCount() do
      if br.GetUnitIsUnit(br._G.UnitCreator(br._G.GetObjectWithIndex(i)), "Player") and (br._G.UnitName(br._G.GetObjectWithIndex(i)) == "Searing Totem" or br._G.UnitName(br._G.GetObjectWithIndex(i)) == "Magma Totem") then
        X2,Y2,Z2 = br.GetObjectPosition(br._G.GetObjectIndex(i))
      end
    end
    local X1,Y1,Z1 = br.GetObjectPosition(Unit1)
...
function br.isSafeToAoE(spellID,Unit,effectRng,minUnits,aoeType)
    ...
    local maxRange = select(6,_G.GetSpellInfo(spellID))
...
function br.inRange(spellID,unit)
    ...
    local inRange = _G.IsSpellInRange(spellName,unit)
    if inRange ~= nil then
        return _G.IsSpellInRange(spellName,unit) == 1
    else
        return false
    end
end
