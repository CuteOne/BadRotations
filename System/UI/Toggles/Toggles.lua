...
-- Default Values
	...
	if not br.mainButton then br.mainButton = _G.CreateFrame("Button","MyButtonBR",UIParent,"SecureHandlerClickTemplate") end
...
-- Set Main Button
	...
	br.mainButton:SetScript("OnClick", function()
		if br.data.settings[br.selectedSpec].toggles['Power'] ~= 0 then
			...
			_G.GameTooltip:SetText("|cffFF0000Disable BadRotations \n|cffFFDD11Hold Left Alt and scroll mouse to adjust size.", 225/255, 225/255, 225/255)
			br.mainButton:SetNormalTexture(br.backIconOn)
			mainButtonFrame.texture:SetTexture(br.genericIconOn)
		end
	end)
	br.mainButton:SetScript("OnEnter", function()
	...
	br.mainButton:SetScript("OnLeave", function()
		_G.GameTooltip:Hide()
	end)
	br.mainButton:SetScript("OnReceiveDrag", function()
	...
	br.mainButton:SetScript("OnMouseWheel", function(delta)
	...
	mainButtonFrame = _G.CreateFrame("Frame", nil, br.mainButton)
	mainButtonFrame:SetWidth(br.data.settings["buttonSize"]*1.67)
	mainButtonFrame:SetHeight(br.data.settings["buttonSize"]*1.67)
