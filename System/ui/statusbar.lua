...
function createStatusBar(parent,option,x,y)
	...
	if  br.data.options[br.selectedSpec] then
		currentValue = br.data.options[br.selectedSpec][value.."Status"]
	end
	...
	if _G[parent..value.."Status"] == nil then
		...
		_G[parent..value.."Status"]:SetAlpha(br.data.BadRotationsUI.alpha)
		-- status part
		...
		if enabled then
			...
			-- here i need to track the current values from br.options.1.
			_G[parent..value.."Status"]:SetScript("OnMouseWheel", function(self, delta)
				...
				br.data.options[br.selectedSpec][value.."Status"] = currentValue or 0
		...
		_G[parent..value.."StatusText"]:SetAlpha(br.data.BadRotationsUI.alpha)
		_G[parent..value.."StatusText"]:SetJustifyH("CENTER")
		_G[parent..value.."StatusText"]:SetFont(br.data.BadRotationsUI.font,br.data.BadRotationsUI.fontsize,"THICKOUTLINE")
...
function createNovaStatusBar(parent,option,x,y,width,heigth)
	...
	if _G[parent..value.."Nova"] == nil then
		...
		_G[parent..value.."Nova"]:SetAlpha(br.data.BadRotationsUI.alpha)
		-- status part
		...
		_G[parent..value.."NovaText"]:SetAlpha(br.data.BadRotationsUI.alpha)
		_G[parent..value.."NovaText"]:SetJustifyH("CENTER")
		_G[parent..value.."NovaText"]:SetFont(br.data.BadRotationsUI.font,br.data.BadRotationsUI.fontsize,"THICKOUTLINE")
		_G[parent..value.."NovaText"]:SetText(currentValue, nil, nil, nil, nil, false)
		if br.friendDebug == nil then
			br.friendDebug = {}
		end
		br.friendDebug[#br.friendDebug+1] = parent..value.."Nova"
	end
end
