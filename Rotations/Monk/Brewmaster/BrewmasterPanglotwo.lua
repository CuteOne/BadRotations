...
local function actionList_Extras()
    -- Taunt
    if (br.player.ui.mode.taunt == 1 or (br.player.ui.mode.taunt == 3)) and inInstance then
    ...
    if (br.player.ui.mode.taunt == 2 or (br.player.ui.mode.taunt == 3)) then
        for i = 1, #enemies.yards30 do
            local thisUnit = enemies.yards30[i]
            if UnitThreatSituation("player", thisUnit) ~= nil and UnitThreatSituation("player", thisUnit) <= 2 and UnitAffectingCombat(thisUnit) then
                if cast.provoke(thisUnit) then return end
    ...
    if isChecked("Summon Dave - The Statue") then
        ...
        --local bX,bY,bZ
        for i = 1, 5 do
            if select(2,GetTotemInfo(i)) == "Black Ox Statue" or GetTotemInfo(i) == nil then
                castOx = false
                break
            else
                castOx = true
