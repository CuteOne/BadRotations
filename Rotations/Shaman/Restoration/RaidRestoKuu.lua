...
local function runRotation()
    if br.timer:useTimer("debugRestoration", 0.1) then
            ...
            if ui.checked("Raid Boss Helper") and br.player.eID and (br.player.eID == 2127 or br.player.eID == 2418 or br.player.eID == 2402) then
                for i = 1, br._G.GetObjectCount() do
                    ...
                    if br.player.eID ~= 2402 then
                        if (objectID == 133392 and br.getBuffRemain(thisUnit, 274148) == 0) or objectID == 171577 or objectID == 173112 then
                            if br.getHP(thisUnit) < 100 and lowest.hp >= ui.value("Raid Boss Helper") then
                                if not buff.riptide.exists(thisUnit) then
                                    br._G.CastSpellByName(br._G.GetSpellInfo(61295), thisUnit)
                                    br.addonDebug("Casting Riptide")
                                    return true
                                    --cast.riptide("target") then return true end
                                end
                                if br.getHP(thisUnit) < 50 and movingCheck then
                                    --if cast.healingSurge("target") then return true end
                                    br._G.CastSpellByName(br._G.GetSpellInfo(8004), thisUnit)
                                    br.addonDebug("Casting Healing Surge")
                                    return true
                                else
                                    --if cast.healingWave("target") then return true end
                                    if movingCheck then
                                        br._G.CastSpellByName(br._G.GetSpellInfo(77472), thisUnit)
                        ...
                        if (objectID == 165759 or objectID == 165778) and not br.shadeUp and br.getHP(thisUnit) < 100 then
                            -- Earth Shield
                            if not buff.earthShield.exists(thisUnit) and objectID == 165759 and cd.earthShield.remains() < gcdMax then
                                ...
                                br._G.CastSpellByName(br._G.GetSpellInfo(974), thisUnit)
                            ...
                            if ui.checked("Primordial Wave") and covenant.necrolord.active and cd.primordialWave.remains() < gcdMax then
                                if buff.riptide.refresh(thisUnit) then
                                    br._G.CastSpellByName(br._G.GetSpellInfo(326059), thisUnit)
                            ...
                            if ui.checked("Unleash Life") and talent.unleashLife and cd.unleashLife.remains() < gcdMax then
                                br._G.CastSpellByName(br._G.GetSpellInfo(73685), thisUnit)
        ...
        local function actionList_AMR()
            ...
            -- Earthen Wall Totem
            if ui.checked("Earthen Wall Totem") and talent.earthenWallTotem and br._G.GetTime() - br.shaman.resto["Earthen Wall Totem"] >= 2 then
        ...
        if br.pause() or cd.global.remains() > 0 then
            ...
            if inCombat and not br._G.IsMounted() and not drinking then
                if (buff.ghostWolf.exists() and mode.ghostWolf == 1) or not buff.ghostWolf.exists() then
                    ...
                    if mode.dPS == 1 and br.GetUnitExists("target") and br._G.UnitCanAttack("player", "target") and br.getFacing("player", "target") and lowest.hp > ui.value("DPS Threshold") then
                        br.addonDebug("Using DPS APL as lowest unit is at " .. tostring(lowest.hp))
                        actionList_DPS()
                    end
                    --end
                    if movingCheck and mode.dPS == 1 and br.getFacing("player", "target") then
                        if cast.lightningBolt() then
