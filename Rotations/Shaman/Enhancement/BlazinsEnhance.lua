-------------------------------------------------------
-- Author = CuteOne rework by Blazin
-- Patch = 11.0.2
...
local function createToggles()
    -- Rotation Button
    local RotationModes = {
        [1] = { mode = "Auto", value = 1 , overlay = "Automatic Rotation", tip = "Swaps between Single and Multiple based on number of targets in range.", highlight = 1, icon = br.player.spells.windstrike },
        [2] = { mode = "Mult", value = 2 , overlay = "Multiple Target Rotation", tip = "Multiple target rotation used.", highlight = 0, icon = br.player.spells.crashLightning },
...
local function createOptions()
    ...
    local function rotationOptions()
        ...
        section = br.ui:createSection(br.ui.window.profile, "General")
			...
			-- Skyfury
			br.ui:createCheckbox(section,"Skyfury")
...
local function canLightning(aoe)
    ...
    local windstrike        = (talent.stormstrike and buff.ascendance.exists()) and cd.stormstrike.remain() or 99
			...
			for i = 1, #enemies.yards30 do
				...
				if unit.interruptable(thisUnit, ui.value("Interrupt At")) then
					if cast.windShear(thisUnit) then
						ui.debug("Casting Wind Shear on " .. unit.name(thisUnit))
						return true
					end
				end
			end
...
actionList.AOE = function()
    ...
    -- Tempest
	-- Tempest
	 if cast.able.tempest() and buff.maelstromWeapon.stack() >= 6 then
        if cast.iceStrike() then ui.debug("Casting Tempest") return true end
    end
	-- Crash Lightning
    ...
    if cast.able.stormstrike() and not buff.ascendance.exists() and buff.crashLightning.exists()
        and (equiped.tier(29) >= 2 and not buff.maelstromOfElements.exists() and buff.maelstromWeapon.stack() <= 5) and buff.awakeningStorms.stack() < 2 and not buff.tempest.exists() then
    ...
    if cast.able.stormstrike() and not buff.ascendance.exists() and buff.crashLightning.exists() and buff.awakeningStorms.stack() < 2 and not buff.tempest.exists() then
    ...
    -- Earth Elemental
    -- earth_elemental
    if ui.alwaysCdNever("Earth Elemental") and cast.able.earthElemental() and ((#enemies.yards8 > 1 and unit.ttd("target") > 15) or unit.isDummy("target")) then
...
actionList.Single = function()
    ...
    if cast.able.stormstrike() and buff.doomWinds.exists() and buff.awakeningStorms.stack() < 2 and not buff.tempest.exists() then
    ...
    if cast.able.stormstrike() and not buff.ascendance.exists() and equiped.tier(29) >= 2 and not buff.maelstromOfElements.exists() and buff.maelstromWeapon.stack() <= 5 and buff.awakeningStorms.stack() < 2 and not buff.tempest.exists() then
    ...
    if cast.able.stormstrike() and not buff.ascendance.exists() and buff.awakeningStorms.stack() < 2 and (talent.deeplyRootedElements or buff.earthenWeapon.exists() or buff.legacyOfTheFrostWitch.exists()) then
...
actionList.PreCombat = function()
    ...
    if not unit.inCombat() and not (unit.flying() or unit.mounted() or unit.taxi()) then
		...
		-- Skyfury
		if ui.checked("Skyfury") and cast.able.skyfury() and not buff.skyfury.exists() then
            if cast.skyfury("player") then ui.debug("Casting Skyfury") return true end
		end
        -- Lightning Shield
        -- lightning_shield
...
local function runRotation()
    ...
    elseif (unit.inCombat() and var.profileStop==true) or ui.pause() or unit.mounted() or unit.flying() or ui.mode.rotation == 4 then
        ...
        if unit.inCombat() and unit.valid("target") and not var.profileStop then
            ...
            if ui.alwaysCdNever("Ascendance") and cast.able.ascendance() and ((#enemies.yards8 > 1 and unit.ttd("target") > 15) or unit.isDummy("target")) then
