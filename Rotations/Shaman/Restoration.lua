...
--[[ 	-- Combats Starts Here
	...
	--if isInCombat("player") then
		...
		--if isAlive() and (isEnnemy() or isDummy("target")) and getLineOfSight("player","target") == true and getFacing("player","target") == true then
			...
			-- Chain Heal
			if nNova[2] ~= nil and nNova[2].hp <= 80 then
				if #getAllies(nNova[2].unit,30) >= 3 then
					if castSpell(nNova[2].unit,_ChainHeal,true) then return; end
				end
			end
			...
			-- Riptide
			for i = 1, #nNova do
				if nNova[i].hp <= 99 and getBuffRemain(nNova[i].unit,_Riptide) < 3 then
