	...
	local function createOptions()
        ...
        -- Cooldown Options
            ...
            -- Fire/Storm Elemental Totem
                bb.ui:createCheckbox(section,"Fire/Storm Elemental Totem")
            bb.ui:checkSectionState(section)
        -- Defensive Options
            ...
            -- Earth Elemental Totem
                bb.ui:createSpinner(section, "Earth Elemental Totem", 30, 0, 100, 5, "|cffFFFFFFHealth Percent to Cast At")
            -- Earthquake Totem
                bb.ui:createSpinner(section, "Earthquake Totem", 40, 0, 100, 5, "|cffFFFFFFHealth Percent to Cast At")
            -- Healing Surge
            ...
            -- Thunderstorm
                bb.ui:createSpinner(section, "Thunderstorm", 25, 0, 100, 5, "|cffFFFFFFHealth Percent to Cast At")
            bb.ui:checkSectionState(section)
        -- Interrupt Options
            ...
            -- Thunderstorm
                bb.ui:createCheckbox(section,"Thunderstorm - Interrupt")
            -- Earthquake Totem
                bb.ui:createCheckbox(section,"Earthquake Totem - Interrupt")
	...
	--------------------
		...
		-- Action List - Cooldowns
            ...
            -- Racial: Orc Blood Fury | Troll Berserking | Blood Elf Arcane Torrent
                    ...
                    if isChecked("Racial") and (bb.player.race == "Orc" or bb.player.race == "Troll" or bb.player.race == "Blood Elf") and ((talent.elementalMastery and buff.elementalMastery) or not talent.elementalMastery) then
                        if castSpell("player",racial,false,false,false) then return end
                    end
        ...
        -- Action List - Single Target
            ...
            -- Ascendance
                ...
                if debuff.remain.flameShock > buff.duration.ascendance and (combatTime >= 60 or (buff.bloodlust or buff.heroism)) and cd.lavaBurst > 0 and not buff.stormkeeper then
            ...
            -- Flame Shock 
                ...
                if power >= 20 and debuff.remain.flameShock <= buff.duration.ascendance and cd.ascendance + buff.duration.ascendance <= debuff.duration.flameShock then
                    if cast.flameShock() then return end
                end
            -- Earth Shock
            ...
            -- Lava Burst
                ...
                if debuff.remain.flameShock > getCastTime(spell.lavaBurst) and (cd.lavaBurst == 0 or buff.ascendance) then
            ...
            -- Frost Shock
                ...
                if talent.icefury and buff.icefury and ((power >= 20 and moveIn > buff.remain.icefury) or buff.remain.icefury < (1.5 * UnitSpellHaste("player") * buff.stack.icefury)) then
                    if cast.frostShock() then return end
                end
                if moving and buff.icefury then
                    if cast.frostShock() then return end
                end
            -- Earth Shock
            ...
            -- Ice Fury
                ...
                if power <= 70 and moveIn > 30 and ((talent.ascendance and cd.ascendance > buff.duration.icefury) or not talent.ascendance) then
                    if cast.icefury() then return end
                end
            -- Liquid Magma
            ...
            -- Totem Mastery 
                ...
                if (resonanceTotemTimer < 10 or (resonanceTotemTimer < (buff.duration.ascendance + cd.ascendance) and cd.ascendance < 15)) and lastSpell ~= spell.totemMastery then
                    if cast.totemMastery() then resonanceTotemCastTime = GetTime() + 120; return end
                end
            -- Lava Beam
            ...
            -- Chain Lightning
                ...
                if #enemies.yards8 > 1 and not mode.rotation == 3 and not debuff.lightningRod then
    ...
    --------------------------
        ...
        ---------------------------
                ...
                -- Racials
                            ...
                            if not talent.ascendance or buff.ascendance or cd.ascendance > 50 then
                                if race == "Orc" then
                                    if castRacial() then return end
                                end
                            end
                            if not talent.ascendance or buff.ascendance then
                                if race == "Troll" then
                                    if castRacial() then return end
                                end
                            end
