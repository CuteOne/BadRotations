...
local function runRotation()
        ...
        if prepullOpener and movingCheck then
            ...
            if talent.elementalBlast then
                ...
                if cast.lavaBurst() then
                    br.addonDebug("Casting Lavaburst")
                    return
                end
            end
        end
    ...
    local function actionList_Defensive()
        ...
        if useDefensive() then
            -- Pot/Stoned
            if ui.checked("Pot/Stoned") and php <= ui.value("Pot/Stoned") and inCombat and (hasHealthPot() or br.hasItem(5512) or br.hasItem(166799)) then
            ...
            -- Healing Surge
            if ui.checked("Healing Surge") and movingCheck and ((mana >= 90 and php <= ui.value("Healing Surge")) or (php <= ui.value("Healing Surge") / 2 and mana > 20)) then
                if cast.healingSurge("player") then
                    br.addonDebug("Casting Healing Surge")
                    return
                end
    ...
    local function actionList_AoE()
        ...
        if talent.echoingShock and power >= 60 then
            if cast.echoingShock() then
                br.addonDebug("Casting Echoing Shock")
                return
            end
        end
        -- Ascendance
        ...
        if runeforge.echoesOfGreatSundering.equiped and not buff.echoesOfGreatSundering.exists() then
            if cast.earthShock() then
                br.addonDebug("Casting Earth Shock (EotGS)")
                return
            end
        end
        --Earth Elemental
        ...
        if
         ...
         then
            if cast.earthElemental() then
                br.addonDebug("Casting Earth Elemental (Deeptremor Stone)")
                return
            end
        end
        --Lava Burst
        ...
        if
            #enemies.yards8t >= ui.value("Earthquake Targets") and
                (not talent.masterOfTheElements or buff.stormKeeper.exists() or power >= ui.value("Earth Shock Maelstrom Dump") or buff.masterOfTheElements.exists() or
    ...
    local function actionList_StormEleST()
        ...
        if
            (runeforge.echoesOfGreatSundering.equiped and buff.echoesOfGreatSundering.exists()) or
        ...
        if (#enemies.yards10t == 0 and power >= 60 and (buff.windGust.stack() < 20 or power > 90)) then
            if cast.earthShock() then
                br.addonDebug("Casting Earth Shock")
                return
            end
        end
        -- Lightning Bolt
        ...
        if talent.staticDischarge then
            if cast.staticDischarge() then
                br.addonDebug("Casting Static Discharge")
                return
            end
        end
        -- Earth Elemental
    ...
    local function actionList_ST()
        ...
        if flameShockCount < #enemies.yards40f then
            for i = 1, #enemies.yards40f do
                if
                    debuff.flameShock.remains(enemies.yards40f[i]) <= 5.4 or
                        (talent.ascendance and debuff.flameShock.remains(enemies.yards40f[i]) < (cd.ascendance.remains() + buff.ascendance.duration()) and
                            cd.ascendance.remains() < 4)
                 then
                    if cast.flameShock(enemies.yards40f[i]) then
                        br.addonDebug("Casting Flameshock")
                        return
        ...
        if
            movingCheck and talent.elementalBlast and ((talent.masterOfTheElements and buff.masterOfTheElements.exists() and power < 60) or not talent.masterOfTheElements) and
                holdBreak
         then
            if cast.elementalBlast() then
                br.addonDebug("Casting Elemental Blast")
    ...
    elseif pause() or cd.global.remains() > 0 or (UnitExists("target") and not UnitCanAttack("target", "player")) or mode.rotation == 4 or br.isCastingSpell(293491) then
        return
    else
        ---------------------------------
