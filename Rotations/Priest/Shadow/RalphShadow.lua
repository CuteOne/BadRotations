...
local function CwC()
    ...
    if not pool and cast.current.mindFlay() and mfTicks >= 1 and (not debuff.devouringPlague.exists() or power > 90) then
        if cast.devouringPlague("target") then
            ui.debug("Cancel Mindflay to DP target [CwC]")
            return
        end
    end
                ...
                if not talent.deathAndMadness and getHP(thisUnit) < 20 or talent.deathAndMadness and getHP(thisUnit) and ttd(thisUnit) < 7 then
                    if not noDotCheck(thisUnit) then
                        if cast.shadowWordDeath(thisUnit) then
                            ui.debug("Casting SW:D on low enemies [CwC]")
                            return
                        end
                    end
                end
    ...
    if (useCDs() and cd.voidEruption.exists() or not useCDs()) and ((snmCutoff and not pool) or (not swpCheck and #searEnemies > 1)) and cast.current.mindSear() then
        if power > 30 then
            if cast.searingNightmare("target") then
                ui.debug("Searing nightmare if will hit 3+ targets [CwC]")
                return
            end
        end
    end
    ...
    if
        ((useCDs() and cd.voidEruption.exists()) or not useCDs()) and talent.searingNightmare and not swpCheck and #searEnemies > 2 and
            select(1, UnitChannelInfo("player")) == GetSpellInfo(48045) and
            power > 30
     then
        if cast.searingNightmare("target") then
            ui.debug("SNM to refresh SW:P [CwC]")
            return
        end
    end
    -- Consume dark thoughts proc while channeling mindflay or mind sear
    if (cast.current.mindFlay() or cast.current.mindSear()) then
        if buff.darkThoughts.exists() then
            if cast.mindBlast("target") then
                ui.debug("Consuming dark thoughts before SnM if insanity < 30 [CwC]")
                return
            end
        end
    end
...
actionList.Defensive = function()
    ...
    if isChecked("Fade") then
        if not solo and UnitThreatSituation("player") ~= nil and UnitThreatSituation("player") > 1 then
            if cast.fade("player") then
                ui.debug("Defensive - Casting fade")
                return
            end
        end
    end
    ...
    if ui.checked("Dispersion") and inCombat and php <= ui.value("Dispersion") then
        if cast.dispersion() then
            ui.debug("Casting dispersion [Defensive]")
            return
        end
    end
    -- Dispel Magic
    ...
    -- PowerWord: Shield
    if
        not debuff.weakenedSoul.exists("player") and
            (ui.checked("PWS: Body and Soul") and talent.bodyAndSoul and moving and not IsFalling() or inCombat and php <= ui.value("Power Word: Shield"))
     then
        if cast.powerWordShield("player") then
            ui.debug("BnS or low health PW:S [Defensive]")
            return
        end
    end
    -- Shadow Mend
    if
        cast.able.shadowMend() and not moving and
            (ui.checked("Shadow Mend OOC") and php <= ui.value("Shadow Mend OOC") and not inCombat or ui.checked("Shadow Mend") and php <= ui.value("Shadow Mend") and inCombat)
     then
            ...
            for i = 1, #enemies.yards30 do
                ...
                if unit.interruptable(thisUnit, ui.value("Interrupt At")) then
                    if cast.psychicHorror(thisUnit) then
                        return
                    end --Print("pH on any") return end
                end
            end
        end
        -- Psychic Scream
...
actionList.Cooldown = function()
    ...
    if useCDs() then
        ...
        if covenant.kyrian.active then
            if
                not voidform and cd.voidEruption.exists() and #searEnemies > 1 and not talent.searingNightmare or
                    (voidform and #searEnemies < 2 and not talent.searingNightmare and cast.last.voidBolt()) or
                    (voidform and talent.searingNightmare)
             then
                if cast.boonOfTheAscended() then
                    return
                end
            end
        end
            ...
            if pot_use then
                if buff.voidForm.exists() then
                    if canUseItem(171349) then
                        if useItem(171349) then
                            ui.debug("Pot is good [Cooldown]")
                            return
                        end
                    end
                end
        ...
        if ui.checked("Use Trinkets") then
            ...
            if
                trinket13 ~= 180117 and trinket13 ~= 179350 and trinket13 ~= 184024 and trinket13 == 178809 and trinket13 ~= 175921 and trinket13 ~= 173069 and trinket13 ~= 178826 and
                    trinket13 ~= 184021 and
                    trinket13 ~= 184030
             then
            ...
            if
                trinket14 ~= 180117 and trinket14 ~= 179350 and trinket14 ~= 184024 and trinket14 == 178809 and trinket14 ~= 175921 and trinket14 ~= 173069 and trinket14 ~= 178826 and
                    trinket14 ~= 184021 and
                    trinket14 ~= 184030
             then
    ...
    if not unit.inCombat() and not (unit.flying() or unit.mounted() or unit.taxi()) then
        ...
        if ui.checked("VT OoC") and not debuff.vampiricTouch.exists() and not moving then
            if cast.vampiricTouch("target") then
                ui.debug("Pulling with VT [Pre-Combat]")
                return
            end
        end
        -- Pre-Pull
        if ui.checked("Pre-Pull Timer") and ui.pullTimer() <= cast.time.vampiricTouch() then
            if cast.vampiricTouch("target") then
                ui.debug("Casting Vampiric Touch [Pre-Combat]")
                return
            end
        end
    end
...
actionList.Main = function()
    ...
    if buff.boonOfTheAscended.exists() then
        ...
        if #searEnemies <= 3 then
            if createCastFunction("target", nil, 1, nil, 325283) then
                ui.debug("Casting Ascended Blast [Main]")
                return
            end
        end
        ...
        if #novaEnemies > 1 and #searEnemies > 1 + snmEnabled() then
            if createCastFunction("target", nil, 1, nil, 325020) then
                ui.debug("Casting Ascended Nova [Main]")
                return
            end
        end
    end
    ...
    if
        voidform and
            (power <= 85 and talent.hungeringVoid and talent.searingNightmare and #searEnemies <= 6 or ((talent.hungeringVoid and not talent.searingNightmare) or #searEnemies == 1))
     then
        if cast.voidBolt("target") then
            ui.debug("Casting Void Bolt [Main]")
            return
        end
    end
    ...
    if (voidform or buff.dissonantEchoes.exists()) and (#searEnemies < (4 + dEchoesCheck()) and power <= 85 and talent.searingNightmare or not talent.searingNightmare) then
        if buff.dissonantEchoes.exists() then
            if cast.devoidBolt("target") then
                ui.debug("Casting Void Bolt to consume Dissonant Echoes proc [Main]")
                return
            end
        else
            if cast.voidBolt("target") then
                ui.debug("Casting Void Bolt on target [Main]")
                return
            end
        end
    end
    ...
    if cd.shadowWordDeath.ready() then
        for i = 1, #enemies.yards40 do
            ...
            if not talent.deathAndMadness and getHP(thisUnit) < 20 or talent.deathAndMadness and ttd(thisUnit) < 7 then
                if not noDotCheck(thisUnit) then
                    if cast.shadowWordDeath(thisUnit) then
                        ui.debug("SW:D on low enemies [Main]")
                        return
                    end
                end
            end
    ...
    if not debuff.vampiricTouch.exists() and not moving and #searEnemies < searCutoff then
        if cast.vampiricTouch("target") then
            ui.debug("Casting VT on target [Main]")
            return
        end
    end
    ...
    if useCDs() and not voidform and not moving and power >= 40 and (power <= 85 or (talent.searingNightmare and snmCutoff)) and cd.shadowfiend.exists() then
        if cast.voidEruption() then
            ui.debug("Casting Void Eruption [Main]")
            return
        end
    end
    ...
    if buff.faeGuardians.exists() and not debuff.wrathfulFaerie.exists("target") then
        if cast.shadowWordPain("target") then
            ui.debug("Casting SW:P on target [Main]")
            return
        end
    end
    ...
    if
        talent.shadowCrash and cd.shadowCrash.ready() and ui.checked("Shadow Crash") and ttd("target") > 3 and
            ((not tankMoving and (inRaid or inInstance)) or (not inInstance and not inRaid and not isMoving("target")))
     then
        if cast.shadowCrash("best", nil, 1, 8) then
            ui.debug("Casting Shadow Crash [Main]")
            SpellStopTargeting()
            return
        end
    end
    ...
    if
        ((useCDs() and cd.voidEruption.exists()) or not useCDs()) and talent.searingNightmare and #searEnemies > searCutoff and not moving and power > 30 and
            not cast.current.mindSear()
     then
        if cast.mindSear("target") then
            ui.debug("Mind Sear on 4+ [Main]")
            return
        end
    end
    ...
    if debuff.shadowWordPain.remain() < SWPmaxTargets then
        for i = 1, #enemies.yards40 do
            local thisUnit = enemies.yards40[i]
            if
                debuff.shadowWordPain.refresh(thisUnit) and not talent.misery and not (talent.searingNightmare and #searEnemies > searCutoff) and
                    (not talent.psychicLink or (talent.psychicLink and #searEnemies <= 2))
             then
                if not noDotCheck(thisUnit) then
                    if cast.shadowWordPain(thisUnit) then
                        ui.debug("Spreading SW:P [Main]")
                        return
                    end
                end
            end
    ...
    if not moving and talent.searingNightmare and power > 30 and #searEnemies > searCutoff and not swpCheck and cd.shadowfiend.exists() then
        if cast.mindSear("target") then
            ui.debug("Casting Mind Sear to refresh SW:P [Main]")
            return
        end
    end
    ...
    if debuff.vampiricTouch.count() < VTmaxTargets and not cast.last.vampiricTouch() then
        for i = 1, #enemies.yards40 do
            local thisUnit = enemies.yards40[i]
            if
                not moving and not cast.current.vampiricTouch() and
                    (debuff.vampiricTouch.refresh(thisUnit) and ttd(thisUnit) > 6 or (talent.misery and debuff.shadowWordPain.refresh() or buff.unfurlingDarkness.exists()))
             then
                if not noDotCheck(thisUnit) then
                    if cast.vampiricTouch(thisUnit) then
                        ui.debug("Spreading VT to enemies [Main]")
                        return
                    end
                end
            end
    ...
    if talent.damnation and not allDotsUp then
        if cast.damnation("target") then
            ui.debug("Casting Damnation on target [Main]")
            return
        end
    end
    ...
    if (debuff.devouringPlague.refresh("target") or power > 75) and (not pool or power >= 85) and (not talent.searingNightmare or (talent.searingNightmare and not snmCutoff)) then
        if cast.devouringPlague("target") then
            ui.debug("Casting DP on target [Main]")
            return
        end
    end
    ...
    if not talent.deathAndMadness and thp < 20 and #searEnemies < 4 or talent.deathAndMadness and ttd("target") < 7 then
        if cast.shadowWordDeath("target") then
            ui.debug("Casting SW:D on low target [Main]")
            return
        end
    end
    for i = 1, #enemies.yards40 do
        ...
        if not talent.deathAndMadness and getHP(thisUnit) < 20 and #searEnemies < 4 or talent.deathAndMadness and ttd(thisUnit) < 7 then
            if not noDotCheck(thisUnit) then
                if cast.shadowWordDeath(thisUnit) then
                    ui.debug("Casting SW:D on low enemies [Main]")
                    return
                end
            end
        end
    ...
    if ui.checked("Surrender to Madness") and talent.surrenderToMadness and ttd("target") < 25 and not buff.voidForm.exists() then
        if cast.surrenderToMadness() then
            ui.debug("Casting Surrender to Madness [Main]")
            return
        end
    end
    ...
    if
        not buff.boonOfTheAscended.exists() and not moving and dotsUp and ttd("target") > 5 and not voidform and not cast.last.voidEruption() and not vtCheck() and
            #searEnemies < (5 + (6 * ToF())) and
            power <= 60
     then
        if cast.voidTorrent("target") then
            ui.debug("Casting Void Torrent on target [Main]")
            return
        end
    end
    ...
    if useCDs() and debuff.vampiricTouch.exists("target") and (talent.searingNightmare and #searEnemies > searCutoff or debuff.shadowWordPain.exists("target")) then
        if talent.mindBender and ttd("target") >= 6 then
            if cast.mindBender("target") then
                ui.debug("Casting Mindbender [Main]")
                return
            end
        else
            if cast.shadowfiend("target") then
                ui.debug("Casting Shadowfiend [Main]")
                return
            end
        end
    end
    ...
    if covenant.venthyr.active and not cast.last.voidEruption() and not moving then
        if cast.mindgames("target") then
            ui.debug("Casting Mindgames [Main]")
            return
        end
    end
    ...
    if not moving and dotsUp and (#searEnemies < 4 and not talent.misery or #searEnemies < 6 and talent.misery) then
        if cast.mindBlast("target") then
            ui.debug("Casting Mind Blast on target [Main]")
            return
        end
    end
    ...
    if
        not moving and not cast.current.vampiricTouch() and
            (debuff.vampiricTouch.refresh("target") and ttd("target") > 6 or (talent.misery and debuff.shadowWordPain.refresh() or buff.unfurlingDarkness.exists()))
     then
        ...
        for i = 1, #enemies.yards40 do
            ...
            if
             ...
             then
                if not noDotCheck(thisUnit) then
                    if cast.vampiricTouch(thisUnit) then
                        ui.debug("Refreshing VT on enemies [Main]")
                        return
                    end
                end
            end
    ...
    if
        debuff.shadowWordPain.refresh("target") and ttd("target") > 4 and not talent.misery and not (talent.searingNightmare and #searEnemies > searCutoff) and
            (not talent.psychicLink or (talent.psychicLink and #searEnemies <= 2))
     then
        if cast.shadowWordPain("target") then
            ui.debug("Casting SW:P on target [Main]")
    ...
    if debuff.shadowWordPain.count() < SWPmaxTargets then
        for i = 1, #enemies.yards40 do
            local thisUnit = enemies.yards40[i]
            if
                debuff.shadowWordPain.refresh(thisUnit) and ttd(thisUnit) > 4 and not talent.misery and not (talent.searingNightmare and #searEnemies > searCutoff) and
                    (not talent.psychicLink or (talent.psychicLink and #searEnemies <= 2))
             then
                if not noDotCheck(thisUnit) then
                    if cast.shadowWordPain(thisUnit) then
    ...
    if dotsUp and not moving and #searEnemies <= searCutoff and cd.mindBlast.exists() and cd.voidTorrent.exists() then
        if cast.mindFlay("target") then
            ui.debug("Casting Mindflay on target [Main]")
            return
        end
    end
    ...
    if not talent.searingNightmare and #searEnemies > searCutoff and not moving and cd.mindBlast.exists() and cd.voidTorrent.exists() then
        if cast.mindSear("target") then
            ui.debug("Casting Mind Sear on target [Main]")
            return
        end
    end
    ...
    if moving then
        if cast.shadowWordPain("target") then
            ui.debug("Spamming SW:P on target while moving [Main]")
            return
        end
    end
