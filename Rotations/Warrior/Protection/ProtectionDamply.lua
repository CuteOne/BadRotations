...
--------------------
    ...
    -- Action List - Main
        ...
        -- Ignore Pain
            ...
            if (rage >= 25 and not talent.vengeance) or (buff.vengeanceIgnorePain.exists and buff.ultimatum.exists) or (buff.vengeanceIgnorePain.exists and rage >= 39) 
        ...
        -- Shield Slam
            -- shield_slam,if=!(cooldown.shield_block.remains<=gcd.max*2&!buff.shield_block.up&talent.heavy_repercussions.enabled)
            if (talent.heavyRepercussions) then
                if cast.shieldSlam() then return end
            end
        -- Revenge
            -- revenge,if=cooldown.shield_slam.remains<=gcd.max*2
            if cd.shieldSlam then
                if cast.revenge() then return end
            end
        -- Devastate
            -- devastate
