local rotationName = "WinterzBETA" -- Change to name of profile listed in options drop down
--
--
--
--
    ...
    br.ui:createToggle(InterruptModes,"Interrupt", 4, 0)
        ...
        local TorghastMode = {
            [1] = {
                mode = "On",
                value = 1,
                overlay = "Torghast Mode ON",
                tip = "Will use special logic for Torghast",
                highlight = 1,
                icon = br.player.spell.heroicThrow
            },
            [2] = {
                mode = "Off",
                value = 2,
                overlay = "Torghast Mode OFF",
                tip = "No special logic will be used",
                highlight = 0,
                icon = br.player.spell.heroicThrow
            }
        }
        br.ui:createToggle(TorghastMode,"Torghast", 1, 1)
...
local function createOptions()
    ...
    local function rotationOptions()
        ...
        --- GENERAL OPTIONS ---
        -----------------------
        section = br.ui:createSection(br.ui.window.profile, "General - \124cffffff00Version 1.1.2\124r")
        -- Battle Shout
...
-- BT if Fresh Meat and not Enraged   test later
        ...
        -- Dragon Roar
        if buff.enrage.exists("player") and br.isChecked("Dragon Roar") and cast.able.dragonRoar() then
            if cast.dragonRoar() then
                return
            end
        end
        ...
        -- Bladestorm
        if br.isChecked("Bladestorm AoE Units") and (cd.spearOfBastion.remain() > 5) and #enemies.yards8 >= br.getOptionValue("Bladestorm AoE Units") and buff.enrage.exists("player") and br.player.ui.mode.cooldown ~= 3 then
