local rotationName = "ForsoothPreservation" -- Change to name of profile listed in options drop down
    ...
    elseif var.haltProfile and cast.current.id() ~= br.empowerID then
        ...
        if unit.valid("target") and (cd.global.remain() == 0 or var.fireBreathStage > 0) then
            if unit.exists(units.dyn40) and unit.distance(units.dyn40) < 40 then
                ...
                -- Living Flame
                if cast.able.livingFlame(units.dyn25) and var.moveCast and ui.useST(25,2) and cast.last.fireBreath(1) then
...
local id = 1468 -- Change to the spec id profile is for.
