...
local function runRotation()
    ...
    -- Eternity Surge - Stage per # of Units
    var.eternityTargetStages = 1
    if #enemies.yards12f >= 4 then var.eternityTargetStages = 4 end
    if #enemies.yards12f > 1 and #enemies.yards12f < 4 then var.eternityTargetStages = #enemies.yards12f end

    if #enemies.yards12f == 0 then
    ...
    elseif var.haltProfile then
        ...
        -----------------------------
        -- Check for combat
        if unit.valid("target") and (cd.global.remain() == 0 or var.fireBreathStage > 0 or var.eternitySurgeStage > 0) then
            if unit.exists(units.dyn40) and unit.distance(units.dyn40) < 40 then
                ...
                -- Shattering Start
                if cast.able.shatteringStar() then
                    if cast.shatteringStar() then ui.debug("Casting Shattering Star") return true end
                end
                -- Eternity Surge
                if cast.able.eternitySurge() and var.moveCast then
                    if var.eternitySurgeStage == 0 and not cd.eternitySurge.exists() then
                        if cast.eternitySurge() then ui.debug("Casting Eternity Surge") return true end
                    end
                    if var.eternitySurgeStage >= var.eternityTargetStages then
                        if cast.eternitySurge() then ui.debug("Casting Eternity Surge - Empowered Stage "..var.eternitySurgeStage) return true end
                    end
                end
                -- Disintegrate
                if cast.able.disintegrate() and (essence >= 3 or buff.essenceBurst.exists()) and var.moveCast
                    and buff.essenceBurst.exists() and #enemies.yards25f < 3
                then
                    if cast.disintegrate() then ui.debug("Casting Disintegrate") return true end
                end
                -- Pyre
                if cast.able.pyre() and (essence >= 2 or buff.essenceBurst.exists()) and #enemies.yards25f >= 3 then
                    if cast.pyre() then ui.debug("Casting Pyre") return true end
                end
                -- Living Flame
                if cast.able.livingFlame(units.dyn25) and var.moveCast
                    and (essence < 3 or not buff.essenceBurst.exists()) and #enemies.yards25f < 3
                then
                    if cast.livingFlame(units.dyn25) then ui.debug("Casting Living Flame") return true end
                end
                -- Azure Strike
                if cast.able.azureStrike() and (not var.moveCast or #enemies.yards25f > 2) and (essence < 2 or not buff.essenceBurst.exists()) then
