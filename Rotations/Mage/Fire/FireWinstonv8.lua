...
--------------------
                    ...
                    if isChecked("Pull OoC") --[[and isChecked("No Crit-Opener")]] and isValidUnit("target") then
                        if not moving and not buff.hotStreak.exists() then
                            if cast.fireball() then --fballLast = true 
                                ...
                                return true end
                        elseif moving then
                            if cast.scorch("target") then --Print("opener scorch 2")
                                return true end 
                        end
                    --end
                end
    ...
    -- Action List - Combustion Phase
        ...
        -- Fire Blast
            ...
            -- elseif lucisDreams and buff.combustion.exists() and not buff.heatingUp.exists() and cast.last.pyroblast() then
...
--------------------------
    ...
    ----------------------
        ...
        -- Action List - Combustion Phase
                    ...
                    if useCDs() then --and (not traits.blasterMaster.active or not talent.flameOn) then --and firestarterInactive then 
