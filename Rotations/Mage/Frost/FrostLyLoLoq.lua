...
local function runRotation()
    ...
    local function SimCAPLMode()
            ...
            if useCDs() then
                ...
                if isChecked("Icy Veins") then
                    if not buff.icyVeins.exists() then
                        if cast.icyVeins() then return true end
                    end
                end
        ...
        local function actionList_AOE()
            ...
            if useCDs() and isChecked("Frozen Orb") then
                if cast.frozenOrb() then return true end
            end
            --actions.aoe+=/blizzard
            if mode.rotation == 1 or mode.rotation == 2 then
                if cast.blizzard("best", nil, 4, 8) then return true end
            end
            --actions.aoe+=/comet_storm
            if useCDs() and isChecked("Comet Storm") then
                if cast.cometStorm() then return true end
            end
            ...
            if lastSpell == spell.frostbolt and buff.fingersOfFrost.stack() < (2 + iceHand) and not buff.brainFreeze.exists() then
                if cast.waterJet() then return true end
            end
            ...
            if not debuff.frostBomb.exists() or debuff.frostBomb.remain() < 2 and fof_react > 0 and ttdUnit >= 12 + getCastTime(spell.frostBomb)+0.5 then
                if cast.frostBomb() then return true end
            end
            ...
            if (getOptionValue("Artifact") == 1 or (getOptionValue("Artifact") == 2 and useCDs())) then
                if not buff.brainFreeze.exists() then
                    if cast.ebonbolt() then return true end
                end
            end
            --actions.aoe+=/glacial_spike
            if buff.icicles.stack() == 5 then
                if cast.glacialSpike() then return true end
            end
        ...
        local function actionList_SINGLE()
            ...
            if mode.rotation == 1 or mode.rotation == 2 then
                if getCastTime(spell.blizzard) == 0 and #enemies.yards40 > 1 and fof_react < 3 then
                    if cast.blizzard("best", nil, 4, 8) then return true end
                end
            end
    ...
    local function IcyVeinsAPLMode()
        ...
        local function actionList_COMBAT()
            ...
            if useCDs() and isChecked("Rune of Power") then
            ...
            if useCDs() and isChecked("Icy Veins") then
                if not buff.icyVeins.exists() then
                    if cast.icyVeins() then return true end
                end
            end
            ...
            if isChecked("Use Pet Spells") then
                ...
                if not isBoss(target) and (buff.fingersOfFrost.stack() <= 2 or (buff.fingersOfFrost.stack() <= 1 and not artifact.icyHand)) then
                    if cast.freeze(target,"best", 1, 8) then return true end
                end
            end
            ...
            if mode.rotation == 1 or mode.rotation == 2 and #getEnemies(target, 8) >=4 then
                if cast.blizzard("best", nil, 4, 8) then return true end
            end
    ...
    if not executando and (getSpellCD(61304) == 0 or lastGCD) then
        --        if  br.timer:useTimer("bla", 0.2) then
        executando = true
        profile()
        executando = false
        --        end
