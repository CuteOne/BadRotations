...
local function createToggles()
    -- Rotation Button
    local RotationModes = {
        [1] = {mode = "Auto", value = 1, overlay = "Automatic Rotation", tip = "Swaps between Single and Multiple based on number of targets in range.", highlight = 1, icon = br.player.spell.arcaneExplosion},
...
actionList.Defensive = function()
    ...
    if unit.hp() < 95 then
        if cast.able.frostNova("player","aoe",1,12) then
            if cast.frostNova("player","aoe",1,12) then ui.debug("Casting Frost Nova") return true end
        end
    end
    ...
    elseif haltProfile then
        ...
        if actionList.PreCombat() then return true end
            ...
            if cast.able.arcaneIntellect("player") and not buff.arcaneIntellect.exists("player") then
                if cast.arcaneIntellect("player") then ui.debug("Casting Arcane Intellect") return true end
            end
        -----------------------------
        --- In Combat - Rotations ---
        -----------------------------
        ...
        if unit.valid("target") and cd.global.remain() == 0 then
            if unit.exists(units.dyn40) and unit.distance(units.dyn40) < 40 then
                ...
                if cast.able.arcaneExplosion("player","aoe",1,8) then
                    if cast.arcaneExplosion("player","aoe",1,8) then ui.debug("Casting Arcane Explosion") return true end
                end
                --Fire Blast
