    ...
    elseif var.haltProfile then
        ...
        if unit.valid("target") and cd.global.remain() == 0 then
            if unit.exists(units.dyn40) and unit.distance(units.dyn40) < 40 then
                ...
                -- actions=auto_attack
                if not br._G.IsCurrentSpell(6603) and unit.distance("target") <= 5 then
                ...
                if cast.able.deathAndDecay("playerGround") and not buff.deathAndDecay.exists() and #enemies.yards8 > 2 then
                    if cast.deathAndDecay("playerGround") then ui.debug("Casting Death and Decay") return true end
                end
