...
----------------
    ...
    --------------------
        -- Action List - Extras
            ...
            -- Dark Simulacrum
                if simSpell~=_DarkSimulacrum and getBuffRemain("player",_DarkSimulacrum)>0 and getCastTimeRemain(simUnit)<8 then
                    if castSpell(simUnit,simSpell,false,false,true,true,false,true,false) then br.im.simulacrum = nil return end
                    --CastSpellByName(GetSpellInfo(simSpell),tarUnit.dyn40)
                end
                if simSpell~=nil and getBuffRemain("player",_DarkSimulacrum)==0 then
                    br.im.simulacrum = nil
                end
        ...
        -- Action List - Utility
            ...
            -- Chains of Ice
                for i = 1, #dynTable30AoE do
                    ...
                    local chainsOfIceRemain = getDebuffRemain(thisUnit,br.player.spell.chainsOfIce,"player") or 0
                    if not getFacing(thisUnit,"player") and getFacing("player",thisUnit) and isMoving(thisUnit) and getDistance(thisUnit)>8 and inCombat then
                        if br.player.castChainsOfIce() then return end
                    end
                end
            -- Death's Advance
                -- deaths_advance,if=movement.remains>2
                if isMoving("player") and getNumEnemies("player",20)==0 and inCombat then
                    if br.player.castDeathsAdvance() then return end
                end
            -- Death Grip
                if isChecked("Death Grip") and ((solo and #br.friend==1) or hasThreat(br.player.units.dyn30AoE)) then
                    if inCombat then
                        if br.player.castDeathGrip(thisUnit) then return end
                    end
                end
            -- Gorefiend's Grasp
                if isChecked("Gorefiend's Grasp") and ((solo and #br.friend==1) or hasThreat(br.player.units.dyn20AoE)) then
                    if br.player.castGorefiendsGrasp() then return end
                end
            -- Unholy Presence
                -- if not buff.unholyPresence and moving and (not inCombat or (inCombat and getDistance(br.player.units.dyn5)>10)) then --and (power<40 or (power<70 and glyph.shiftingPresences)))) then
                --     if br.player.castUnholyPresence() then return end
                -- end
            -- Frost Presence
                if not buff.frostPresence --[[and php > getOptionValue("Blood Presence")]] and getDistance(br.player.units.dyn5)<=10 then --and (power<40 or (power<70 and glyph.shiftingPresences)) then
                    if br.player.castFrostPresence() then return end
                end
            -- Path of Frost
                if not inCombat and swimming and not buff.pathOfFrost then
                    if br.player.castPathOfFrost() then return end
                end
            -- Raise Ally
                if isChecked("Mouseover Targetting") then
                    if br.player.castRaiseAllyMouseover() then return end
                else
                    if br.player.castRaiseAlly() then return end
                end
        ...
        -- Action List - Defensive
            ...
            -- Anti-Magic Shell
                    ...
                    if isChecked("Anti-Magic Shell") and php<=getOptionValue("Anti-Magic Shell") and incombat and not talent.antiMagicZone and ((buff.breathOfSindragosa and power<25) or cd.breathOfSindragosa>40 or (not talent.breathOfSindragosa)) then
                        if br.player.castAntiMagicShell() then return end
                    end
            -- Anti-Magic Zone
                   -- antimagic_shell,damage=100000,if=((dot.breath_of_sindragosa.ticking&runic_power<25)|cooldown.breath_of_sindragosa.remains>40)|!talent.breath_of_sindragosa.enabled
                    if isChecked("Anti-Magic Zone") and php<=getOptionValue("Anti-Magic Zone") and (not isMoving("player")) and talent.antiMagicZone and ((buff.breathOfSindragosa and power<25) or cd.breathOfSindragosa>40 or (not talent.breathOfSindragosa)) then
                        if br.player.castAntiMagicZone() then return end
                    end
            -- Blood Presence
                    if isChecked("Blood Presence") and php<=getOptionValue("Blood Presence") and not buff.bloodPresence and getDistance(br.player.units.dyn5)<5 then --and (power<40 or (power<70 and glyph.shiftingPresences)) then
                        if br.player.castBloodPresence() then return end
                    end
            -- Conversion
                    if isChecked("Conversion") and php <= getOptionValue("Conversion") and power>=50 and inCombat then
                        if br.player.castConversion() then return end
                    end
            -- Death Pact
                    if isChecked("Death Pact") and php <= getOptionValue("Death Pact") and inCombat then
                        if br.player.castDeathPact() then return end
                    end
            -- Death Siphon
                    if isChecked("Death Siphon") and php <= getOptionValue("Death Siphon") and inCombat then
                        if br.player.castDeathSiphon() then return end
                    end
            -- Death Strike
                    if isChecked("Death Strike") and php<=getOptionValue("Death Strike") and not buff.killingMachine and inCombat then
                        if br.player.castDeathStrike() then return end
                    end
            -- Icebound Fortitude
                    if isChecked("Icebound Fortitude") and php <= getOptionValue("Icebound Fortitude") and inCombat then
                        if br.player.castIceboundFortitude() then return end
                    end
            -- Lichborne
                    if isChecked("Lichborne") and hasNoControl(br.player.spell.lichborne) then
                        if br.player.castLichborne() then return end
                    end
            -- Remorseless Winter
                    if isChecked("Remorseless Winter") and (useAoE() or php <= getOptionValue("Remorseless Winter")) and inCombat then
                        if br.player.castRemorselessWinter() then return end
                    end
            -- Desecrated Ground
                    if isChecked("Desecrated Ground") and hasNoControl(br.player.spell.desecratedGround) then
                        if br.player.castDesecratedGround() then return end
                    end
                    ...
                    if isChecked("Asphyxiate") then
                        for i=1, #dynTable30AoE do
                            ...
                            if canInterrupt(thisUnit,getOptionValue("InterruptAt")) then
                                if br.player.castAsphyxiate(thisUnit) then return end
                            end
                        end
                    end
            -- Dark Simulacrum
                    if isChecked("Dark Simulacrum") and (isInPvP() or castingSimSpell) then
                        for i=1, #dynTable30AoE do
                            thisUnit = dynTable30AoE[i].unit
                            if canInterrupt(thisUnit,getOptionValue("InterruptAt")) then
                                if br.player.castDarkSimulacrum() then return end
                            end
                        end
                    end
            -- Mind Freeze
                    if isChecked("Mind Freeze") then
                        for i=1, #dynTable5AoE do
                            ...
                            if canInterrupt(thisUnit,getOptionValue("InterruptAt")) then
                                if br.player.castMindFreeze() then return end
                            end
                        end
                    end
            -- Strangulate
                    if isChecked("Strangulate") then
                        for i=1, #dynTable30AoE do
                            thisUnit = dynTable30AoE[i].unit
                            if canInterrupt(thisUnit,getOptionValue("InterruptAt")) then
                                if br.player.castStrangulate(thisUnit) then return end
                            end
                        end
                    end   
        ...
        -- Action List - Cooldowns
            ...
            -- Empower Rune Weapon
                    ...
                    if getTimeToDie(br.player.units.dyn5)<=60 and buff.strengthPot then
                        if br.player.castEmpowerRuneWeapon() then return end
                    end
        ...
        -- Action List - Pre-Combat
            ...
            -- Horn of Winter
                if isChecked("Horn of Winter") and not (IsFlying() or IsMounted()) and not inCombat then
                    if br.player.castHornOfWinter() then return end
                end
            -- Frost Presence
                if not buff.frostPresence and php > getOptionValue("Blood Presence") and getDistance(br.player.units.dyn5)<=8 then
                    if br.player.castFrostPresence() then return end
                end
                if isChecked("Pre-Pull Timer") and pullTimer <= getOptionValue("Pre-Pull Timer") then
            -- Army of the Dead
                    -- army_of_the_dead
                    if br.player.castArmyOfTheDead() then return end
            -- Pre-Pot
            ...
            -- Pillar of Frost
                    -- pillar_of_frost
                    if br.player.castPillarOfFrost() then return end
                end -- Pre-Pull
            -- Start Attack
        ...
        -- Action List - Single Target: Boss
            ...
            -- Howling Blast
                ...
                if ((rune.death==1 and rune.frost==0 and rune.unholy==0) or (rune.death==0 and rune.frost==1 and rune.unholy==0)) and power<88 then
                    if br.player.castHowlingBlast() then return end
                end
        ...
        -- Action List - Multi-Target: Boss
            ...
            -- Death and Decay
                ...
                if rune.unholy==1 then
                    if br.player.castDeathAndDecay() then return end
                end
            -- Plague Strike
                -- plague_strike,if=unholy=2
                if rune.unholy==2 then
                    if br.player.castPlagueStrike() then return end
                end
            -- Blood Tap
                -- blood_tap
                if br.player.castBloodTap() then return end
        ...
        -- Action List - Single Target: 2 Hand Weapon
            ...
            -- Defile
                    -- defile
                    if br.player.castDefile() then return end
            -- Blood Tap
            ...
            -- Outbreak
                    ...
                    if disease.max.dyn30==0 then
                        if br.player.castOutbreak() then return end
                    end
            -- Unholy Blight
                    -- unholy_blight,if=!disease.min_ticking
                    if disease.min.dyn10AoE==99 then
                        if br.player.castUnholyBlight() then return end
                    end
            -- Breath of Sindragosa
                    -- breath_of_sindragosa,if=runic_power>75
                    if power>75 then
                        if br.player.castBreathOfSindragosa() then return end
                    end
            ...
            -- Frost Strike
                    ...
                    if power>76 or (level<63 and power>40) then
                        if br.player.castFrostStrike() then return end
                    end
            -- Howling Blast
        ...
        -- Action List - Single Target: 1 Hand Weapon
            ...
            -- Howling Blast
                    ...
                    if (not (thp-3*(thp/getTimeToDie(br.player.units.dyn5))<=35 and cd.soulReaper<3)) or rune.death+rune.frost>=2 then
                        if br.player.castHowlingBlast() then return end
                    end 
            -- Outbreak
        ...
        -- Action List - Multi-Target
            ...
            -- Blood Boil
                ...
                if debuff.bloodPlague and ((not talent.unholyBlight) or cd.unholyBlight<49) and (lineCD == nil or lineCD <= GetTime()-28) then
                    if br.player.castBloodBoil() then lineCD=GetTime(); return end
                end
            -- Defile
                -- defile
                if br.player.castDefile() then return end
    ...
    -----------------
                ...
                -- Soul Reaper
                    ...
                    if thp-3*(thp/getTimeToDie(br.player.units.dyn5))<=35 then
                        if br.player.castSoulReaper() then return end
                    end
                -- Blood Tap
                    -- blood_tap,if=(target.health.pct-3*(target.health.pct%target.time_to_die)<=35&cooldown.soul_reaper.remains=0)
                    if (thp-3*(thp/getTimeToDie(br.player.units.dyn5))<35 and cd.soulReaper==0) then
                        if br.player.castBloodTap() then return end
                    end
                -- Run Action List - Single Target: Two Hand
                    ...
                    if br.player.enemies.yards10<4 and twoHand and not useAoE() then
                ...
                -- Run Action List - Single Target: One Hand
                    ...
                    if br.player.enemies.yards10<3 and oneHand and not useAoE() then
                ...
                -- Run Action List - Mutli-Target
                    ...
                    if (br.player.enemies.yards10>=3 and oneHand) or (br.player.enemies.yards10>=4 and twoHand) or useAoE() then
