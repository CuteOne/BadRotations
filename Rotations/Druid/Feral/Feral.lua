function DruidFeral()
  if select(3, UnitClass("player")) == 11 and GetSpecialization() == 2 then
    ...
    if hastar == nil then hastar = GetObjectExists("target") end
    ...
    elseif not isChecked("Death Cat Mode") then
      ...
      then
        ...
        if useProwl() and not stealth
          and (GetObjectExists(dynamicUnit.dyn20) or isKnown(eprl)) and GetTime()-leftCombat > lootDelay
        then
      ...
      if hastar and attacktar and isInCombat("player") and cat and profileStop==false then
        ...
        --- In Combat - Cooldowns ---
        -----------------------------
        if not stealth and GetObjectExists(dynamicUnit.dyn5) then
          ...
          if ((not clearcast and powmax-power>=60) or powmax-power>=80) and GetObjectExists(dynamicUnit.dyn5) then
        ...
        if not stealth and isInCombat("player") and BadBoy_data['AoE'] ~= 4 then
          ...
          if srRemains==0 and combo>0 and power>25 and GetObjectExists(dynamicUnit.dyn5) then
            ...
            if useCleave() then
              for i=1, #enemiesTable do
                if enemiesTable[i].distance<5 then
                  ...
                  rkDmg = RKD(thisUnit)
                  if power>35 then
                    -- if=remains<3&((target.time_to_die-remains>3&active_enemies<3)|target.time_to_die-remains>6)
                    if rkRemain<3 and ((ttd-rkRemain>3 and enemies<3) or ttd-rkRemain>6) then
                      if castSpell(thisUnit,rk,false,false,false) then return end
                    end
                    -- if=remains<4.5&(persistent_multiplier>=dot.rake.pmultiplier|(talent.bloodtalons.enabled&(buff.bloodtalons.up|!buff.predatory_swiftness.up)))&((target.time_to_die-remains>3&active_enemies<3)|target.time_to_die-remains>6)
                    if rkRemain<4.5 and (rkCalc>=rkDmg or (bloodtalons and (btRemain>0 or psRemain==0))) and ((ttd-rkRemain>3 and enemies<3) or ttd-rkRemain>6) then
                      if castSpell(thisUnit,rk,false,false,false) then return end
                    end
                  end
                end
              ...
              if useCleave() then
                for i=1, #enemiesTable do
                  if enemiesTable[i].distance<40 then
                    ...
                    ttd = getTimeToDie(thisUnit)
                    --if=remains<4.2&active_enemies<=5&target.time_to_die-remains>tick_time*5
                    if mfRemain<4.2 and enemies<=5 and ttd-mfRemain>mfTick*5 then
          ...
          if combo<5 and (ttm<=1 or berserk or tfCooldown<3 or tfRemain>0 or (incBuff and incRemain<3) or rpRemain<8-combo or clearcast or (bloodtalons and psRemain>0 and psRemain<6.5-combo)) then
            ...
            -- Shred
            if not useAoE() and power>40 then
              if castSpell(dynamicUnit.dyn5,shr,false,false,false) then return end
            end
          end --End Generator
        end --End No Stealth
      ...
      -- Start Attack
      if GetObjectExists(dynamicUnit.dyn5) and not stealth and isInCombat("player") and cat and profileStop==false then
        StartAttack()
      end
    end
