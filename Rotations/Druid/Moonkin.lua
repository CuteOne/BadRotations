...
--[[
		...
		if (#myEnemiesTable == 1 and BadBoy_data["AoE"] == 3) or BadBoy_data["AoE"] == 1 then
			...
			if isStanding(0.3) and (eclipseDirection == 0 or (eclipseDirection == 1 and eclipseEnergy > 80)) and castSpell("target",_Wrath,false,true) then return; end


			-- starfire,if=(eclipse_energy>=0&eclipse_change>cast_time)|(eclipse_energy<0&cast_time>eclipse_change)
			if isStanding(0.3) and (eclipseDirection == 1 or (eclipseDirection == 0 and eclipseEnergy < -80)) and castSpell("target",_Starfire,false,true) then return; end
