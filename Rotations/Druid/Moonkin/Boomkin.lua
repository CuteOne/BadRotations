if select(3, UnitClass("player")) == 11 then
	...
	function DruidMoonkin()
		...
		buff = {
			...
			solarPeak = {
				up = UnitBuffID("player",),
				remains = getBuffRemain("player",),
				cd = getSpellCD(),
				},
			lunarEmpowerment = {
		...
		if currentConfig ~= "boomkind by ragnar" then
			MoonkinToggles()
			MoonkinConfig()
			currentConfig = "boomkin by ragnar"
		end
		...
		if isInCombat("player") then
			...
			------------------------------------------------------------------------------------------------------
				...
				if eclipse.energy>40 then
					if castSpell("target",spell.celestialAlignment) then return end
				end
				
				-- actions.single_target+=/incarnation,if=eclipse_energy>0
				if eclipse.energy>0 then
					if castSpell("target",spell.incarnation) then return end
				end
				
				-- actions.single_target+=/sunfire,if=remains<7|(buff.solar_peak.up&!talent.balance_of_power.enabled)
				if getDebuffRemain("target",spell.sunfireDebuff,"player")
				or (buff.solarpeak.up and not getTalent(7,3)) then
					if castSpell("target",spell.sunfire,false,false) then return end
				end
				...
				if not getTalent(7,3) then
					...
					if castSpell("target",moonfire,false,false) then return end
				end
				...
				if (eclipse.energy>=0 and eclipse.eclipse.changeTimer<spell.starfireCastTime) 
