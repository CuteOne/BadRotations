...
--DPS and NoT HEALING INCOMBAT
	...
	if bb.data["Healing"] == 2 then
		...
		-- Barkskin if < 30% or Badly Debuffed
		if isChecked("Barkskin") 
			and (getHP("player") <= getValue("Barkskin") or isBadlyDeBuffed("player")) then
		...
		if isChecked("Lifebloom") then
			...
			if 	(not UnitExists("focus") or UnitIsDeadOrGhost("focus"))
				and UnitExists("target") 
				and UnitIsFriend("targettarget", "player")
					then FocusUnit("targettarget")
			end
			
			if getBuffRemain("focus",33763,"player") < 3 
				and not UnitIsUnit("targettarget","focus") 
				and UnitExists("targettarget")
				and UnitIsFriend("player", "targettarget")
				and getDistance("target","targettarget") < 10
					then FocusUnit("targettarget")
			end			
		end
		
      		--[[ 18.5 - LifeBloom Fast Swich]]
		if 	isChecked("Lifebloom") 
			and canCast(33763,false,false)
			and getBuffRemain("focus",33763) == 0
			and UnitExists("focus") 
			and getDistance("player","focus") < 43 
				then
