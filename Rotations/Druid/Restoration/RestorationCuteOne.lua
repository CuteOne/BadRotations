...
--------------
        ...
        local power, powermax, powergen                     = br.player.power.amount.mana, br.player.power.mana.max, br.player.power.regen
        ...
        if not solo then
            for i = 1, #br.friend do
                ...
                if thisUnit ~= "player" then
                    if UnitBuffID(thisUnit,spell.rejuvenation,"player") ~= nil then
...
--- In Combat Rotation ---
--------------------------
            if inCombat and profileStop==false --[[and not (flight or travel or IsMounted() or IsFlying())]] then
                -- if not stealth then
    ------------------------------
    ...
    ------------------------
                    if getOptionValue("APL Mode") == 2 then
                        for i = 1, #br.friend do
                            ...
                            if (UnitGroupRolesAssigned(tankUnit) == "TANK" or npcID == "72218") and getHP(tankUnit) < 100 then
                                local lifebloomBuff = UnitBuffID(tankUnit,spell.buffs.lifebloom,"player") ~= nil
        -- Cenarion Ward
        ...
        -- Rejuvenation
                        ...
                        for i = 1, #br.friend do
                            ...
                            if UnitGroupRolesAssigned(br.friend[i].unit) == "TANK" or npcID == "72218" then
                                local tankUnit = br.friend[i].unit
                                local rejuvenationRefresh = getBuffRemain(tankUnit,spell.buffs.rejuvenation,"player") <= getBuffDuration(tankUnit,spell.buffs.rejuvenation,"player") * 0.3
                                local germinationRefresh = getBuffRemain(tankUnit,spell.buffs.germination,"player") <= getBuffDuration(tankUnit,spell.buffs.germination,"player") * 0.3
        ...
        -- Rejuvenation
                        ...
                        if getBuffRemain(lowestHP,spell.buffs.rejuvenation,"player") <= getBuffDuration(lowestHP,spell.buffs.rejuvenation,"player") * 0.3
