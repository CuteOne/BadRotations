--Version 1.0.0
local rotationName = "Kuu" -- Based on Panglo's CR
...
local function runRotation()
    ...
    local function List_Extras()
        ...
        if br.player.mode.taunt == 1 and inInstance and combatTime > 5 then
        ...
        if br.player.mode.taunt == 2 and combatTime > 5 then
            for i = 1, #enemies.yards30 do
                local thisUnit = enemies.yards30[i]
                if UnitThreatSituation("player", thisUnit) ~= nil and UnitThreatSituation("player", thisUnit) <= 2 and UnitAffectingCombat(thisUnit) then
                    if cast.growl(thisUnit) then
        ...
        if isChecked("Wild Charge") and br.player.mode.wildCharge == 1 then
            if getDistance("target") > 9 and cast.able.wildCharge() and bear then
    ...
    local function List_Interrupts()
        if useInterrupts() then
            ...
            if isChecked("Skull Bash") and not cd.skullBash.exists()  then
                for i = 1, #enemies.yards13 do
                    thisUnit = enemies.yards13[i]
                    if canInterrupt(thisUnit, getOptionValue("Interrupt At")) and br.timer:useTimer("Interrupt", 0.5) then
            ...
            if isChecked("Mighty Bash") and talent.mightyBash and (cd.skullBash.exists() or level < 70) then
            ...
            if isChecked("Incapacitating Roar") and (cd.skullBash.exists() or level < 70) then
                for i = 1, #enemies.yards10 do
