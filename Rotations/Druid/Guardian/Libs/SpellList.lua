if select(3, UnitClass("player")) == 11 then

    ------Abilities-------
    bar	=	22812	--Barkskin
    ber	=	106951	--Berserk
    cy	=	33786	--Cyclone
    dsh	=	1850	--Dash
    er	=	339		--Entangling Roots
    ff	=	770		--Faerie Fire
    fb	=	22568	--Ferocious Bite
    fr	=	22842	--Frenzied Regeneration
    grl	=	6795	--Growl
    ht	=	5185	--Healing Touch
    hib	=	2637	--Hibernate
    hur	=	16914	--Hurricane
    inn	=	29166	--Innervate
    lac =	33745	--Lacerate
    ma	=	22570	--Maim
    mgl	=	33917	--Mangle
    mow	=	1126	--Mark of the Wild
    ml	=	6807	--Maul
    mf	=	155625--8921	--Moonfire
    ng	=	16689	--Nature's Grasp
    nv	=	124974	--Nature's Vigil
    prl	=	5215	--Prowl
    rk	=	155722  --1822	--Rake
    rb	=	20484	--Rebirth
    rej	=	774		--Rejuvenation
    rc	=	2782	--Remove Corruption
    rv	=	50769	--Revive
    rp	=	1079	--Rip
    svr	=	174544 	--Savage Roar
    sd 	= 	62606 	--Savage Defense
    shg =	114236	--Shred Glyphed
    shr	=	5221	--Shred
    sb	=	106839	--Skull Bash
    sth	=	2908	--Soothe
    sro	=	77764	--Stampeding Roar
    si	=	61336	--Survival Instincts
    sw	=	106785	--Swipe
    swb	=	779		--Swipe: Bear Form
    thr	=	106830	--Thrash
    thb	=	77758	--Thrash: Bear Form
    tf	=	5217	--Tiger's Fury
    tq	=	740		--Tranquility
    wth =	5176	--Wrath

    ------Talents------
    inc	=	102543	--Incarnation: King of the Jungle
    incg=	102558 	--Incarnation: Son of Ursoc
    fon	=	102703	--Force of Nature
    how	=	108288 	--Heart of the Wild
    howg=	108293	--Heart of the Wild Guardian
    mb	=	5211	--Mighty Bash
    ty	=	132469	--Typhoon
    bt 	=	155672	--Bloodtalons
    uv  = 	102793  --Ursol's Vortex
    pulv= 	80313  	--Pulverize
    pulvb = 158792 	--Pulverize Buff

    ------Glyphs------
    gsh	=	114234	--Glyph of Shred
    gsr =	127540	--Glyph of Savagery
    svg	=	127538	--Savage Roar Glyphed

    ------Procs------
    ps	= 	69369	--Predatory Swiftness
    wa	= 	113746	--Weakened Armor - Faerie Fire Debuff
    cc 	= 	135700	--Clearcasting
    dcd	=	145152	--Dream of Cenarius Damage Buff
    dcg = 	145162 	--Dream of Cenarius Guardian Buff
    inb = 	102543	--Incarnation: King of the Jungle buff
    ro1	=	139121	--Rune of Re-Origination: Haste
    ro2	=	139117	--Rune of Re-Origination: Critical Strike
    ro3	=	139120	--Rune of Re-Origination: Mastery
    frf =	144865	--Feral Fury
    frr	=	146874 	--Feral Rage
    spd	=	81022	--Stampede
    obf =   33697   --Orc Racia: Blood Fury
    rber=	26297	--Troll Racial: Berserking
    beat=   25046   --Blood Elf Racial: Arcane Torrent
    sm 	=	58984	--Night Elf Racial: Shadowmeld
    tac = 	135286 	--Tooth and Claw
    rks	=	163505	--Rake - Pounce Effect

    ------Forms------
    af	=	1066	--Aquatic Form
    bf	=	5487	--Bear Form
    cf 	= 	768		--Cat Form
    cosf=	171745	--Claws of Shirvallah
    flf	=	165962 --33943	--Flight Form
    mkf = 	24858	--Moonkin Form
    sff	=	165962 --40120	--Swift Flight Form
    trf	=	783		--Travel Form

    ------Perks------
    erej=	157280	--Enhanced Rejuvination
    eprl=	157274	--Enhanced Prowl
    eber=	157269	--Enhanced Berserk
    irk =	157276	--Improved Rake

_AquaticForm 			= 1066;
_ArcaneStorm 			= 106996;
_AstralCommunion 		= 127663;
_Barkskin 				= 22812;
_BearForm 				= 5487;
_CatForm 				= 768;
_CelestialAlignment 	= 112071;
_Cyclone 				= 33786;
_Dash 					= 1850;
_DisorientingRoar 		= 99;
_EclipseLunar 			= 171743;
_EclipseSolar 			= 171744;
_EmpowermentLunar 		= 164547;
_EmpowermentSolar 		= 164545;
_EntanglingRoots 		= 339;
_FaerieSwarm 			= 102355;
_FerociousBite 			= 22568;
_ForceOfNature 			= 33831;
_FrenziedRegeneration 	= 22842;
_Growl 					= 6795;
_HealingTouch 			= 5185;
_Hibernate 				= 2637;
_Hurricane 				= 16914;
_Innervate 				= 29166;
_Lacerate 				= 33745;
_Main 					= 22570;
_Mangle 				= 33917;
_MarkOfTheWild 			= 1126;
_Maul 					= 6807;
_MightOfUrsoc 			= 106922;
_Moonfire 				= 8921;
_MoonkinForm 			= 24858;
_NaturesGrasp 			= 16689;
_NaturesSwiftness 		= 132158;
_NaturesVigil			= 124974;
_Pounce 				= 9005;
_Prowl 					= 5215;
_Rake 					= 1822;
_Rebirth 				= 20484;
_Rejuvenation 			= 774;
_RemoveCorruption 		= 2782;
_Renewal 				= 108238;
_Revive 				= 50769;
_SolarBeam 				= 78675;
_Soothe 				= 2908;
_StampedingRoar 		= 106898;
_Starfall 				= 48505;
_Starfire 				= 2912;
_Starsurge 				= 78674;
_Sunfire 				= 93402;
_SwiftFlightForm 		= 40120;
_Swipe 					= 106785;
_Symbiosis 				= 110309;
_Tranquility 			= 740;
_TravelForm 			= 783;
_WildMushroom 			= 88747;
_WildMushroomDetonate 	= 88751;
_Wrath 					= 5176;
_ShootingStars 			= 93400;


end