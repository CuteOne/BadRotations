...
local function createOptions()
    ...
    local function rotationOptions()
        ...
        -- Seal of Command
        br.ui:createCheckbox(section, "Seal of Command")
...
actionList.Extras = function()
    -- Seal of Command
    if ui.checked("Seal of Command") and cast.able.sealOfCommand("player") and not unit.formActive(spells.sealOfCommand) then
...
actionList.PreCombat = function()
    if not unit.inCombat() and not unit.mounted() then
        if unit.valid("target") then
            ...
            -- Crusader Strike
            if cast.able.crusaderStrike("target") and unit.exists("target") and unit.distance("target") < 5 then
