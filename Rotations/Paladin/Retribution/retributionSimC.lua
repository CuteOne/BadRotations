...
-- execution_sentence
	if self.castExecutionSentence(units.dyn40) then
		return
	end
...
--actions+=/call_action_list,name=single
	...
	-- actions.single+=/judgment,if=talent.empowered_seals.enabled
		if talent.empoweredSeals then
			...
			if (seal == true and (buff.maraadsTruth < recharge.judgment or buff.maraadsTruth == 0))
			-- & seal.righteousness&buff.liadrins_righteousness.remains<cooldown.judgment.duration
			or (seal == false and (buff.liadrinsRighteousness < recharge.judgment or buff.liadrinsRighteousness == 0))
			-- &seal.righteousness&cooldown.avenging_wrath.remains<cooldown.judgment.duration
			or (seal == false and cd.avengingWrath < recharge.judgment) then
	...
	-- actions.single+=/seal_of_truth,if=talent.empowered_seals.enabled
		if talent.empoweredSeals then
			...
			if (seal ~= true and buff.maraadsTruth == 0) 
				...
				or (seal ~= true and cd.avengingWrath < recharge.judgment and buff.liadrinsRighteousness > recharge.judgment) then
			...
			elseif (seal == true and buff.maraadsTruth > recharge.judgment and buff.liadrinsRighteousness == 0 and buff.avengingWrath == 0 and not hasBloodLust()) then
	...
	-- actions.single+=/judgment,if=holy_power<5&talent.seraphim.enabled
		if (holyPower < 5 and talent.seraphim) 
			...
			or (holyPower <= 3 or (holyPower == 4 and cd.crusaderStrike >= gcd*2 and buff.avengingWrath == 0 or getHP(units.dyn5) > 35)) then
				if self.castJudgment() then return end
		end
	...
	-- actions.single+=/seal_of_truth,if=talent.empowered_seals.enabled
		if talent.empoweredSeals then
			...
			if (seal ~= true and buff.maraadsTruth < recharge.judgment) then
				if self.castSeal(1) then return end
			...
			elseif (seal == true and buff.liadrinsRighteousness < recharge.judgment and not hasBloodLust()) then
				if self.castSeal(2) then return end
			end
		end
...
-- Cleave > 2 Targets 
	...
	-- actions.cleave+=/hammer_of_the_righteous,if=active_enemies>=4&(holy_power<=3|(holy_power=4&target.health.pct>=35&buff.avenging_wrath.down))
		if (enemies.yards8 >= 4 and holyPower < 5 and talent.seraphim) 
	...
	-- actions.cleave+=/judgment,if=holy_power<=3|(holy_power=4&cooldown.crusader_strike.remains>=gcd*2&target.health.pct>35&buff.avenging_wrath.down)
		if (holyPower < 5 and talent.seraphim)
			or (holyPower <= 3 or (holyPower == 4 and cd.crusaderStrike >= gcd*2 and getHP(units.dyn5) > 35 and buff.avengingWrath == 0)) then
				if self.castJudgment() then return end
		end
