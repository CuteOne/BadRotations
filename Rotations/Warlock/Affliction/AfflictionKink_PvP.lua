local rotationName = "Kink-PvP-v0.1"
----------------------------------------------------
...
local function createOptions ()
	...
	local function rotationOptions ()
        ...
        br.ui:checkSectionState(section)
        -- Interrupt Options
        section = br.ui:createSection(br.ui.window.profile, "Affliction .:|:. PvP")
            -- Soulstone
		    br.ui:createDropdown(section, "Fear Unit", {"|cffFFFFFFTarget","|cffFFFFFFMouseover",	"|cffFFFFFFTank", "|cffFFFFFFHealer", "|cffFFFFFFHealer/Tank", "|cffFFFFFFAny", "|cffFFFFFFPlayer"},
            1, "|cffFFFFFFTarget to cast on")

            -- Fear
            br.ui:createDropdown(section, "Fear Key", br.dropOptions.Toggle, 6)
            ...
            -- Spell lock
            br.ui:createDropdown(section, "Spell Lock Key", br.dropOptions.Toggle, 6)
...
local function runRotation()
    ...
    LoS                                           = getLineOfSight("player","target")
    ui                                            = br.player.ui
    pet                                           = br.player.pet
   ...
   --[[local priorityTarget
    ...
    function unstableAfflictionFUCK(unit)
        ...
        and getTTD(unit) >= gcdMax + cast.time.unstableAffliction() + 0.6
        then
            if not debuff.unstableAffliction.exists("target") 
            or debuff.unstableAffliction.remains(unit) < gcdMax + cast.time.unstableAffliction() then
               if cast.unstableAffliction() then br.addonDebug("Casting Unstable Affliction") return true end
            end
        end

        if debuff.unstableAffliction.remains(unit) < gcdMax + cast.time.unstableAffliction() + 0.6 then
           if cast.unstableAffliction() then br.addonDebug("Casting Unstable Affliction") return true end
        end
    end
    ...
    elseif haltProfile then
        ...
        if inCombat and not profileStop and (not cast.current.drainLife() or (cast.current.drainLife() and php > 80)) then
            ...
            -- Multi Target
            if mode.SingleModes ==  if #enemies.yards10t >= 3 and mode.single ~= 1 then if actionList.multi() then return true end end
            ...
            -- Malefic Rapture
            if not moving and isPvP() then
                -- Vile Taint ticking
                if shards > 3 or debuff.agony.exists("target") and debuff.corruption.exists("target") and debuff.unstableAffliction.exists("target") then
                    if cast.maleficRapture() then br.addonDebug("Casting Malefic Rupture (Full Shards or Full Dots)") return true end
                end
            end
