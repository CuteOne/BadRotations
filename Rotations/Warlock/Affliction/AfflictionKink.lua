...
local  castSummonId, opener, summonTime
    ...
    local function actionList_Rotation()
        if spellQueueReady() then
            ...
            if cast.able.maleficRapture() and not moving and shards > 0 and getDistance("target") <= 40 then
                -- Max Shards
               if shards > 4 then if cast.maleficRapture() then br.addonDebug("[Action:Rotation] Malefic Rapture, Max Shards") return true end end 
            end
