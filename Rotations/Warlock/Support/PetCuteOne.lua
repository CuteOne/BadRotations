...
br.loader.rotations.support["PetCuteOne"] = function()
    ...
    local haltProfile = (inCombat and profileStop) or (IsMounted() or IsFlying()) or br.functions.misc:pause(true) or mode.rotation == 4
    -- Units
    units.get(5)
    units.get(40)
    -- Enemies
    ...
    enemies.yards40r = br.engines.enemiesEngineFunctions:getEnemiesInRect(10, 40, false) or 0

    local petTarget
    if petTarget == nil or not UnitExists(petTarget) or not br.functions.misc:isValidUnit(petTarget) then
        if br.functions.misc:getOptionValue("Pet Target") == 1 and br.functions.misc:isValidUnit(units.dyn40) then
            petTarget = units.dyn40
        elseif br.functions.misc:getOptionValue("Pet Target") == 2 and br.functions.misc:isValidUnit("target") then
            petTarget = "target"
        elseif br.functions.misc:getOptionValue("Pet Target") == 3 then
            for i = 1, #enemies.yards40 do
                local thisUnit = enemies.yards40[i]
                if (br.functions.misc:isValidUnit(thisUnit) or br.functions.unit:isDummy()) then
                    petTarget = thisUnit
                    break
                end
            end
    ...
    local petDistance = br.functions.range:getDistance(petTarget, "pet") or 99
    local petExists = br._G.UnitExists("pet")
    local petHealth = br.functions.unit:getHP("pet")
    local petMode = getCurrentPetMode()
    local validTarget = br._G.UnitExists(petTarget) and
        (br.functions.misc:isValidUnit(petTarget) or br.functions.unit:isDummy()) --or (not UnitExists("pettarget") and br.functions.misc:isValidUnit("target")) or br.functions.unit:isDummy()
    ...
    --     end
    -- end
    if br.functions.misc:isChecked("Pet - Auto Attack/Passive") then
        ...
        elseif petMode ~= "Passive" and ((inCombat and #enemies.yards40 == 0) or haltProfile) and not br.functions.cast:isUnitCasting("player") then
            br._G.PetPassiveMode()
        end
        -- Pet Attack / retreat
        if (inCombat or petCombat) and not haltProfile then
            br._G.PetAttack(petTarget)
        elseif not inCombat or (inCombat and not br.functions.misc:isValidUnit(petTarget)) or haltProfile
            and IsPetAttackActive() and not br.functions.cast:isUnitCasting("player")
        then
            br._G.PetStopAttack()
            br._G.PetFollow()
        end
    ...
    -- if br.functions.misc:isChecked("Spirit Mend") and cast.able.spiritmend() and br.functions.unit:getHP(friendUnit) <= br.functions.misc:getOptionValue("Spirit Mend") then
    --     if cast.spiritmend(friendUnit) then return end
    -- end
    -- if br.functions.misc:isChecked("Cat-like Reflexes") and cast.able.catlikeReflexes() and petHealth <= br.functions.misc:getOptionValue("Cat-like Reflexes") then
    --     if cast.catlikeReflexes() then return end
    -- end
    -- if br.functions.misc:isChecked("Survival of the Fittest") and cast.able.survivalOfTheFittest()
    --     --[[and petCombat ]]and petHealth <= br.functions.misc:getOptionValue("Survival of the Fittest")
    -- then
    --     if cast.survivalOfTheFittest() then return end
    -- end
    -- -- Bite/Claw
    -- if br.functions.misc:isChecked("Bite / Claw") and petCombat and validTarget and petDistance < 5 and not haltProfile and not br.engines.enemiesEngineFunctions:isTotem(petTarget) then
    ...
    -- end
    -- -- Dash
    -- if br.functions.misc:isChecked("Dash") and cast.able.dash() and validTarget and petDistance > 10 and br.functions.range:getDistance(pertTarget) < 40 then
    --     if cast.dash("pet") then return end
    -- end
    -- -- Purge
    -- if br.functions.misc:isChecked("Purge") and inCombat then
    ...
    --             if br.functions.misc:getOptionValue("Purge") == 1 or (br.functions.misc:getOptionValue("Purge") == 2 and UnitIsUnit(thisUnit,"target")) then
    --                 if br.functions.aura:canDispel(thisUnit,spell.spiritShock) then
    ...
    -- end
    -- -- Growl
    -- if br.functions.misc:isChecked("Auto Growl") and inCombat then
    ...
    --         if br.functions.misc:getOptionValue("Misdirection") == 3 and cast.able.misdirection("pet") and #enemies.yards8p > 1 then
    ...
    -- end
    -- -- Play Dead / Wake Up
    -- if br.functions.misc:isChecked("Play Dead / Wake Up") and not deadPet then
    --     if cast.able.playDead() and petCombat and not buff.playDead.exists("pet")
    --         and petHealth < br.functions.misc:getOptionValue("Play Dead / Wake Up")
    --     then
    ...
    -- end
    -- -- Prowl
    -- if br.functions.misc:isChecked("Prowl / Spirit Walk") and not petCombat
    --     and (not IsResting() or br.functions.unit:isDummy()) and #enemies.yards40nc > 0
    -- then
    ...
    -- end
    -- -- Mend Pet
    -- if br.functions.misc:isChecked("Mend Pet") and cast.able.mendPet() and petExists and not deadPet
    --     and not buff.mendPet.exists("pet") and petHealth < br.functions.misc:getOptionValue("Mend Pet") and not haltProfile and not pause()
    -- then
