...
local function createOptions()
    ...
    local function rotationOptions()
        ...
        br.ui:createDropdownWithout(section, "Elysian Decree", alwaysCdAoENever, 1,
            "|cffFFFFFFWhen to use Elysian Decree")
        br.ui:checkSectionState(section)
        -- Defensive Options
        section = br.ui:createSection(br.ui.window.profile, "Defensive")
        -- Basic Healing Module
        br.player.module.BasicHealing(section)
        br.ui:createSpinnerWithout(section, "PoS removes Necrotic", 20, 0, 50, 1, "",
            "|cffFFFFFFNecrotic stacks Phial of Serenity to use at")
        -- Fiery Brand
        ...
        br.ui:createSpinnerWithout(section, "Hold Demon Spikes", 1, 0, 2, 1,
            "|cffFFBB00Number of Demon Spikes the bot will hold for manual use.");
        -- Metamorphosis
        ...
        br.ui:createSpinner(section, "Sigil of Chains - AoE", 3, 0, 10, 1,
...
actionList.Extras = function()
        ...
        for i = 1, #enemies.yards30 do
            ...
            if not unit.isTanking(thisUnit) and unit.threat(thisUnit) and not unit.isExplosive(thisUnit) then
                if cast.torment(thisUnit) then
                    ui.debug("Casting Torment [Not Tanking]")
                    return true
                end
            end
        end
    ...
    if ui.checked("Throw Glaive") and cast.able.throwGlaive() then
        for i = 1, #enemies.yards30 do
            local thisUnit = enemies.yards30[i]
            if not unit.isTanking(thisUnit) and unit.threat(thisUnit) and not unit.isExplosive(thisUnit) then
                if cast.throwGlaive(thisUnit) then
                    ui.debug("Casting Throw Glaive [Not Tanking]")
                    return true
                end
            end
        end
    ...
    if ui.useDefensive() then
        ...
        then
            if cast.metamorphosis() then
                ui.debug("Casting Metamorphosis")
                return true
            end
        end
        -- Fiery Brand
        ...
        then
            if cast.sigilOfChains("player", "ground") then
                ui.debug("Casting Sigil of Chains [HP]")
                return true
            end
        end
        ...
        then
            if cast.sigilOfChains("best", false, ui.value("Sigil of Chains - AoE"), 8) then
                ui.debug("Casting Sigil of Chains [AOE]")
                return true
            end
        end
            ...
            if unit.interruptable(thisUnit, ui.value("Interrupt At")) then
                ...
                if ui.checked("Sigil of Silence") and cast.able.sigilOfSilence(thisUnit) and cd.disrupt.remain() > 0 then
                    if cast.sigilOfSilence(thisUnit, "ground", 1, 8) then
                        ui.debug("Casting Sigil of Silence")
                        return true
                    end
                end
                -- Sigil of Misery
    ...
    if not unit.inCombat() then
        ...
        if unit.valid("target") then
            if unit.reaction("target", "player") < 4 then
                -- Sigil Of Flame
                -- sigil_of_flame
                if ui.checked("Sigil of Flame") and cast.able.sigilOfFlame("target", "ground", 1, 8) then
                ...
                if ui.checked("Throw Glaive") and cast.able.throwGlaive("target", "aoe") and #enemies.get(10, "target", true) == 1 and ui.checked("Auto Engage") then
                    if cast.throwGlaive("target", "aoe") then
...
actionList.Combat = function()
    ...
    if var.fdReady then
        var.canSpb = (var.SingleTarget and buff.soulFragments.stack() >= 5) or
        (var.SmallAoe and buff.soulFragments.stack() >= 4) or (var.BigAoe and buff.soulFragments.stack() >= 3)
    else
    ...
    if ui.mode.mover == 1 and cast.able.infernalStrike("player", "ground", 1, 6) then
        if cast.infernalStrike("player", "ground", 1, 6) then
            ui.debug("Casting Infernal Strike [Combat]")
            return true
        end
    end
    ...
    if cast.able.demonSpikes() and not buff.demonSpikes.exists() then
        if cast.demonSpikes() then
            ui.debug("Casting Demon Spikes [Combat]")
            return true
        end
    end
    ...
    if ui.checked("Metamorphosis") and unit.inCombat() and unit.hp() <= ui.value("Metamorphosis") and cast.able.metamorphosis() and not buff.metamorphosis.exists() then
        if cast.metamorphosis() then
            ui.debug("Casting Metamorphosis [Combat]")
            return true
        end
    end
...
actionList.BigAoe = function()
    ...
    if ui.alwaysCdNever("Fel Devastation") and cast.able.felDevastation("player", "cone", 1, 8) and ((talent.collectiveAnguish or talent.stokeTheFlames)) then
        if cast.felDevastation("player", "cone", 1, 8) then
            ui.debug("Casting Fel Devastation - Collective Anguish | Stoke the Flames [Big Aoe]")
            return true
        end
    end
    ...
    if ui.alwaysCdAoENever("The Hunt", ui.value("Units to AoE"), #enemies.yards50r) and cast.able.theHunt() then
        if cast.theHunt() then
            ui.debug("Casting The Hunt [Big Aoe]")
            return true
        end
    end
    ...
    if ui.alwaysCdAoENever("Elysian Decree", ui.value("Units to AoE"), #enemies.yards30) and cast.able.elysianDecree("target", "ground", 1, 8) and unit.standingTime() > 2
        and ((ui.delay("ElysianDecree0", 1) and (fury() >= 40 and (buff.soulFragments.stack() <= 1 or buff.soulFragments.stack() >= 4))))
    then
        if cast.elysianDecree("target", "ground", 1, 8) then
            ui.debug("Casting Elysian Decree [Big Aoe]")
            return true
        end
    end
    ...
    if ui.alwaysCdNever("Fel Devastation") and cast.able.felDevastation("player", "cone", 1, 8) then
        if cast.felDevastation("player", "cone", 1, 8) then
            ui.debug("Casting Fel Devastation [Big Aoe]")
            return true
        end
    end
    ...
    if ui.alwaysCdNever("Soul Carver") and cast.able.soulCarver() and buff.soulFragments.stack() < 3 then
        if cast.soulCarver() then
            ui.debug("Casting Soul Carver [Big Aoe]")
            return true
        end
    end
    ...
    if cast.able.spiritBomb() and buff.soulFragments.stack() >= 4 then
        if cast.spiritBomb() then
            ui.debug("Casting Spirit Bomb [Big Aoe]")
            return true
        end
    end
    ...
    if cast.able.fracture() then
        if cast.fracture() then
            ui.debug("Casting Fracture [Big Aoe]")
            return true
        end
    end
    ...
    if cast.able.shear() then
        if cast.shear() then
            ui.debug("Casting Shear [Big Aoe]")
            return true
        end
    end
    ...
    if cast.able.soulCleave() and not var.dontCleave then
        if cast.soulCleave() then
            ui.debug("Casting Soul Cleave [Big Aoe]")
            return true
        end
    end
...
actionList.FieryDemise = function()
    ...
    if ui.checked("Immolation Aura") and cast.able.immolationAura("player", "aoe", 1, 8) then
        if cast.immolationAura("player", "aoe", 1, 8) then
            ui.debug("Casting Immolation Aura [Fiery Demise]")
            return true
        end
    end
    ...
    if ui.checked("Sigil of Flame") and cast.able.sigilOfFlame("target", "ground", 1, 8) and ((talent.ascendingFlame or debuff.sigilOfFlame.count(units.dyn30AoE) == 0)) then
        if cast.sigilOfFlame("target", "ground", 1, 8) then
            ui.debug("Casting Sigil Of Flame [Fiery Demise]")
            return true
        end
    end
    ...
    if cast.able.felblade() and (cd.felDevastation.remains() <= (unit.ttd(units.dyn5) + unit.gcd())) and fury() < 50 then
        if cast.felblade() then
            ui.debug("Casting Felblade [Fiery Demise]")
            return true
        end
    end
    ...
    if ui.alwaysCdNever("Fel Devastation") and cast.able.felDevastation("player", "cone", 1, 8) then
        if cast.felDevastation("player", "cone", 1, 8) then
            ui.debug("Casting Fel Devastation [Fiery Demise]")
            return true
        end
    end
    ...
    if ui.alwaysCdNever("Soul Carver") and cast.able.soulCarver() and buff.soulFragments.stack() < 3 then
        if cast.soulCarver() then
            ui.debug("Casting Soul Carver [Fiery Demise]")
            return true
        end
    end
    ...
    if ui.alwaysCdAoENever("The Hunt", ui.value("Units to AoE"), #enemies.yards50r) and cast.able.theHunt() then
        if cast.theHunt() then
            ui.debug("Casting The Hunt [Fiery Demise]")
            return true
        end
    end
    ...
    then
        if cast.elysianDecree("target", "ground", 1, 8) then
            ui.debug("Casting Elysian Decree [Fiery Demise]")
            return true
        end
    end
    ...
    if cast.able.spiritBomb() and var.canSpb then
        if cast.spiritBomb() then
            ui.debug("Casting Spirit Bomb [Fiery Demise]")
            return true
        end
    end
...
actionList.Filler = function()
    ...
    if ui.checked("Sigil of Chains - Filler") and cast.able.sigilOfChains() and talent.cycleOfBinding and talent.sigilOfChains then
        if cast.sigilOfChains() then
            ui.debug("Casting Sigil Of Chains [Filler]")
            return true
        end
    end
    ...
    if ui.checked("Sigil of Misery - Filler") and cast.able.sigilOfMisery(units.dyn5, "ground", 1, 8) and talent.cycleOfBinding and talent.sigilOfMisery then
        if cast.sigilOfMisery(units.dyn5, "ground", 1, 8) then
            ui.debug("Casting Sigil Of Misery [Filler]")
            return true
        end
    end
    ...
    if ui.checked("Sigil of Silence - Filler") and cast.able.sigilOfSilence(units.dyn5, "ground", 1, 8) and talent.cycleOfBinding and talent.sigilOfSilence then
        if cast.sigilOfSilence(units.dyn5, "ground", 1, 8) then
            ui.debug("Casting Sigil Of Silence [Filler]")
            return true
        end
    end
    ...
    if cast.able.felblade() then
        if cast.felblade() then
            ui.debug("Casting Felblade [Filler]")
            return true
        end
    end
    ...
    if ui.checked("Throw Glaive") and cast.able.throwGlaive() then
        if cast.throwGlaive() then
            ui.debug("Casting Throw Glaive [Filler]")
            return true
        end
    end
...
actionList.Maintenance = function()
    ...
    then
        if cast.fieryBrand() then
            ui.debug("Casting Fiery Brand [Maintenance]")
            return true
        end
    end
    ...
    -- Sigil Of Flame
    -- sigil_of_flame,if=talent.ascending_flame|active_dot.sigil_of_flame=0
    if ui.checked("Sigil of Flame") and cast.able.sigilOfFlame("target", "ground", 1, 8) and ((talent.ascendingFlame or debuff.sigilOfFlame.count(units.dyn5) == 0)) then
        if cast.sigilOfFlame("target", "ground", 1, 8) then
            ui.debug("Casting Sigil Of Flame [Maintenance]")
            return true
        end
    end
    ...
    if ui.checked("Immolation Aura") and cast.able.immolationAura("player", "aoe", 1, 8) then
        if cast.immolationAura("player", "aoe", 1, 8) then
            ui.debug("Casting Immolation Aura [Maintenance]")
            return true
        end
    end
    ...
    if cast.able.bulkExtraction() and ((5 - buff.soulFragments.stack()) <= #enemies.yards8) and buff.soulFragments.stack() <= 2 then
        if cast.bulkExtraction() then
            ui.debug("Casting Bulk Extraction [Maintenance]")
            return true
        end
    end
    ...
    if cast.able.spiritBomb() and var.canSpb then
        if cast.spiritBomb() then
            ui.debug("Casting Spirit Bomb [Maintenance]")
            return true
        end
    end
    ...
    if cast.able.felblade() and (((fury.deficit() >= 40 and #enemies.yards8 == 1) or ((cd.felDevastation.remains() <= (unit.ttd(units.dyn5) + unit.gcd())) and fury() < 50))) then
        if cast.felblade() then
            ui.debug("Casting Felblade [Maintenance]")
            return true
        end
    end
    ...
    if cast.able.fracture() and (cd.felDevastation.remains() <= (unit.ttd(units.dyn5) + unit.gcd())) and fury() < 50 then
        if cast.fracture() then
            ui.debug("Casting Fracture [Maintenance]")
            return true
        end
    end
    ...
    if cast.able.shear() and (cd.felDevastation.remains() <= (unit.ttd(units.dyn5) + unit.gcd())) and fury() < 50 then
        if cast.shear() then
            ui.debug("Casting Shear [Maintenance]")
            return true
        end
    end
    ...
    if cast.able.spiritBomb() and fury.deficit() <= 30 and #enemies.yards8 > 1 and buff.soulFragments.stack() >= 4 then
        if cast.spiritBomb() then
            ui.debug("Casting Spirit Bomb - AOE [Maintenance]")
            return true
        end
    end
    ...
    if cast.able.soulCleave() and fury.deficit() <= 40 then
        if cast.soulCleave() then
            ui.debug("Casting Soul Cleave [Maintenance]")
            return true
        end
    end
...
actionList.SingleTarget = function()
    ...
    if ui.alwaysCdAoENever("The Hunt", ui.value("Units to AoE"), #enemies.yards50r) and cast.able.theHunt() then
        if cast.theHunt() then
            ui.debug("Casting The Hunt [Single Target]")
            return true
        end
    end
    ...
    if ui.alwaysCdNever("Soul Carver") and cast.able.soulCarver() then
        if cast.soulCarver() then
            ui.debug("Casting Soul Carver [Single Target]")
            return true
        end
    end
    ...
    then
        if cast.felDevastation("player", "cone", 1, 8) then
            ui.debug("Casting Fel Devastation - CA or (StF and BB) [Single Target]")
            return true
        end
    end
    ...
    -- Elysian Decree
    -- elysian_decree
    if ui.alwaysCdAoENever("Elysian Decree", ui.value("Units to AoE"), #enemies.yards30) and cast.able.elysianDecree("target", "ground", 1, 8) and unit.standingTime() > 2 then
        if cast.elysianDecree("target", "ground", 1, 8) then
            ui.debug("Casting Elysian Decree [Single Target]")
            return true
        end
    end
    ...
    if ui.alwaysCdNever("Fel Devastation") and cast.able.felDevastation("player", "cone", 1, 8) then
        if cast.felDevastation("player", "cone", 1, 8) then
            ui.debug("Casting Fel Devastation [Single Target]")
            return true
        end
    end
    ...
    if cast.able.soulCleave() and not var.dontCleave then
        if cast.soulCleave() then
            ui.debug("Casting Soul Cleave [Single Target]")
            return true
        end
    end
    ...
    if cast.able.fracture() then
        if cast.fracture() then
            ui.debug("Casting Fracture [Single Target]")
            return true
        end
    end
    ...
    if cast.able.shear() then
        if cast.shear() then
            ui.debug("Casting Shear [Single Target]")
            return true
        end
    end
...
actionList.SmallAoe = function()
    ...
    if ui.alwaysCdAoENever("The Hunt", ui.value("Units to AoE"), #enemies.yards50r) and cast.able.theHunt() then
        if cast.theHunt() then
            ui.debug("Casting The Hunt [Small Aoe]")
            return true
        end
    end
    ...
    then
        if cast.felDevastation("player", "cone", 1, 8) then
            ui.debug("Casting Fel Devastation - CA or (StF and BB) [Small Aoe]")
            return true
        end
    end
    ...
    then
        if cast.elysianDecree("target", "ground", 1, 8) then
            ui.debug("Casting Elysian Decree [Small Aoe]")
            return true
        end
    end
    ...
    if ui.alwaysCdNever("Fel Devastation") and cast.able.felDevastation("player", "cone", 1, 8) then
        if cast.felDevastation("player", "cone", 1, 8) then
            ui.debug("Casting Fel Devastation [Small Aoe]")
            return true
        end
    end
    ...
    if ui.alwaysCdNever("Soul Carver") and cast.able.soulCarver() and buff.soulFragments.stack() < 3 then
        if cast.soulCarver() then
            ui.debug("Casting Soul Carver [Small Aoe]")
            return true
        end
    end
    ...
    if cast.able.soulCleave() and buff.soulFragments.stack() <= 1 and not var.dontCleave then
        if cast.soulCleave() then
            ui.debug("Casting Soul Cleave [Small Aoe]")
            return true
        end
    end
    ...
    if cast.able.fracture() then
        if cast.fracture() then
            ui.debug("Casting Fracture [Small Aoe]")
            return true
        end
    end
    ...
    if cast.able.shear() then
        if cast.shear() then
            ui.debug("Casting Shear [Small Aoe]")
            return true
        end
    end
