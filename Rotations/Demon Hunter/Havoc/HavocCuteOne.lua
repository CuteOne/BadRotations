...
local function createOptions()
    ...
    local function rotationOptions()
        ...
        br.ui:createSpinnerWithout(section, "Units To AoE", 3, 1, 10, 1,
            "|cffFFBB00Number of Targets to use AoE spells on.")
        -- Fel Rush Charge Hold
        br.ui:createSpinnerWithout(section, "Hold Fel Rush Charge", 1, 0, 2, 1,
            "|cffFFBB00Number of Fel Rush charges the bot will hold for manual use.");
        ...
        -- Elysian Decree
        br.ui:createDropdownWithout(section, "Elysian Decree", alwaysCdAoENever, 1,
            "|cffFFBB00When to use Elysian Decree.")
        br.ui:checkSectionState(section)
        -- Defensive Options
        ...
        br.ui:createSpinner(section, "Sigil of Misery - AoE", 3, 0, 10, 1,
            "|cffFFFFFFNumber of Units in 8 Yards to Cast At")
        br.ui:checkSectionState(section)
        -- Interrupt Options
...
actionList.Extras = function()
    ...
    if ui.checked("Glide") and cast.able.glide() and not buff.glide.exists() then
        if var.falling >= ui.value("Glide") then
            if cast.glide("player") then
                ui.debug("Casting Glide")
                return true
            end
        end
    end
    ...
    if ui.useDefensive() then
        ...
        then
            if cast.chaosNova() then
                ui.debug("Casting Chaos Nova [HP]")
                return true
            end
        end
        ...
        then
            if cast.chaosNova() then
                ui.debug("Casting Chaos Nova [AoE]")
                return true
            end
        end
        -- Consume Magic
            ...
            for i = 1, #enemies.yards30 do
                ...
                then
                    if cast.consumeMagic(thisUnit) then
                        ui.debug("Casting Consume Magic")
                        return true
                    end
                end
            end
        ...
        then
            if cast.sigilOfMisery("player", "ground") then
                ui.debug("Casting Sigil of Misery [HP]")
                return true
            end
        end
        ...
        then
            if cast.sigilOfMisery("best", false, ui.value("Sigil of Misery - AoE"), 8) then
                ui.debug("Casting Sigil of Misery [AOE]")
                return true
            end
        end
        ...
        for i = 1, #enemies.yards30 do
            ...
            if ui.checked("Sigil of Misery") and cast.able.sigilOfMisery(thisUnit) and cd.disrupt.remain() > 0 then
                if cast.sigilOfMisery(thisUnit, "ground", 1, 8) then
                    ui.debug("Casting Sigil of Misery [Interrupt]")
                    return true
                end
            end
        end
...
actionList.Cooldown = function()
    if unit.exists(units.dyn5) and unit.distance(units.dyn5) < 5 then
        ...
        if ui.alwaysCdAoENever("Metamorphosis", 3, #enemies.yards8) and cast.able.metamorphosis("player") and #enemies.yards8 > 0 then
            ...
            then
                if cast.metamorphosis("player") then
                    ui.debug("Casting Metamorphosis - Not Demonic [Cooldown]")
                    return true
                end
            end
        end
        ...
        then
            if cast.elysianDecree("best", nil, 1, 8) then
                ui.debug("Casting Elysian Decree [Cooldown]")
                return true
            end
        end
        ...
        then
            if use.beaconToTheBeyond() then
                ui.debug("Using Beacon To The Beyond [Cooldown]")
                return true
            end
        end
        ...
        then
            if use.dragonfireBombDispenser() then
                ui.debug("Using Dragonfire Bomb Dispenser [Cooldown]")
                return true
            end
        end
        ...
        if use.able.elementiumPocketAnvil() and not cast.last.felRush(1) and cd.elementiumPocketAnvil.remains() then
            if use.elementiumPocketAnvil() then
                ui.debug("Using Elementium Pocket Anvil [Cooldown]")
                return true
            end
        end
        ...
        if use.able.slot(13) and (((var.trinketSyncSlot == 1 and (buff.metamorphosis.exists() or (not talent.demonic
                        and cd.metamorphosis.remains() > (math.max(unit.ttdGroup(40), cd.slot.duration(13) / 2))) or unit.ttdGroup(40) <= 20)
                    or (var.trinketSyncSlot == 2 and not cd.slot.exists(14)) or not var.trinketSyncSlot)
                and (not talent.initiative or buff.initiative.exists())))
        then
            if use.slot(13) then
                ui.debug("Using Trinket 1 [Cooldown]")
                return true
            end
        end
        ...
        if use.able.slot(14) and (((var.trinketSyncSlot == 2 and (buff.metamorphosis.exists() or (not talent.demonic
                        and cd.metamorphosis.remains() > (math.max(unit.ttdGroup(40), cd.slot.duration(14) / 2))) or unit.ttdGroup(40) <= 20)
                    or (var.trinketSyncSlot == 1 and not cd.slot.exists(13)) or not var.trinketSyncSlot)
                and (not talent.initiative or buff.initiative.exists())))
        then
            if use.slot(14) then
                ui.debug("Using Trinket 2 [Cooldown]")
                return true
            end
        end
    end
...
actionList.Precombat = function()
    if not unit.inCombat() and not (unit.flying() or unit.mounted()) then
        ...
        then
            if use.augmentation() then
                ui.debug("Using Augmentation [Precombat]")
                return true
            end
        end
        ...
        if unit.exists("target") and unit.valid("target") and unit.facing("target") and unit.distance("target") < 30 then
            ...
            if ui.alwaysCdNever("Racial") and cast.able.racial() and unit.race() == "BloodElf" then
                if cast.racial() then
                    ui.debug("Casting Arcane Torrent [Precombat]")
                    return true
                end
            end
            ...
            if cast.able.immolationAura("player", "aoe", 1, 8) then
                if cast.immolationAura("player", "aoe", 1, 8) then
                    ui.debug("Casting Immolation Aura [Precombat]")
                    return true
                end
            end
            ...
            if ui.checked("Auto Engage") and var.solo then
                ...
                if ui.checked("Throw Glaive") and cast.able.throwGlaive("target", "aoe", 1, 8) and #enemies.yards10tnc == 1 then
                    if cast.throwGlaive("target", "aoe", 1, 8) then
                        ui.debug("Casting Throw Glaive [Pre-Pull]")
                        return true
                    end
                end
                -- Torment
            ...
            if unit.distance("target") < 5 then
                if cast.able.autoAttack("target") then
                    if cast.autoAttack("target") then
                        ui.debug("Casting Auto Attack [Pre-Pull]")
                        return true
                    end
                end
            end
...
actionList.Combat = function()
    ...
    then
        ...
        if cast.autoAttack(var.lowestBurningWound) then
            ui.debug("Casting Auto Attack with retarget - Burning Wound [Combat]")
            return true
        end
    end
    ...
    then
        ...
        if cast.autoAttack(var.minTTDUnit) then
            ui.debug("Casting Auto Attack with retarget - Lowest TTD Non-Boss [Combat]")
            return true
        end
    end
    ...
    var.poolingForBladeDance = var.bladeDance and fury() < (75 - var.demonBlades * 20) and
        cd.bladeDance.remains() < unit.gcd(true)
    ...
    var.poolingForEyeBeam = talent.demonic and not talent.blindFury and cd.eyeBeam.remains() < (unit.gcd(true) * 3) and
        fury.deficit() > 30
    ...
    var.holdingMeta = (talent.demonic and talent.essenceBreak) and var.value3minTrinket
        and unit.ttdGroup(40) > cd.metamorphosis.remains() + 30 + var.shatteredDestiny * 60 and
        cd.metamorphosis.remains() < 20
    ...
    then
        if cast.immolationAura("player", "aoe", 1, 8) then
            ui.debug("Casting Immolation Aura - Ragefire +3 | No Essence Break [Combat]")
            return true
        end
    end
    ...
    then
        if cast.immolationAura("player", "aoe", 1, 8) then
            ui.debug("Casting Immolation Aura - A Fire Inside Inertia [Combat]")
            return true
        end
    end
    ...
    then
        ...
        if ui.mode.mover ~= 3 and (unit.distance("target") < 8 or (not ui.checked("Fel Rush Only In Melee") and unit.distance("target") >= 8)) then
            if cast.felRush() then
                ui.debug("Casting Fel Rush - Unbound Chaos [Combat]")
                return true
            end
        end
    end
    ...
    then
        if cast.theHunt() then
            ui.debug("Casting The Hunt [Combat]")
            return true
        end
    end
    ...
    then
        if cast.immolationAura("player", "aoe", 1, 8) then
            ui.debug("Casting Immolation Aura - Inertia [Combat]")
            return true
        end
    end
    ...
    then
        if cast.immolationAura("player", "aoe", 1, 8) then
            ui.debug("Casting Immolation Aura - Inertia No Unbound Chaos [Combat]")
            return true
        end
    end
    ...
    then
        ...
        if ui.mode.mover ~= 3 and (unit.distance("target") < 8 or (not ui.checked("Fel Rush Only In Melee") and unit.distance("target") >= 8)) then
            if cast.felRush() then
                ui.debug("Casting Fel Rush - Unbound Chaos Low Buff|TTD [Combat]")
                return true
            end
        end
    end
    ...
    then
        ...
        if ui.mode.mover ~= 3 and (unit.distance("target") < 8 or (not ui.checked("Fel Rush Only In Melee") and unit.distance("target") >= 8)) then
            if cast.felRush() then
                ui.debug("Casting Fel Rush - Intertia No Buff [Combat]")
                return true
            end
        end
    end
    ...
    then
        ...
        if ui.mode.mover ~= 3 and (unit.distance("target") < 8 or (not ui.checked("Fel Rush Only In Melee") and unit.distance("target") >= 8)) then
            if cast.felRush() then
                ui.debug("Casting Fel Rush - Intertia Meta|Recent Essence Break [Combat]")
                return true
            end
        end
    end
    ...
    if cast.able.annihilation() and buff.innerDemon.exists() and cd.metamorphosis.remains() <= unit.gcd() * 3 then
        if cast.annihilation() then
            ui.debug("Casting Annihilation [Combat]")
            return true
        end
    end
    ...
    then
        ...
        if cast.vengefulRetreat() then
            ui.debug("Casting Vengeful Retreat - Eye Beam Soon [Combat]")
            return true
        end
        -- end
    end
    ...
    then
        ...
        -- elseif ui.mode.mover == 2 then
        if cast.vengefulRetreat() then
            ui.debug("Casting Vengeful Retreat - No Initiative Essence Break [Combat]")
            return true
        end
        -- end
    end
    ...
    then
        ...
        -- elseif ui.mode.mover == 2 then
        if cast.vengefulRetreat() then
            ui.debug("Casting Vengeful Retreat - Initiative Essence Break [Combat]")
            return true
        end
        -- end
    end
    ...
    then
        ...
        -- elseif ui.mode.mover == 2 then
        if cast.vengefulRetreat() then
            ui.debug("Casting Vengeful Retreat - Initiative No Essence Break [Combat]")
            return true
        end
        -- end
    end
    ...
    then
        ...
        if ui.mode.mover ~= 3 and (unit.distance("target") < 8 or (not ui.checked("Fel Rush Only In Melee") and unit.distance("target") >= 8)) then
            if cast.felRush() then
                ui.debug("Casting Fel Rush - Momentum [Combat]")
                return true
            end
        end
    end
    ...
    then
        ...
        if ui.mode.mover ~= 3 and (unit.distance("target") < 8 or (not ui.checked("Fel Rush Only In Melee") and unit.distance("target") >= 8)) then
            if cast.felRush() then
                ui.debug("Casting Fel Rush - Intertia [Combat]")
                return true
            end
        end
    end
    ...
    then
        if cast.essenceBreak() then
            ui.debug("Casting Essence Break [Combat]")
            return true
        end
    end
    ...
    then
        if cast.deathSweep("player", "aoe", 1, 8) then
            ui.debug("Casting Death Sweep [Combat]")
            return true
        end
    end
    ...
    then
        if cast.theHunt() then
            ui.debug("Casting The Hunt - No Essence Break Debuff [Combat]")
            return true
        end
    end
    ...
    then
        if cast.felBarrage("player", "aoe", 1, 8) then
            ui.debug("Casting Fel Barrage [Combat]")
            return true
        end
    end
    ...
    if cast.able.glaiveTempest("player", "aoe", 1, 8) and unit.standingTime() > 2
        ...
        if cast.glaiveTempest("player", "aoe", 1, 8) then
            ui.debug("Casting Glaive Tempest [Combat]")
            return true
        end
    end
    ...
    if cast.able.annihilation() and buff.innerDemon.exists() and cd.eyeBeam.remains() <= unit.gcd() and not buff.felBarrage.exists() then
        if cast.annihilation() then
            ui.debug("Casting Annihilation - Inner Demon [Combat]")
            return true
        end
    end
    ...
    then
        ...
        if ui.mode.mover ~= 3 and (unit.distance("target") < 8 or (not ui.checked("Fel Rush Only In Melee") and unit.distance("target") >= 8)) then
            if cast.felRush() then
                ui.debug("Casting Fel Rush - Momentum Eye Beam Soon [Combat]")
                return true
            end
        end
    end
    ...
    then
        if cast.eyeBeam() then
            ui.debug("Casting Eye Beam [Combat]")
            return true
        end
    end
    ...
    then
        if cast.bladeDance("player", "aoe", 1, 8) then
            ui.debug("Casting Blade Dance [Combat]")
            return true
        end
    end
    ...
    then
        if cast.throwGlaive() then
            ui.debug("Casting Throw Glaive - Max Charges [Combat]")
            return true
        end
    end
    ...
    if cast.able.immolationAura("player", "aoe", 1, 8) and #enemies.yards8 >= 2 and fury() < 70 and not debuff.essenceBreak.exists(units.dyn5) then
        if cast.immolationAura("player", "aoe", 1, 8) then
            ui.debug("Casting Immolation Aura - No Essence Break [Combat]")
            return true
        end
    end
    ...
    then
        if cast.annihilation() then
            ui.debug("Casting Annihilation  - No Blade Dance Pooling [Combat]")
            return true
        end
    end
    ...
    then
        if cast.felblade() then
            ui.debug("Casting Felblade [Combat]")
            return true
        end
    end
    ...
    then
        if cast.throwGlaive() then
            ui.debug("Casting Throw Glaive [Combat]")
            return true
        end
    end
    ...
    then
        if cast.immolationAura("player", "aoe", 1, 8) then
            ui.debug("Casting Immolation Aura - No Eye Beam Before Recharge [Combat]")
            return true
        end
    end
    ...
    then
        if cast.throwGlaive() then
            ui.debug("Casting Throw Glaive - No Eye Beam Pooling [Combat]")
            return true
        end
    end
    ...
    if cast.able.chaosStrike() and not var.poolingForBladeDance and not var.poolingForEyeBeam and not buff.felBarrage.exists() then
        if cast.chaosStrike() then
            ui.debug("Casting Chaos Strike [Combat]")
            return true
        end
    end
    ...
    if cast.able.felblade() and fury.deficit() >= 40 then
        if cast.felblade() then
            ui.debug("Casting Felblade - Low Fury [Combat]")
            return true
        end
    end
    ...
    then
        ...
        if ui.mode.mover ~= 3 and (unit.distance("target") < 8 or (not ui.checked("Fel Rush Only In Melee") and unit.distance("target") >= 8)) then
            if cast.felRush() then
                ui.debug("Casting Fel Rush - 2 Charges [Combat]")
                return true
            end
        end
    end
    ...
    if cast.able.demonsBite(var.lowestBurningWound) and talent.burningWound and debuff.burningWound.remains(var.lowestBurningWound) < 4
        and debuff.burningWound.count(var.lowestBurningWound) < (math.max(#enemies.yards5, 3))
    then
        if cast.demonsBite(var.lowestBurningWound) then
            ui.debug("Casting Demons Bite - Low Burning Wounds [Combat]")
            return true
        end
    end
    ...
    then
        ...
        if ui.mode.mover ~= 3 and (unit.distance("target") < 8 or (not ui.checked("Fel Rush Only In Melee") and unit.distance("target") >= 8)) then
            if cast.felRush() then
                ui.debug("Casting Fel Rush - 2 Charges AoE [Combat]")
                return true
            end
        end
    end
    ...
    if cast.able.demonsBite() then
        if cast.demonsBite() then
            ui.debug("Casting Demons Bite [Combat]")
            return true
        end
    end
    ...
    then
        ...
        if ui.mode.mover ~= 3 and (unit.distance("target") < 8 or (not ui.checked("Fel Rush Only In Melee") and unit.distance("target") >= 8)) then
            if cast.felRush() then
                ui.debug("Casting Fel Rush - Momentum Expire Soon [Combat]")
                return true
            end
        end
    end
    ...
    then
        if ui.mode.mover ~= 3 then
            if cast.felRush() then
                ui.debug("Casting Fel Rush - Close Distance [Combat]")
                return true
            end
        end
    end
    ...
    then
        ...
        -- elseif ui.mode.mover == 2 then
        if cast.vengefulRetreat() then
            ui.debug("Casting Vengeful Retreat - No Initiative Close Distance [Combat]")
            return true
        end
        -- end
    end
    ...
    then
        if cast.throwGlaive() then
            ui.debug("Casting Throw Glaive - Melee [Combat]")
            return true
        end
    end
...
actionList.MetaEnd = function()
    ...
    if cast.able.deathSweep() and not buff.felBarrage.exists() then
        if cast.deathSweep() then
            ui.debug("Casting Death Sweep [Meta End]")
            return true
        end
    end
    ...
    if cast.able.annihilation() and not buff.felBarrage.exists() then
        if cast.annihilation() then
            ui.debug("Casting Annihilation [Meta End]")
            return true
        end
    end
