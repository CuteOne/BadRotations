...
---------------
    ...
    -- Mover
        MoverModes = {
            [1] = { mode = "On", value = 2 , overlay = "Auto Movement Enabled", tip = "Will Cast Movement Abilities.", highlight = 1, icon = bb.player.spell.felRush},
            [2] = { mode = "Off", value = 1 , overlay = "Auto Movement Disabled", tip = "Will NOT Cast Movement Abilities", highlight = 0, icon = bb.player.spell.felRush}
        };
	...
	--------------------
        ...
        -- Action List - PreCombat
            local function actionList_PreCombat()
                ...
                -- Throw Glaive
                    if ObjectExists("target") and not UnitIsDeadOrGhost("target") and UnitCanAttack("target", "player") and getDistance("target") < 30 then
    ...
    --------------------------
        ...
        ---------------------------
                ...
                -- Vengeful Retreat
                        ...
                        if useMover() and (talent.prepared or talent.momentum) and not buff.prepared and not buff.momentum and charges.felRush > 0 and getDistance("target") < 5 then
                ...
                -- Demon's Bite
                        ...
                        if buff.remain.metamorphosis > gcd and cd.bladeDance < gcd and power < 70 and worthUsing() then
                            if cast.demonsBite() then return end
                        end
                -- Blade Dance
                        if worthUsing() then
                ...
                -- Demon's Bite
                        ...
                        if not buff.metamorphosis and cd.bladeDance < gcd and power < 55 and worthUsing() then
                            if cast.demonsBite() then return end
                        end
