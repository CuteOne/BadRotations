...
--------------------
	...
	-- Action List - Patient Sniper
		...
		-- Arcane Shot
			...
			if #enemies.yards8t == 1 and (not t20_2pc or buff.t20_2pc_critical_aimed.remain() > getCastTime(spell.aimedShot) + gcd) and vulnAimCast > 0 and canGCD and power + (powerRegen*getCastTime(spell.arcaneShot)) + (powerRegen*getCastTime(spell.aimedShot)) + 20 < powerMax and (not poolForPiercing or lowestVuln > gcd) then
				if cast.arcaneShot() then return end
			end
		-- Aimed Shot
