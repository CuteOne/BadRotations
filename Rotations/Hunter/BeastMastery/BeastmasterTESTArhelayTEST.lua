...
local rotationName = "TESTArhelayTEST"
...
local function createOptions()
    ...
    local function rotationOptions()
        ...
        -- General Options
        section = br.ui:createSection(br.ui.window.profile, "General - \124cffffff00Test version\124r")
            -- Target Lock
        ...
        section = br.ui:createSection(br.ui.window.profile, "Cooldowns")
            ...
            -- Bloodshed
            br.ui:createCheckbox(section,"Bloodshed")			
            -- Death Chakram
            br.ui:createCheckbox(section,"Death Chakram")
            -- Dire Beast
            br.ui:createCheckbox(section,"Dire Beast")
            -- Wailing Arrow
            br.ui:createCheckbox(section,"Wailing Arrow")			
            -- Stampede
...
actionList.St = function()
    ...
    -- Death Chakram 
    if ui.checked("Death Chakram") and cast.able.deathChakram()
    then
        if cast.deathChakram() then ui.debug("[ST] Death Chakram") return true end
    end
	
	-- Bloodshed 
    if ui.checked("Bloodshed") and cast.able.bloodshed()
    then
        if cast.bloodshed() then ui.debug("[ST] Bloodshed") return true end
    end
	...
	-- Dire Beast 
    if ui.checked("Dire Beast") and cast.able.direBeast()
    then
        if cast.direBeast() then ui.debug("[ST] Dire beast") return true end
    end	

	-- Wailing Arrow 
    if ui.checked("Wailing Arrow") and cast.able.wailingArrow()
    then
        if cast.wailingArrow() then ui.debug("[ST] Wailing arrow") return true end
    end	
...
actionList.Cleave = function()
    ...
    if ui.checked("Death Chakram") and cast.able.deathChakram()
    then
        if cast.deathChakram() then ui.debug("[AOE] Death Chakram") return true end
    end
	
	-- Bloodshed 
    if ui.checked("Bloodshed") and cast.able.bloodshed()
    then
        if cast.bloodshed() then ui.debug("[AOE] Bloodshed") return true end
    end	
    ...
    if ui.checked("Dire Beast") and cast.able.direBeast()
    then
        if cast.direBeast() then ui.debug("[AOE] Dire beast") return true end
    end	

	-- Wailing Arrow 
    if ui.checked("Wailing Arrow") and cast.able.wailingArrow()
    then
        if cast.wailingArrow() then ui.debug("[AOE] Wailing arrow") return true end
    end		
