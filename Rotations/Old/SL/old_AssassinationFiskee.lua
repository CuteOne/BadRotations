...
--------------
    ...
    if br.isChecked("Auto Target Burn Units") then
    ...
    local tricksUnit
    if br.isChecked("Auto Tricks") and GetSpellCooldown(spell.tricksOfTheTrade) == 0 and inCombat and not buff.subterfuge.exists() and not buff.masterAssassin.exists() then
...
--------------------
    local function actionList_Extra()
        if not inCombat then
            ...
            if br.isChecked("Auto Stealth") and IsUsableSpell(GetSpellInfo(spell.stealth)) and not cast.last.vanish() and not IsResting() and
            (botSpell ~= spell.stealth or (botSpellTime == nil or GetTime() - botSpellTime > 0.1)) then
                ...
                if #enemies.yards25nc > 0 and br.getOptionValue("Auto Stealth") == 2 then
                    if cast.stealth() then return end
                end
            end
        end
        ...
        if UnitIsVisible("target") and inCombat and (burnUnits[br.GetObjectID("target")] ~= nil or (not br.isChecked("Dot Players") and UnitIsFriend("target", "player") and validTarget)) and targetDistance < 5 then
            ...
            if getenvdamage() >= br._G.UnitHealth("target") then
    ...
    local function actionList_Interrupts()
        ...
        if not stealthedRogue then
            for i=1, #enemies.yards15 do
                ...
                if useInterrupts() and br.canInterrupt(thisUnit,br.getOptionValue("Interrupt %")) then
                    if br.isChecked("Kick") and distance < 5 then
                        if cast.kick(thisUnit) then return end
                    end
                    if cd.kick.remain() ~= 0 then
                        if br.isChecked("Kidney Shot") then
    ...
    local function actionList_Cooldowns()
        ...
        if useCDs() and ttd("target") > br.getOptionValue("CDs TTD Limit") and targetDistance < 5 then
            ...
            if br.isChecked("Vendetta") and not stealthedRogue and not debuff.vendetta.exists("target") then
                if br.isChecked("Hold Vendetta") and (not talent.subterfuge or not trait.shroudedSuffocation.active or (debuff.garrote.applied("target") > 1
            ...
            if mode.vanish == 1 and not stealthedRogue and gcd < 0.2 and br.getSpellCD(spell.vanish) == 0 then
                ...
                if talent.subterfuge and enemies10 == 1 and br.getSpellCD(spell.garrote) == 0 and (not debuff.garrote.exists("target")) and comboDeficit >= (1+2*sSActive) then
                ...
                if talent.exsanguinate and talent.subterfuge and enemies10 == 1 and comboDeficit >=(1+2*sSActive) and cd.exsanguinate.remain() < 1 and debuff.garrote.refresh("target") and br.getSpellCD(spell.garrote) == 0 then
                    if cast.pool.garrote() then return true end
                    if cast.vanish("player") then return true end
                end
                ...
                if talent.nightstalker and not talent.exsanguinate and comboDeficit >= comboMax and (debuff.vendetta.exists("target") or not br.isChecked("Vendetta")) then
                    if cast.vanish("player") then return true end
                end
                ...
                if talent.subterfuge and (not talent.exsanguinate or enemies10 > 1) and not stealthedRogue and br.getSpellCD(spell.garrote) == 0 and debuff.garrote.refresh("target") and ((enemies10 <= 3 and comboDeficit >= 1 + enemies10) or (enemies10 >= 4 and comboDeficit >= 4)) then
                    if cast.pool.garrote(nil, nil, 2) then return true end
                    if cast.vanish("player") then return true end
                end
            ...
            elseif #enemies.yards30 > 1 then
                for i = 1, #enemies.yards30 do
                    ...
                    if br.getTTD(thisUnit) ~= 999 then
                        ...
                        if br._G.UnitHealth(thisUnit) <= reapingDamage or reapTTD < 2 or buff.reapingFlames.remain() <= 1.5 then
        ...
        if mode.exsang == 1 and enemies10 > 1 and talent.exsanguinate and br.getSpellCD(spell.exsanguinate) == 0 and debuff.rupture.remain("target") > 16 and (not debuff.garrote.refresh("target") or garroteCheck == false) and ttd("target") > 4 then
            if cast.exsanguinate("target") then return true end
        end
    ...
    local function actionList_Direct()
        ...
        if combo >= (4 + dSEnabled) and ((debuff.vendetta.exists("target") or not useCDs() or ttd("target") < br.getOptionValue("CDs TTD Limit")) or debuff.toxicBlade.exists("target") or buff.masterAssassin.exists() or energyDeficit <= (25 + energyRegenCombined) or enemies10 > 1) and (not talent.exsanguinate or cd.exsanguinate.remain() > 2 or not debuff.vendetta.exists() or mode.exsang == 2 or ttd("target") < 8) then
            if cast.envenom("target") then return true end
        end
        ...
        if useFiller and enemies10 == 2 then
            if (br.getOptionValue("Poison") == 1 and not debuff.deadlyPoison.exists("target")) or (br.getOptionValue("Poison") == 2 and not debuff.woundPoison.exists("target")) then
                if cast.mutilate("target") then return true end
            end
            for i = 1, #enemyTable5 do
                local thisUnit = enemyTable5[i].unit
                if (br.getOptionValue("Poison") == 1 and not debuff.deadlyPoison.exists(thisUnit)) or (br.getOptionValue("Poison") == 2 and not debuff.woundPoison.exists(thisUnit)) then
                    if cast.mutilate(thisUnit) then return true end
                end
            end
        end
        -- Throw  Poisoned Knife if we can't reach the target
        if br.isChecked("Poisoned Knife out of range") and not stealthedRogue and #enemyTable5 == 0 and energy >= br.getOptionValue("Poisoned Knife out of range") then
            for i = 1, #enemyTable30 do
    ...
    local function actionList_Dot()
        ...
        if mode.exsang == 1 and talent.exsanguinate and debuff.garrote.remain("target") < 16 and (mode.vanish == 0 or cd.vanish.remain() > 4) and not debuff.garrote.exsang("target") and cd.exsanguinate.remain() < 2 and enemies10 == 1 and debuff.garrote.remain("target")*0.5 < ttd("target") then
            if cast.garrote("target") then return true end
        end
        ...
        if mode.exsang == 1 and enemies10 < 3 and talent.exsanguinate and debuff.rupture.remain("target") < 25 and (not debuff.garrote.refresh("target") or debuff.vendetta.exists("target")) and ((combo >= comboMax and cd.exsanguinate.remain() < 1) or (not debuff.rupture.exists("target") and (combatTime > 10 or combo >= 2))) and ttd("target") > 10 then
            if cast.rupture("target") then return true end
        end
...
--- In Combat - Rotations ---
-----------------------------
        if (inCombat or (not br.isChecked("Disable Auto Combat") and (cast.last.vanish(1) or cast.last.vanish(2) or mode.vanish == 1 or (validTarget and targetDistance < 5)))) and opener == true then
