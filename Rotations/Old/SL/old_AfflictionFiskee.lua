...
--------------------
    ...
    -- Dummy Test
        ...
        --Soulstone
        if br.isChecked("Auto Soulstone Mouseover") and inCombat and not moving and UnitIsPlayer("mouseover") and UnitIsDeadOrGhost("mouseover") and br.GetUnitIsFriend("mouseover","player") then
...
-- Action List - Burst Target
    local function actionList_BurstTarget()
        ...
        if br.getSpellCD(spell.summonDarkglare) == 0 and (useCDs() or (br.isChecked("CDs With Burst Key") and burstKey)) and debuff.agony.exists() and debuff.corruption.exists() and (debuff.unstableAffliction.stack() == 5 or shards == 0) and (not talent.phantomSingularity or (talent.phantomSingularity and (cd.phantomSingularity.remain() > 0 or #enemies.yards15t < br.getOptionValue("PS Units") or mode.ps ~= 1))) then
        ...
        if mode.ps == 1 and #enemies.yards15t >= br.getOptionValue("PS Units") or (br.isChecked("CDs With Burst Key") and burstKey) or (br.isChecked("Ignore PS units when using CDs") and useCDs()) then
        ...
        -- actions+=/dark_soul
        if useCDs() or br.isChecked("CDs With Burst Key") and not moving then
            if cast.darkSoul("player") then return true end
        end
        -- actions+=/berserking
        if br.isChecked("Racial") and race == "Troll" and (useCDs() or (br.isChecked("CDs With Burst Key") and burstKey)) and not moving then
            if cast.racial("player") then return true end
        end
        -- actions+=/unstable_affliction,if=cooldown.summon_darkglare.remains<=soul_shard*cast_time
        if not moving and ttd("target") > 2 and (((useCDs() or (br.isChecked("CDs With Burst Key") and burstKey)) and cd.summonDarkglare.remain() <= shards * cast.time.unstableAffliction()) or (not useCDs() and not br.isChecked("CDs With Burst Key"))) then
            if cast.unstableAffliction() then return true end
        end
        --Spread agony
        if br.isChecked("Spread Agony on ST") and mode.rotation == 3 then
            ...
            if (talent.creepingDeath and agonyCount < 6) or (not talent.creepingDeath and agonyCount < 8) and agonyCount < br.getOptionValue("Multi-Dot Limit") then
                for i = 1, #enemyTable40 do
                    local thisUnit = enemyTable40[i].unit
        ...
        if ((useCDs() or (br.isChecked("CDs With Burst Key") and burstKey)) and cd.summonDarkglare.remain() < timeToShard * (5 - shards) and ttd("target") > cd.summonDarkglare.remain()) then
...
-- Action List - Rotation
    local function actionList_Rotation()
        ...
        -- actions+=/summon_darkglare,if=dot.agony.ticking&dot.corruption.ticking&(buff.active_uas.stack=5|soul_shard=0)&(!talent.phantom_singularity.enabled|cooldown.phantom_singularity.remains)
        if br.getSpellCD(spell.summonDarkglare) == 0 and useCDs() and debuff.agony.exists() and debuff.corruption.exists() and (debuff.unstableAffliction.stack() == 5 or shards == 0) and (not talent.phantomSingularity or (talent.phantomSingularity and (cd.phantomSingularity.remain() > 0 or #enemies.yards15t < br.getOptionValue("PS Units") or mode.ps ~= 1))) then
            br._G.CastSpellByName(GetSpellInfo(spell.summonDarkglare))
            return true
        end
        ...
        if mode.ps == 1 and combatTime > 40 and (cd.summonDarkglare.remain() >= 45 or cd.summonDarkglare.remain() < 8) then
            if br.getOptionValue("Phantom Singularity Target") == 1 and (#enemies.yards15t >= br.getOptionValue("PS Units") or (br.isChecked("Ignore PS units when using CDs") and useCDs())) then
                if cast.phantomSingularity("target", "aoe", 1, 15) then return true end
            elseif br.getOptionValue("Phantom Singularity Target") == 2 then
                if br.isChecked("Ignore PS units when using CDs") and useCDs() then
                    if cast.phantomSingularity("best",false,1,10) then return true end
                else
                    if cast.phantomSingularity("best",false,br.getOptionValue("PS Units"),10) then return true end
                end
            end
        end
        ...
        if talent.siphonLife and siphonCount < (8 - creepingDeathValue - seedTargetsHit) and siphonCount < br.getOptionValue("Multi-Dot Limit") then
        ...
        -- actions+=/berserking
        if br.isChecked("Racial") and race == "Troll" and useCDs() and not moving then
            if cast.racial("player") then return true end
        end
    ...
    -- Summon Pet
        ...
        -- Food
            ...
            if (not br.isChecked("Opener") or opener == true) then
                if useCDs() and br.isChecked("Pre-Pull Timer") then --and pullTimer <= br.getOptionValue("Pre-Pull Timer") then
                    if pullTimer <= br.getOptionValue("Pre-Pull Timer") - 0.5 then
                    ...
                    if pullTimer <= br.getOptionValue("Pre-Pull Timer") - 0.5 then
                        if talent.soulEffigy and not effigied then
                            if not br.GetObjectExists("target") then
            ...
            -- Pet Attack/Follow
                    ...
                    if not moving and #br.getEnemies("target", 10, true) >= 3 then
                    ...
                    if not moving and #br.getEnemies("target", 10, false) < 3 and not talent.haunt and not talent.drainSoul then
                        if cast.shadowBolt("target") then return true end
                    end
                    --else agony
                    if cast.agony("target") then return true end
...
-----------------------
    ...
    -- Pet Attack
                ...
                -- Burst
                if (br.isChecked("Burst Target Key (hold)") and burstKey and not GetCurrentKeyBoardFocus()) or mode.rotation == 3 then
