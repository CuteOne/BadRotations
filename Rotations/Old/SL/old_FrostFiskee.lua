...
local function runRotation()
    ...
    local function isFrozen(unit)
        local function getRawDistance(unit)
            ...
            local x2, y2, z2 = br._G.ObjectPosition(unit)
    ...
    if br.timer:useTimer("targetMove", 0.8) or combatTime < 0.2 then
        if UnitIsVisible("target") then
            local currentX, currentY, currentZ = br._G.ObjectPosition("target")
            local targetMoveDistance = math.sqrt(((currentX - lastTargetX) ^ 2) + ((currentY - lastTargetY) ^ 2) + ((currentZ - lastTargetZ) ^ 2))
            lastTargetX, lastTargetY, lastTargetZ = br._G.ObjectPosition("target")
    ...
    local function actionList_Extras()
        ...
        --Pet assist
        if br.isChecked("Pet Management") and UnitIsVisible("pet") and not petFollowActive() and (not inCombat or br.getDistance("target", "pet") > 40) then
            PetFollow()
        end
        -- Spell Steal
    ...
    local function actionList_Defensive()
        if useDefensive() then
            ...
            if br.getDistance("target") < 12 and not br.isBoss("target") then
    ...
    local function actionList_Essences()
        ...
        if br.isChecked("Guardian of Azeroth") and cast.able.guardianOfAzeroth() and (br.getOptionValue("Use Essences") == 1 or (br.getOptionValue("Use Essences") == 2 and useCDs())) then
        ...
        if standingTime > 1 and br.isChecked("Focused Azerite Beam") and cast.able.focusedAzeriteBeam() and (br.getOptionValue("Use Essences") == 1 or (br.getOptionValue("Use Essences") == 2 and useCDs())) and essence.focusedAzeriteBeam.active and cd.focusedAzeriteBeam.remains() <= gcd and ((essence.focusedAzeriteBeam.rank < 3 and not moving) 
        or essence.focusedAzeriteBeam.rank >= 3) and not buff.runeOfPower.exists("player") and br.getFacing("player","target") and (getEnemiesInRect(10,25,false,false) >= br.getOptionValue("Focused Azerite Beam") or ((getEnemiesInRect(10,40,false,false) >= 1 or (br.getDistance("target") < 6 and br.isBoss("target")))))
        then
        ...
        if br.isChecked("Memory of Lucid Dreams") and cast.able.memoryOfLucidDreams() and (br.getOptionValue("Use Essences") == 1 or (br.getOptionValue("Use Essences") == 2 and useCDs())) and blizzardUnits < 5 and (iciclesStack <= 1 or not talent.glacialSpike) and cd.frozenOrb.remain() > 10 and useCDs() then
        ...
        if br.isChecked("Purifying Blast") and cast.able.purifyingBlast() and (br.getOptionValue("Use Essences") == 1 or (br.getOptionValue("Use Essences") == 2 and useCDs())) and blizzardUnits > 3 or not buff.runeOfPower.exists() then
        ...
        if br.isChecked("Ripple in Space") and cast.able.rippleInSpace() and (br.getOptionValue("Use Essences") == 1 or (br.getOptionValue("Use Essences") == 2 and useCDs())) and blizzardUnits > 3 or not buff.runeOfPower.exists() then
        ...
        if br.isChecked("Worldvein Resonance") and cast.able.worldveinResonance() and (br.getOptionValue("Use Essences") == 1 or (br.getOptionValue("Use Essences") == 2 and useCDs())) and blizzardUnits > 3 or not buff.runeOfPower.exists() then
        ...
        if br.isChecked("The Unbound Force") and cast.able.theUnboundForce() and buff.recklessForce.exists() and (br.getOptionValue("Use Essences") == 1 or (br.getOptionValue("Use Essences") == 2 and useCDs())) then
		...
		for i = 1, #enemies.yards40 do
            ...
            local distance = br.getDistance(thisUnit)
                if br.isChecked("Reaping Flames") and cast.able.reapingFlames(thisUnit) and not buff.runeOfPower.exists("player") and (br.getOptionValue("Reaping Flames") == 1) then
                    if cast.reapingFlames(thisUnit) then
                        br.addonDebug("Reaping 1")
                        return
                    end
                elseif br.isChecked("Reaping Flames") and cast.able.reapingFlames(thisUnit) and not buff.runeOfPower.exists("player") and br.getOptionValue("Reaping Flames") == 2 and (buff.reapingFlames.exists("player") and (br._G.UnitHealth(thisUnit) <= reapingDamage*2)) or (not buff.reapingFlames.exists("player") and (br._G.UnitHealth(thisUnit) <= reapingDamage)) then
                    if cast.reapingFlames(thisUnit) then
    ...
    local function actionList_Cooldowns()
        if useCDs() and not moving and targetUnit.ttd >= br.getOptionValue("Cooldowns Time to Die Limit") then
    ...
    local function actionList_Leveling()
        ...
        if cast.able.arcaneExplosion() and br.getDistance("target") <= 10 and manaPercent > 30 and #enemies.yards10 >= br.getOptionValue("Arcane Explosion Units") then
            br._G.CastSpellByName(GetSpellInfo(spell.arcaneExplosion))
        end
        if mode.frozenOrb == 1 and useCDs() then
            if castFrozenOrb(1, true, 4) then return true end
        else
        -- Frozen Orb Key
            if mode.frozenOrb == 2 and br.isChecked("Frozen Orb Key") and SpecificToggle("Frozen Orb Key") and not GetCurrentKeyBoardFocus() then
                br._G.CastSpellByName(GetSpellInfo(spell.frozenOrb))
                return
            end
        end
    ...
    local function actionList_ST()
        ...
        if mode.frozenOrb == 1 and not moving and targetMoveCheck then
            if not br.isChecked("Obey AoE units when using CDs") and useCDs() then
                if castFrozenOrb(1, true, 4) then return true end
            else
        ...
        if talent.cometStorm and not moving and mode.cometStorm == 1 and not isMoving("target") and targetUnit.ttd > 3 and ((not br.isChecked("Obey AoE units when using CDs") and useCDs()) or #br.getEnemies("target", 5) >= br.getOptionValue("Comet Storm Units")) then
            if cast.cometStorm("target") then
                if UnitIsVisible("pet") and not br.isBoss("target") then
        ...
        if mode.rotation ~= 2 and not tankMoving and not moving and not playerCasting then
            if buff.freezingRain.exists() then
                ...
                if blizzardUnits >= 2 and not br.isChecked("Obey AoE units when using CDs") and useCDs() then
                    if br.createCastFunction("best", false, 2, 8, spell.blizzard, nil, true, 3) then
                        return true
                    end
    ...
    local function actionList_AoE()
        ...
        -- arcane explosion
        if mode.rotation ~= 2 and cast.able.arcaneExplosion() and br.getDistance("target") <= 10 and manaPercent > 30 and #enemies.yards10 >= br.getOptionValue("Arcane Explosion Units") then
            br._G.CastSpellByName(GetSpellInfo(spell.arcaneExplosion))
        end
        ...
        if mode.cometStorm == 1 and not moving and not isMoving("target") and targetUnit.ttd > 3 and ((br.isChecked("Ignore AoE units when using CDs") and useCDs()) or #br.getEnemies("target", 5) >= br.getOptionValue("Comet Storm Units")) then
            if cast.cometStorm("target") then
                if UnitIsVisible("pet") and not br.isBoss("target") then
                    C_Timer.After(playerCastRemain + 0.4, function()
                        if UnitIsVisible("target") then
    ...
    local function actionList_Rotation()
        if (((fofExists and not br.isChecked("No Ice Lance")) or ((bfExists or ifCheck()) and iciclesStack > 5)) and interruptCast(spell.frostbolt)) or (bfExists and interruptCast(spell.ebonbolt)) then
            SpellStopCasting()
            return true
        end
        if spellQueueReady() then
            ...
            if not br.isChecked("No Ice Lance") and cast.last.flurry() and not fofExists then
                if cast.iceLance("target") then return true end
            end
    ...
    local function actionList_PreCombat()
        ...
        if not inCombat and not (IsFlying() or IsMounted()) then
            if (not br.isChecked("Opener") or opener == true) then
                ...
                if targetUnit and (not br.isChecked("Opener") or opener == true) then
                    if br.isChecked("Pet Management") and not talent.lonelyWinter and not UnitAffectingCombat("pet") then
                        PetAssistMode()
                        PetAttack("target")
                    end
                    if br.getOptionValue("APL Mode") == 2 then
    ...
    if not inCombat and not hastar and profileStop == true then
        profileStop = false
    elseif (inCombat and profileStop == true) or IsMounted() or UnitChannelInfo("player") or IsFlying() or pause(true) or br.isCastingSpell(293491) or cast.current.focusedAzeriteBeam() then
        if not pause(true) and not talent.lonelyWinter and IsPetAttackActive() and br.isChecked("Pet Management") then
            PetStopAttack()
            PetFollow()
        end
        return true
    ...
    --- In Combat Rotation ---
    --------------------------        
        if (inCombat or cast.inFlight.frostbolt() or targetUnit) and profileStop == false and targetUnit and (opener == true or not br.isChecked("Opener") or not br.isBoss("target")) then
        --------------------------
        --- Defensive Rotation ---
        --------------------------
        ...
        ------------------------------
            ...
            if not pause(true) and targetUnit.calcHP > 0 and (targetUnit.facing or br.isChecked("Auto Facing")) then
                if br.isChecked("Pet Management") and not talent.lonelyWinter and UnitIsVisible("pet") and not br.GetUnitIsUnit("pettarget", "target") and targetUnit then
                    PetAttack()
                end
            --------------------------
            ---      Rotation      ---
