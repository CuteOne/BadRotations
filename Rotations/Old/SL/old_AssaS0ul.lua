    ...
    if #enemies.yards30 > 0 then
        ...
        for i = 1, #enemyTable30 do
            ...
            if IsSpellInRange(spell10y, thisUnit.unit) == 1 then
                if fokIgnore[thisUnit.objectID] == nil and not br.isTotem(thisUnit.unit) then
                    ...
                    if instantPoison10 and not trait.echoingBlades.active and (br.getOptionValue("Poison") == 1 and not debuff.deadlyPoison.exists(thisUnit.unit)) or (br.getOptionValue("Poison") == 2 and not debuff.woundPoison.exists(thisUnit.unit)) then instantPoison10 = false end
                end
        ...
        if br.isChecked("Auto Target Burn Units") and inCombat and not stealthedRogue and #enemies.yards5 > 0 and ((UnitIsVisible("target") and autoTargetUnits[br.GetObjectID("target")] == nil) or not UnitIsVisible("target")) then
            for i = 1, #enemies.yards5 do
                ...
                if autoTargetUnits[objID] ~= nil and (IsHackEnabled("Auto Facing") or br.getFacing("player", thisUnit)) then
...
--------------------
    ...
    local function actionList_Defensive()
        if useDefensive() then
            ...
            if php <= br.getOptionValue("Corruption Immunity") and not IsMounted() and cd.global.remain() == 0 then
    ...
    local function actionList_PreCombat()
        ...
        if br.isChecked("Precombat") and pullTimer <= 1.5 then
    ...
    local function actionList_Cooldowns()
        ...
        if useCDs() and ttd("target") > br.getOptionValue("CDs TTD Limit") and br.isChecked("Potion") and (hasBloodLust() and cd.vendetta.remain() > 0 or debuff.vendetta.exists("target")) and targetDistance < 5 then
            if br.getOptionValue("Potion") == 1 and use.able.superiorBattlePotionOfAgility() and not buff.superiorBattlePotionOfAgility.exists() then
                use.superiorBattlePotionOfAgility()
                return true
            elseif br.getOptionValue("Potion") == 2 and use.able.potionOfUnbridledFury() and not buff.potionOfUnbridledFury.exists() then
        ...
        -- actions.cds+=/use_item,name=galecallers_boon,if=(debuff.vendetta.up|(!talent.exsanguinate.enabled&cooldown.vendetta.remains>45|talent.exsanguinate.enabled&(cooldown.exsanguinate.remains<6|cooldown.exsanguinate.remains>20&fight_remains>65)))&!exsanguinated.rupture
            if hasEquiped(159614, 13) and br.canUseItem(13) and (debuff.vendetta.exists("target") or (not talent.exsanguinate and cd.vendetta.remain() > 45 or talent.exsanguinate and (cd.exsanguinate.remain() < 4 or cd.exsanguinate.remain() > 25) and fightRemain > 65)) and not debuff.rupture.exsang("target") and targetDistance <= 5 then
                br.useItem(13)
            elseif hasEquiped(159614, 14) and br.canUseItem(14) and (debuff.vendetta.exists("target") or (not talent.exsanguinate and cd.vendetta.remain() > 45 or talent.exsanguinate and (cd.exsanguinate.remain() < 4 or cd.exsanguinate.remain() > 25) and fightRemain > 65)) and not debuff.rupture.exsang("target") and targetDistance <= 5 then
                br.useItem(14)
            end
        end
        ...
        if useCDs() and ttd("target") > br.getOptionValue("CDs TTD Limit") and targetDistance < 5 then
            ...
            if br.isChecked("Vendetta") and not stealthedRogue and not debuff.vendetta.exists("target") then
                if br.isChecked("Hold Vendetta") and (not talent.subterfuge or not sSActive or (debuff.garrote.applied("target") > 1
            ...
            elseif mob_count > 1 then
                for i = 1, mob_count do
                    ...
                    if br.getTTD(thisUnit) ~= 999 then
                        ...
                        if br._G.UnitHealth(thisUnit) <= reapingDamage or br.getTTD(thisUnit) < 2.5 or br.getTTD(thisUnit, reapingPercentage) < 2 then
        ...
        if mode.exsang == 1 and talent.exsanguinate and br.getSpellCD(spell.exsanguinate) == 0 and debuff.rupture.remain("target") > 25 and ttd("target") > 4 and
        ...
        if mode.exsang == 1 and enemies10 > 1 and not br.isBoss() and talent.exsanguinate and br.getSpellCD(spell.exsanguinate) == 0 and debuff.rupture.remain("target") > 15 and 
    ...
    local function actionList_Direct()
        ...
        if combo >= (4 + dSEnabled) and ((debuff.vendetta.exists("target") or not useCDs() or ttd("target") < br.getOptionValue("CDs TTD Limit")) or debuff.toxicBlade.exists("target") or buff.masterAssassin.exists() or energyDeficit <= (25 + energyRegenCombined) or enemies10 > 1) and 
    ...
    local function actionList_Dot()
        ...
        if talent.crimsonTempest and not queenBuff and enemies10 == 1 and debuff.rupture.exists("target") and combo >= (comboMax - 1) and debuff.crimsonTempest.refresh("target") and br.GetObjectID("target") ~= 157620
        ...
        if talent.crimsonTempest and not queenBuff and enemies10 > 1 and not buff.stealth.exists() and not buff.vanish.exists() and br.GetObjectID("target") ~= 157620 then
        ...
        if talent.crimsonTempest and not queenBuff and ctenemies10 > (7 - buffEnvenom) and combo >= (4 + dSEnabled) and not debuff.vendetta.exists("target") and 
         not debuff.toxicBlade.exists("target") and not trait.twistTheKnife.activede and energyDeficit > (25 + energyRegenCombined) and br.GetObjectID("target") ~= 157620 then
            if cast.crimsonTempest("player") then return true end
        end
