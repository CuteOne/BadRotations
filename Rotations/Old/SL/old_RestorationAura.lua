...
--------------------
    -- Action List - Extras
        ...
        -- Water Walking
            ...
            -- Ancestral Spirit
            if br.isChecked("Ancestral Spirit") and not inCombat and movingCheck and br.timer:useTimer("Resurrect", 4) then
                if br.getOptionValue("Ancestral Spirit")==1 and UnitIsPlayer("target") and UnitIsDeadOrGhost("target") and br.GetUnitIsFriend("target","player") then
                    if cast.ancestralSpirit("target","dead") then br.addonDebug("Casting Ancestral Spirit") return true end
                end
                if br.getOptionValue("Ancestral Spirit")==2 and UnitIsPlayer("mouseover") and UnitIsDeadOrGhost("mouseover") and br.GetUnitIsFriend("mouseover","player") then
                    if cast.ancestralSpirit("mouseover","dead") then br.addonDebug("Casting Ancestral Spirit") return true end
                end
                if br.getOptionValue("Ancestral Spirit") == 3 then
                    local deadPlayers = {}
                    for i =1, #br.friend do
    ...
    -- Action List - Defensive
        local function actionList_Defensive()
            ...
            if cast.able.earthShield() then
                ...
                if br.isChecked("Earth Shield") then
                    for i = 1, #br.friend do
                    ...
                    if foundShield == false then
                        if br.GetUnitExists("focus") == true then
                            ...
                            for i = 1, #tanks do
                                if not buff.earthShield.exists(tanks[i].unit) and br.getDistance(tanks[i].unit) <= 40 then
            ...
            -- Astral Shift
                ...
                if cd.capacitorTotem.remain() <= gcd then
                    if br.isChecked("Capacitor Totem - HP") and php <= br.getOptionValue("Capacitor Totem - HP") and inCombat and lastSpell ~= spell.capacitorTotem and #enemies.yards5 > 0 then
                        if cast.capacitorTotem("player") then br.addonDebug("Casting Capacitor Totem") return end
                    end
                    if br.isChecked("Capacitor Totem - AoE") and #enemies.yards5 >= br.getOptionValue("Capacitor Totem - AoE") and inCombat and lastSpell ~= spell.capacitorTotem then
                        if cast.capacitorTotem("player") then br.addonDebug("Casting Capacitor Totem") return end
                    end
                end
    ...
    -- Action List - Interrupts
        ...
        -- Capacitor Totem
                        if br.isChecked("Capacitor Totem") then
        ...
        function actionList_PreCombat()
            ...
            if movingCheck and cd.healingRain.remain() <= gcd then
                if (SpecificToggle("Healing Rain Key") and not GetCurrentKeyBoardFocus()) and br.isChecked("Healing Rain Key") then
                    if br._G.CastSpellByName(GetSpellInfo(spell.healingRain),"cursor") then br.addonDebug("Casting Healing Rain") return end 
                end
            end
        -- Riptide
    ...
    -- Action List - DPS
        ...
        local function actionList_AMR()
            ...
            --Spirit Link Key
            if (SpecificToggle("Spirit Link Totem Key") and not GetCurrentKeyBoardFocus()) and br.isChecked("Spirit Link Totem Key") then
                if br._G.CastSpellByName(GetSpellInfo(spell.spiritLinkTotem),"cursor") then br.addonDebug("Casting Spirit Link Totem") return end 
            end
            -- Coastal Mana Potion
            if br.isChecked("Mana Pot") and power <= br.getOptionValue("Mana Pot")
                and inCombat and  br.hasItem(152495)
            then
                if br.canUseItem(152495) then
                    br.useItem(152495)
        ...
        -- Ascendance Key
            if (SpecificToggle("Ascendance Key") and not GetCurrentKeyBoardFocus()) and br.isChecked("Ascendance Key") then
                if cast.ascendance() then br.addonDebug("Casting Ascendance") return end
            end
            -- Tremor Totem Key
            if (SpecificToggle("Tremor Totem Key") and not GetCurrentKeyBoardFocus()) and br.isChecked("Tremor Totem Key") then
                if cast.tremorTotem("player") then br.addonDebug("Casting Tremor Totem") return end
            end
            -- Healing Tide Key
            if (SpecificToggle("Healing Tide Key") and not GetCurrentKeyBoardFocus()) and br.isChecked("Healing Tide Key") then
                if cast.healingTideTotem() then br.addonDebug("Casting Healing Tide Totem") return end
            end
            -- Spirit Link Totem
            if br.isChecked("Spirit Link Totem") and useCDs() and not moving and cd.spiritLinkTotem.remain() <= gcd then
                if raidBurstInc and (not br.isChecked("Burst Count") or (br.isChecked("Burst Count") and burstCount == br.getOptionValue("Burst Count"))) then
        ...
        -- Trinkets
            ...
            if br.isChecked("Life-Binder's Invocation") and essence.lifeBindersInvocation.active and cd.lifeBindersInvocation.remain() <= gcd and getLowAllies(br.getOptionValue("Life-Binder's Invocation")) >= br.getOptionValue("Life-Binder's Invocation Targets") then
        ...
        --  Lucid Dream
            if br.isChecked("Lucid Dreams") and essence.memoryOfLucidDreams.active and mana <= 85 and br.getSpellCD(298357) <= gcd then
            ...
            if movingCheck and cd.healingRain.remain() <= gcd and br.timer:useTimer("HR Delay",5) then
                if br.isChecked("Healing Rain") and not buff.healingRain.exists() and mode.healingR == 1 then
            ...
            if cd.downpour.remain() <= gcd and movingCheck then
                if br.isChecked("Downpour") then
                    if br.isChecked("Downpour on Melee") then
                        -- get melee players
                        for i=1, #tanks do
                            -- get the tank's target
                            local tankTarget = br._G.UnitTarget(tanks[i].unit)
...
-----------------
        ...
        ghostWolf()
        if inCombat then
           if br._G.IsAoEPending()then SpellStopTargeting() br.addonDebug(colorRed.."Canceling Spell") end
        end
        -- Pause
        if pause() then
            return true
...
-----------------------------
            if inCombat and not IsMounted() and not drinking then
                if (buff.ghostWolf.exists() and mode.ghostWolf == 1) or not buff.ghostWolf.exists() then
                    ...
                    actionList_AMR()
                    -- Purge
                    if br.isChecked("Purge") and lowest.hp > br.getOptionValue("DPS Threshold") and power >= br.getOptionValue("Purge Min Mana") then
                    ...
                    if br.player.ui.mode.dPS == 1 and br.GetUnitExists("target") and UnitCanAttack("player","target") and br.getFacing("player","target") and lowest.hp > br.getOptionValue("DPS Threshold") then
