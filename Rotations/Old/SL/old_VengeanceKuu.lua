...
local function runRotation()
    ...
    local function iStrike(unit)
        ...
        if br.getDistance("player", unit) == 0 or br.vengeance.groundOverride[br.getUnitID(unit)] ~= nil then
        ...
        if z ~= nil and TraceLine(px, py, pz + 2, x, y, z + 1, 0x100010) == nil and TraceLine(x, y, z + 4, x, y, z, 0x1) == nil then -- Check z and LoS, ignore terrain and m2 colissions and check no m2 on hook location
            br._G.CastSpellByName(GetSpellInfo(spell.infernalStrike))
    ...
    local function trinketLogic(slot)
        if
            not hasEquiped(169314, slot) and not hasEquiped(165572, slot) and not hasEquiped(167868, slot) and not hasEquiped(169311, slot) and not hasEquiped(167555, slot) and
                ((slot == 13 and php <= br.getOptionValue("Trinket 1")) or slot == 14 and php <= br.getOptionValue("Trinket 2"))
         then
            if slot == 13 then
        ...
        elseif hasEquiped(169314, slot) then
            ...
            if not br.GetUnitIsUnit("player", tank) and php >= 75 and not br.UnitBuffID("player", 296962) and br.timer:useTimer("Font Delay", 4) and not isMoving("player") then
                br.addonDebug("Using Font Of Azshara")
                br.useItem(slot)
                return true
            end
        elseif hasEquiped(169311, slot) then
            if not debuff.razorCoral.exists("target") or (equiped.dribblingInkpod() and (debuff.conductiveInk.exists("target") and (br.getHP("target") < 31)) or ttd("target") < 20) then
    ...
    local function actionList_Extras()
        ...
        -- Torment
        if br.isChecked("Torment") and combatTime > 5 then
            for i = 1, #enemies.yards30 do
                local thisUnit = enemies.yards30[i]
                local target = br._G.UnitTarget(thisUnit)
    ...
    local function actionList_Defensive()
        if useDefensive() then
            -- Demon Spikes
            if br.isChecked("Demon Spikes") and br.timer:useTimer("Spikes delay", 2) and inCombat and
                    ((charges.demonSpikes.count() > br.getOptionValue("Hold Demon Spikes") and php <= br.getOptionValue("Demon Spikes")) or charges.demonSpikes.count() == 2) and
            ...
            -- Null Barrier
            if br.isChecked("Empowered Null Barrier") and inCombat and essence.empoweredNullBarrier.active and cd.empoweredNullBarrier.remain() <= gcd and php <= br.getOptionValue("Empowered Null Barrier") and hasAggro >= 2 then
                if not buff.demonSpikes.exists() and not debuff.fieryBrand.exists("target") and not buff.metamorphosis.exists() then
            ...
            -- Fiery Brand
            if br.isChecked("Fiery Brand") and inCombat and php <= br.getOptionValue("Fiery Brand") and #enemies.yards30 > 0 and hasAggro >= 2 then
            ...
            -- Metamorphosis
            if
                br.isChecked("Metamorphosis") and inCombat and not buff.demonSpikes.exists() and not debuff.fieryBrand.exists("target") and not buff.metamorphosis.exists() and
                    php <= br.getOptionValue("Metamorphosis") and hasAggro >= 2
             then
            ...
            -- Aegis of the Deep
            if br.isChecked("Aegis of the Deep") and essence.aegisOfTheDeep.active and cd.aegisOfTheDeep.remain() <= gcd and inCombat and #enemies.yards8 >= 3 then
            ...
            -- Soul Barrier
            if br.isChecked("Soul Barrier") and inCombat and php < br.getOptionValue("Soul Barrier") and hasAggro >= 2 then
            ...
            -- Sigil of Misery
            if br.isChecked("Sigil of Misery - HP") and php <= br.getOptionValue("Sigil of Misery - HP") and inCombat and #enemies.yards8 > 0 then
                if cast.sigilOfMisery("player", "ground") then
                    br.addonDebug("Casting Sigil of Misery")
                    return
                end
            end
            if br.isChecked("Sigil of Misery - AoE") and #enemies.yards8 >= br.getOptionValue("Sigil of Misery - AoE") and inCombat then
                if cast.sigilOfMisery("best", false, br.getOptionValue("Sigil of Misery - AoE"), 8) then
                    br.addonDebug("Casting Sigil of Misery")
                    return
                end
            end
        ...
        if useInterrupts() then
            for i = 1, #enemies.yards30 do
                ...
                if br.canInterrupt(thisUnit, br.getOptionValue("Interrupt At")) and (sigilDelay == nil or GetTime() - sigilDelay > 2) then
                    ...
                    elseif br.isChecked("Sigil of Silence") and cd.sigilOfSilence.remain() <= gcd then
                        -- Sigil of Misery
                    ...
                    elseif br.isChecked("Sigil of Misery") and cd.sigilOfMisery.remain() <= gcd then
                        if not talent.concentratedSigils then
                    ...
                    elseif br.isChecked("Imprison") and cd.imprison.remain() <= gcd and br.getFacing("player", thisUnit) then
                        local type = UnitCreatureType(thisUnit)
                        for i = 1, #typesLoc do
                            if type == typesLoc[i] then
                                if br.getDistance(thisUnit) < 20 then
    ...
    local function actionList_Cooldowns()
        -- Trinkets
        if useCDs() and br.getDistance("target") < 5 then
            if br.hasItem(166801) and br.canUseItem(166801) then
                br.addonDebug("Using Sapphire of Brilliance")
                br.useItem(166801)
            end
            if br.isChecked("Trinket 1") and canTrinket(13) then
                trinketLogic(13)
            end
            if br.isChecked("Trinket 2") and canTrinket(14) then
                trinketLogic(14)
            end
        end
    ...
    local function actionList_PreCombat()
        if not inCombat then
            ...
            if br.isChecked("Flask / Crystal") then
                if inRaid and canFlask and flaskBuff == 0 and not br.UnitBuffID("player", 188033) then
                    br.useItem(br.player.flask.wod.agilityBig)
                    return
                end
                if flaskBuff == 0 then
                    ...
                    if not br.UnitBuffID("player", 193456) and not br.UnitBuffID("player", 188033) and br.canUseItem(129192) then -- Gaze of the Legion
                        br.useItem(129192)
                        return
                    end
                end
            end
            if br.isChecked("Font of Azshara") and pullTimer <= br.getOptionValue("Font of Azshara") then
                if hasEquiped(169314) and br.canUseItem(169314) and br.timer:useTimer("Font Delay", 4) then
                    br.addonDebug("Using Font Of Azshara")
                    br.useItem(169314)
                end
            end
            if br.isChecked("Cyclotronic on Pull") and pullTimer <= 2 then
                if hasEquiped(167555) and br.canUseItem(167555) and br.timer:useTimer("Cyclo Delay", 3) then
                    br.addonDebug("Using Cyclotronic Blast")
                    br.useItem(167555)
                end
            end
            if br.isChecked("Pre-Pull Timer") and pullTimer <= br.getOptionValue("Pre-Pull Timer") then
                if br.isChecked("Worldvein Resonance") and essence.worldveinResonance.active and cd.worldveinResonance.remain() <= gcd then
            ...
            if br.GetObjectExists("target") and not UnitIsDeadOrGhost("target") and UnitCanAttack("target", "player") and br.getDistance("target") < 5 then
                -- Start Attack
                br._G.StartAttack()
            end
        end -- End No Combat
    ...
    local function actionList_FieryBrand()
        ...
        if br.isChecked("Sigil of Flame") and not isMoving(units.dyn5) and br.getDistance(units.dyn5) < 5 and #enemies.yards5 > 0 and cd.fieryBrand.remain() < 2 then
            if br.vengeance.groundOverride[br.getUnitID("target")] ~= nil and br.getDistance("target") <= 8 then
        ...
        if
            mode.mover == 1 and not cast.last.infernalStrike(1) and charges.infernalStrike.count() == 2 and not cd.fieryBrand.exists() and br.getDistance("target") <= 10 and not isMoving("target") and
        ...
        if debuff.fieryBrand.exists(units.dyn5) then
            ...
            if br.isChecked("Immolation Aura") and #enemies.yards5 > 0 then
            ...
            if br.getDistance(units.dyn20) < 20 then
            ...
            if
                mode.mover == 1 and not cast.last.infernalStrike(1) and charges.infernalStrike.count() == 2 and br.getDistance("target") <= 10 and not isMoving("target") and
                    C_LossOfControl.GetNumEvents() == 0 and
                    GetTime() - iStrikeDelay > 2
             then
            ...
            if br.isChecked("Sigil of Flame") and not isMoving(units.dyn5) and br.getDistance(units.dyn5) < 5 and #enemies.yards5 > 0 then
                if br.vengeance.groundOverride[br.getUnitID("target")] ~= nil and br.getDistance("target") <= 8 then
                    if cast.sigilOfFlame("player", "ground") then
                        br.addonDebug("Casting Sigil Of Flame")
                        return
    ...
    local function actionList_ActiveMitigation()
        if ShouldMitigate() and #enemies.yards8 > 0 and hasAggro >= 2 then
            if br.isChecked("Trinket 1") and canTrinket(13) and not hasEquiped(169311, 13) and not hasEquiped(167555, 13) then
                trinketLogic(13)
                return
            elseif br.isChecked("Trinket 2") and canTrinket(14) and not hasEquiped(169311, 14) and not hasEquiped(167555, 14) then
                -- Metamorphosis
                trinketLogic(14)
                return
            elseif br.isChecked("Metamorphosis") and not buff.demonSpikes.exists() and not debuff.fieryBrand.exists("target") and not buff.metamorphosis.exists() then
                -- Demon Spikes
                if cast.metamorphosis() then
                    br.addonDebug("Casting Metamorphosis (Active Mitigation)")
                    return
                end
            elseif br.isChecked("Demon Spikes") and charges.demonSpikes.count() > 0 and br.timer:useTimer("Spikes delay", 1) then
                -- Soul Barrier
                if not buff.demonSpikes.exists() and not debuff.fieryBrand.exists("target") and not buff.metamorphosis.exists() then
                    if cast.demonSpikes() then
            ...
            elseif br.isChecked("Soul Barrier") then
                if cast.soulBarrier() then
    ...
    elseif (inCombat and profileStop == true) or IsMounted() or IsFlying() or pause() or mode.rotation == 4 then
        ...
        if br.isCastingSpell(296962) and php < 75 then
            RunMacroText("/stopcasting")
        end
        -- Infernal Strike
        if SpecificToggle("Infernal Strike Key") and not GetCurrentKeyBoardFocus() and br.isChecked("Infernal Strike Key") and GetTime() - iStrikeDelay > 2 then
            br._G.CastSpellByName(GetSpellInfo(spell.infernalStrike), "cursor")
            iStrikeDelay = GetTime()
            return
        end
        -- Sigil of Chains
        if SpecificToggle("Sigil of Chains Key") and not GetCurrentKeyBoardFocus() and br.isChecked("Sigil of Chains Key") and cd.sigilOfChains.remain() <= gcd then
            br._G.CastSpellByName(GetSpellInfo(spell.sigilOfChains), "cursor")
            return
        end
        -----------------------
        --- Extras Rotation ---
        ...
        --- In Combat Rotation ---
        --------------------------
        if inCombat and br.isValidUnit("target") and br.getFacing("player", "target") then
            br.ChatOverlay("In-Combat!")
            ...
            -- Consume Magic
            if br.isChecked("Consume Magic") then
                for i = 1, #enemies.yards30 do
                    local thisUnit = enemies.yards30[i]
            ...
            -- Vigilant Protector
            if br.isChecked("Strength of the Warden") and essence.vigilantProtector.active and cd.vigilantProtector.remain() <= gcdMax and #enemies.yards8 >= 3 then
            ...
            -- sigil_of_flame
            if br.isChecked("Sigil of Flame") and not isMoving("target") and #enemies.yards5 > 0 then
                if br.vengeance.groundOverride[br.getUnitID("target")] ~= nil and br.getDistance("target") <= 8 then
                    if cast.sigilOfFlame("player", "ground") then
                        br.addonDebug("Casting Sigil Of Flame")
                        return
            ...
            -- soul_cleave
            if not talent.spiritBomb and php <= br.getOptionValue("Fragless Soul Cleave") then
                if cast.soulCleave() then
                    return
                end
            end
            -- Concentrated Flame Damage
            if br.isChecked("Concentrated Flame") and essence.concentratedFlame.active and php >= br.getOptionValue("Concentrated Flame") and br.getSpellCD(295373) <= gcd then
            ...
            -- soul_cleave
            if talent.spiritBomb and buff.soulFragments.stack() >= 1 and (#enemies.yards5t < 2 and #enemies.yards5t > 0) and php <= br.getOptionValue("Soul Cleave") then
                if cast.soulCleave() then
            ...
            -- spirit_bomb
            if talent.spiritBomb and (buff.soulFragments.stack() >= 4 or (#enemies.yards5t >= 3 and php <= br.getOptionValue("Spirit Bomb"))) then
            ...
            -- soul_cleave
            if painDeficit <= 30 and br.getDistance("target", "player") <= 5 and (not buff.soulFragments.exists() or #enemies.yards5t < 2) and talent.spiritBomb then
                if cast.soulCleave() then
                    br.addonDebug("Casting Soul Cleave")
                    return
                end
            ...
            -- immolation_aura
            if br.isChecked("Immolation Aura") and pain <= 90 and #enemies.yards5 > 0 then
            ...
            -- Font of Azshara
            if br.isChecked("Font of Azshara") and hasEquiped(169314) and br.canUseItem(169314) and br.timer:useTimer("Font Delay", 4) and not isMoving("player") then
                local tank = "player"
                if #getFocusedTank() >= 1 then
                   tank = getFocusedTank()[1]
                end
                if not br.GetUnitIsUnit(tank, "player") and php >= 75 and not br.UnitBuffID("player", 296962) then
                    br.addonDebug("Using Font Of Azshara")
                    br.useItem(169314)
                end
            end
            -- Worldvein Resonance
            if br.isChecked("Worldvein Resonance") and essence.worldveinResonance.active and br.getSpellCD(295186) <= gcd and #enemies.yards5 > 0 then
                if cast.worldveinResonance() then
                    br.addonDebug("Casting Worldvein Resonance")
                    return
                end
            ...
            -- Lucid Dreams
            if br.isChecked("Lucid Dreams") and essence.memoryOfLucidDreams.active and br.getSpellCD(298357) <= gcd and #enemies.yards5 > 0 then
            ...
            if br.isChecked("Throw Glaive") and #enemies.yards30 > 0 and #enemies.yards5 == 0 and cd.throwGlaive.remain() <= gcd then
