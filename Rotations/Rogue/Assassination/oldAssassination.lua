if select(2, UnitClass("player")) == "ROGUE" then
    ...
    function cAssassination:oldAssassination()
        -- --------------
        -- --- Locals ---
        -- --------------
        ...
        local hasMouse, deadMouse, playerMouse, mouseDist = GetObjectExists("mouseover"), UnitIsDeadOrGhost("mouseover"), UnitIsPlayer("mouseover"), getDistance("player","mouseover")
        local hastar, deadtar, attacktar, playertar = GetObjectExists(tarUnit.dyn0), UnitIsDeadOrGhost(tarUnit.dyn0), UnitCanAttack(tarUnit.dyn0, "player"), UnitIsPlayer(tarUnit.dyn0)
        ...
        local antCharge       = select(4, UnitBuff("player", GetSpellInfo(115189))) or 0
        --Specific Target Variables
        local sapRemain   = getDebuffRemain(tarUnit.dyn10,_Sap) or 0
        local rupRemain   = getDebuffRemain(tarUnit.dyn5,_Rupture,"player") or 0
        local ctRemain    = getDebuffRemain(tarUnit.dyn5,_CrimsonTempest,"player") or 0
        local rupDuration = getDebuffDuration(tarUnit.dyn5,_Rupture,"player") or 0
        local venRemain   = getDebuffRemain(tarUnit.dyn5,_Vendetta,"player") or 0
        local ddRemain    = getDebuffRemain(tarUnit.dyn5,_DeadlyPoison,"player") or 0
        local envRemain   = getDebuffRemain(tarUnit.dyn5,_Envenom,"player") or 0
        ...
        if pause() then
            ...
            --- In Combat ---
            -----------------
            if isInCombat("player") and GetObjectExists(tarUnit.dyn5) then
                ------------------------------
            ...
            -- Start Attack
            if tarDist.dyn5<5 and not stealth and isInCombat("player") and GetObjectExists(tarUnit.dyn0) and profileStop==false then
                StartAttack()
            end
        end -- Pause End
    end --Rogue Roation  End
end --Class Check End
