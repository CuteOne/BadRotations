...
--------------
    ...
    local tricksUnit
    if (isChecked("Auto Tricks") or SpecificToggle("Tricks Key")) and GetSpellCooldown(spell.tricksOfTheTrade) == 0 and not buff.subterfuge.exists() and not buff.masterAssassin.exists() then
    ...
    if #enemies.yards30 > 0 then
        ...
        for i = 1, #enemyTable30 do
            ...
            if IsSpellInRange(spell10y, thisUnit.unit) == 1 then
                ...
                if thisUnit.yards5 and getFacing("player",thisUnit.unit) then
...
--------------------
    local function actionList_Extra()
        if not inCombat then
            ...
            if isChecked("Poison") and (br.pulseDelay == nil or (GetTime() - br.pulseDelay > 2)) then
                ...
                if isChecked("Use Crippling Poison") and not moving and buff.cripplingPoison.remain() < 300 and not cast.last.cripplingPoison(1) then
            ...
            if isChecked("Auto Stealth") then
                if not stealth and IsUsableSpell(GetSpellInfo(spell.stealth)) and not cast.last.vanish() and not IsResting() then
                    ...
                    elseif #enemies.yards20nc > 0 and getOptionValue("Auto Stealth") == 2 then
                ...
                elseif stealth and getOptionValue("Auto Stealth") == 2 and #enemies.yards20nc == 0 then
                    cancelBuff(1784)
                end
            end
        end
        ...
        if UnitIsVisible("target") and inCombat and (burnUnits[GetObjectID("target")] ~= nil or (not isChecked("Dot Players") and UnitIsFriend("target", "player") and validTarget)) and targetDistance < 5 then
            ...
            if getenvdamage() >= UnitHealth("target") and combo > 0 then
        ...
        if useDefensive() then
            ...
            if php <= getOptionValue("Corruption Immunity") and not IsMounted() then
                ...
                (cd.cloakOfShadows.exists() or not isChecked("Cloak of Shadows Corruption") or getValue("Cloak of Shadows Corruption") == 3) then
                    if isChecked("Vanish THING") and #br.friend > 1 then
    ...
    local function actionList_Interrupts()
        ...
        if not stealthedRogue then
            for i=1, #enemies.yards15 do
                ...
                if useInterrupts() and canInterrupt(thisUnit,getOptionValue("Interrupt %")) and (interruptDelay == nil or GetTime() - interruptDelay >= 0.25) then
                    if distance < 7 then
                        if isChecked("Kick") and cd.kick.remain() == 0 then
                            if cast.kick(thisUnit) then br.rogueTables.someone_casting = false interruptDelay = GetTime() return end
                        elseif isChecked("Kidney Shot") and combo > 0 and cd.kidneyShot.remain() <= gcd and (cd.kick.remain() > 0 or not isChecked("Kick")) then
                            if cast.kidneyShot(thisUnit) then br.rogueTables.someone_casting = false interruptDelay = GetTime() return end
                        elseif isChecked("Blind") and (cd.kick.remain() > 0 or not isChecked("Kick")) and cd.blind.remain() <= gcd and (cd.kidneyShot.remain() > 0 or not isChecked("Kidney Shot") or combo == 0) then
                            if cast.blind(thisUnit) then br.rogueTables.someone_casting = false interruptDelay = GetTime() return end
                        end
                    elseif distance > 7 and distance <= 15 and isChecked("Blind") then
                        if cast.blind(thisUnit) then br.rogueTables.someone_casting = false interruptDelay = GetTime() return end
                    end
                end
    ...
    local function actionList_Cooldowns()
        ...
        if useCDs() and ttd("target") > getOptionValue("CDs TTD Limit") and targetDistance < 5 then
            ...
            if isChecked("Vendetta") and not stealthedRogue and not debuff.vendetta.exists("target") then
                if isChecked("Hold Vendetta") and (not talent.subterfuge or sSActive == 0 or (debuff.garrote.applied("target") > 1
            ...
            if mode.vanish == 1 and #br.friend > 1 and not stealthedRogue and gcd < 0.2 and getSpellCD(spell.vanish) == 0 then
    ...
    local function actionList_Direct()
        ...
        if combo >= (4 + dSEnabled) and ((debuff.vendetta.exists("target") or not useCDs() or ttd("target") < getOptionValue("CDs TTD Limit")) or debuff.toxicBlade.exists("target") or buff.masterAssassin.exists() or energyDeficit <= (25 + energyRegenCombined) or enemies10 > 1) 
        and (not talent.exsanguinate or cd.exsanguinate.remain() > 2 or (not debuff.vendetta.exists() and comboDeficit >= comboMax) or mode.exsang == 2) and getFacing("player","target")
        then
            if cast.envenom("target") then return true end
        end
        ...
        if useFiller and enemies10 == 2 then
            if (getOptionValue("Poison") == 1 and not debuff.deadlyPoison.exists("target")) or (getOptionValue("Poison") == 2 and not debuff.woundPoison.exists("target")) and getFacing("player","target") then
                if cast.mutilate("target") then return true end
            end
            for i = 1, #enemyTable5 do
                local thisUnit = enemyTable5[i].unit
                if (getOptionValue("Poison") == 1 and not debuff.deadlyPoison.exists(thisUnit)) or (getOptionValue("Poison") == 2 and not debuff.woundPoison.exists(thisUnit)) and getFacing("player",thisUnit) then
        ...
        if useFiller and getFacing("player","target") then
            if cast.mutilate("target") then return true end
        end
        --evis low level
...
-----------------
    ...
    if isChecked("Auto Tricks") then
        if tricksUnit ~= nil then
            for i = 1, #enemies.yards30 do
            local thisUnit = enemies.yards30[i]
                if (UnitTarget(thisUnit) ~= "TANK" or UnitGroupRolesAssigned(UnitTarget(thisUnit)) ~= "TANK") and UnitThreatSituation(tricksUnit, thisUnit) ~= nil and UnitThreatSituation(tricksUnit, thisUnit) <= 2 then
                    if cast.tricksOfTheTrade(tricksUnit) then end
                end
            end
        end
