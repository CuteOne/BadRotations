...
local function createOptions()
    ...
    local function rotationOptions()
        ...
        section = br.ui:createSection(br.ui.window.profile,  "General")
            ...
            br.ui:createCheckbox(section, "Use Crippling Poison")
            ...
            br.ui:createDropdownWithout(section, "Tricks Key", br.dropOptions.Toggle, 6)
...
--------------
    ...
    local stealthedRogue                                = stealth or br.player.buff.vanish.exists() or buff.subterfuge.remains() > 0.4
    ...
    local tricksUnit
    if (br.isChecked("Auto Tricks") or SpecificToggle("Tricks Key")) and GetSpellCooldown(spell.tricksOfTheTrade) == 0 and not buff.subterfuge.exists() and not buff.masterAssassin.exists() then
    ...
    local function shallWeDot(unit)
        if br.isChecked("Auto Garrote HP Limit") and ttd(unit) == 999 and not UnitIsPlayer(unit) and not br.isDummy(unit) then
            local hpLimit = 0
            for i = 1, #br.friend do
                local thisUnit = br.friend[i].unit
    ...
    if #enemies.yards30 > 0 then
        ...
        for i = 1, #enemies.yards30 do
            ...
            if (not noDotCheck(thisUnit) or br.GetUnitIsUnit(thisUnit, "target")) and not UnitIsDeadOrGhost(thisUnit) 
             and (mode.rotation ~= 2  or (mode.rotation == 2 and br.GetUnitIsUnit(thisUnit, "target"))) 
             and (not debuff.vendetta.exists("target") or not talent.toxicBlade or (talent.toxicBlade and talent.subterfuge)) then
                ...
                enemyUnit.objectID = br.GetObjectID(thisUnit)
        ...
        for i = 1, #enemyTable30 do
            ...
            if IsSpellInRange(spell10y, thisUnit.unit) == 1 then
                if fokIgnore[thisUnit.objectID] == nil and not br.isTotem(thisUnit.unit) then
                    tinsert(enemyTable10, thisUnit)
                    if deadlyPoison10 and not trait.echoingBlades.active and (br.getOptionValue("Poison") == 1 and not debuff.deadlyPoison.exists(thisUnit.unit)) or (br.getOptionValue("Poison") == 2 and not debuff.woundPoison.exists(thisUnit.unit)) then deadlyPoison10 = false end
                end
                if debuff.garrote.remain(thisUnit.unit) > 0.5 then garroteCount = garroteCount + 1 end
                if thisUnit.yards5 and br.getFacing("player",thisUnit.unit) then
        ...
        if br.isChecked("Auto Target Burn Units") and inCombat and not stealthedRogue and #enemies.yards5 > 0 and ((UnitIsVisible("target") and autoTargetUnits[br.GetObjectID("target")] == nil) or not UnitIsVisible("target")) then
            for i = 1, #enemies.yards5 do
                local thisUnit = enemies.yards5[i]
                local objID = br.GetObjectID(thisUnit)
                if autoTargetUnits[objID] ~= nil and (IsHackEnabled("AlwaysFacing") or br.getFacing("player", thisUnit)) then
    ...
    local function castBeam(minUnits, safe, minttd)
        if not br.isKnown(spell.focusedAzeriteBeam) or br.getSpellCD(spell.focusedAzeriteBeam) ~= 0 then
            return false
        end
        if br.isChecked("Ignore Azerite Beam Units During CDs") and useCDs() and energy < 70 then
            minUnits = 1
        end
        ...
        if unitsInRect >= minUnits and fightRemain > 10 then
            br._G.CastSpellByName(GetSpellInfo(spell.focusedAzeriteBeam))
            return true
        else
            return false
        end
    ...
    if br.isChecked("Ignore Blacklist for FoK and CT") and mode.rotation ~= 2 then
...
--------------------
    local function actionList_Extra()
        if not inCombat then
            ...
            if br.isChecked("Poison") and (br.pulseDelay == nil or (GetTime() - br.pulseDelay > 2)) then
                if not moving and br.getOptionValue("Poison") == 1 and buff.deadlyPoison.remain() < 300 and not cast.last.deadlyPoison(1) then
                    if cast.deadlyPoison("player") then return true end
                end
                if not moving and br.getOptionValue("Poison") == 2 and buff.woundPoison.remain() < 300 and not cast.last.woundPoison(1) then
                    if cast.woundPoison("player") then return true end
                end
                if br.isChecked("Use Crippling Poison") and not moving and buff.cripplingPoison.remain() < 300 and not cast.last.cripplingPoison(1) then
                    if cast.cripplingPoison("player") then return true end
                end
            end
            -- actions.precombat+=/stealth
            if br.isChecked("Auto Stealth") then
                if not stealth and IsUsableSpell(GetSpellInfo(spell.stealth)) and not cast.last.vanish() and not IsResting() then
                    if br.getOptionValue("Auto Stealth") == 1 then
                        if cast.stealth() then return end
                    elseif #enemies.yards20nc > 0 and br.getOptionValue("Auto Stealth") == 2 then
                ...
                elseif stealth and br.getOptionValue("Auto Stealth") == 2 and #enemies.yards20nc == 0 then
                    cancelBuff(1784)
                end
            end
        end
        ...
        if UnitIsVisible("target") and inCombat and (burnUnits[br.GetObjectID("target")] ~= nil or (not br.isChecked("Dot Players") and UnitIsFriend("target", "player") and validTarget)) and targetDistance < 5 then
            if combo > 0 and br.GetObjectID("target") == 134388 then
                if cast.kidneyShot("target") then return true end
            end
            if getenvdamage() >= br._G.UnitHealth("target") and combo > 0 then
    ...
    local function actionList_Defensive()
        if useDefensive() then
            if br.isChecked("Auto Defensive Unavoidables") then
                ...
                if bossID == 126848 and br.isCastingSpell(256979, boss) and br.GetUnitIsUnit("player", br._G.UnitTarget(boss)) then
                ...
                if br.getSpellCD(spell.evasion) == 0 then
                    ...
                    if bossID == 135475 and UnitCastID(boss) == 266231 and br.GetUnitIsUnit("player", select(3,UnitCastID(boss))) then
                        if cast.evasion() then return true end
                    end
                end
            end
            ...
            if br.isChecked("Health Pot / Healthstone") and (use.able.healthstone() or br.canUseItem(169451)) and php <= br.getOptionValue("Health Pot / Healthstone") and inCombat and (br.hasItem(169451) or has.healthstone()) then
            ...
            if br.isChecked("Concentrated Flame") and php < br.getOptionValue("Concentrated Flame") then
            ...
            if php <= br.getOptionValue("Corruption Immunity") and not IsMounted() then
                ...
                (cd.cloakOfShadows.exists() or not br.isChecked("Cloak of Shadows Corruption") or getValue("Cloak of Shadows Corruption") == 3) then
                    ...
                    elseif br.isChecked("Blind THING") then
            ...
            for i=1, #enemies.yards15 do
                ...
                if useInterrupts() and canInterrupt(thisUnit,br.getOptionValue("Interrupt %")) and (interruptDelay == nil or GetTime() - interruptDelay >= 0.25) then
                    if distance < 7 then
                        ...
                        elseif br.isChecked("Kidney Shot") and combo > 0 and cd.kidneyShot.remain() <= gcd and (cd.kick.remain() > 0 or not br.isChecked("Kick")) then
                            if cast.kidneyShot(thisUnit) then br.rogueTables.someone_casting = false interruptDelay = GetTime() return end
                        elseif br.isChecked("Blind") and (cd.kick.remain() > 0 or not br.isChecked("Kick")) and cd.blind.remain() <= gcd and (cd.kidneyShot.remain() > 0 or not br.isChecked("Kidney Shot") or combo == 0) then
                            if cast.blind(thisUnit) then br.rogueTables.someone_casting = false interruptDelay = GetTime() return end
                        end
                    elseif distance > 7 and distance <= 15 and br.isChecked("Blind") then
                        if cast.blind(thisUnit) then br.rogueTables.someone_casting = false interruptDelay = GetTime() return end
                    end
                end
                if br.isChecked("Stuns") and distance < 5 and combo > 0 and combo <= br.getOptionValue("Max CP For Stun") then
                    ...
                    if interruptID ~=nil and stunList[interruptID] and (GetTime()-(castStartTime/1000)) > 0.1 then
                        if cast.kidneyShot(thisUnit) then br.rogueTables.someone_casting = false return true end
    ...
    local function actionList_Opener()
        if opener == false and validTarget and IsSpellInRange(GetSpellInfo(spell.cheapShot), "target") == 1 then
            if opn1 == false then
                if not br.isChecked("Disable Auto Combat") and stealthedRogue then
                    if combo >= 5 then
    ...
    local function actionList_PreCombat()
        ...
        if br.isChecked("Trinkets") and br.isChecked("Precombat") and pullTimer <= 6 then
            if hasEquiped(169314) and br.canUseItem(169314) then
                br.useItem(169314)
                if br.getOptionValue("Potion") == 1 and use.able.superiorBattlePotionOfAgility() and not buff.superiorBattlePotionOfAgility.exists() then
                    use.superiorBattlePotionOfAgility()
                    return true
                elseif br.getOptionValue("Potion") == 2 and use.able.potionOfUnbridledFury() and not buff.potionOfUnbridledFury.exists() then
                    use.potionOfUnbridledFury()
                    return true
                elseif br.getOptionValue("Potion") == 3 and use.able.potionOfFocusedResolve() and not buff.potionOfFocusedResolve.exists() then
    ...
    local function actionList_Cooldowns()
        ...
        if useCDs() and ttd("target") > br.getOptionValue("CDs TTD Limit") and br.isChecked("Potion") and (hasBloodLust() or debuff.vendetta.exists("target")) and targetDistance < 5 then
            if br.getOptionValue("Potion") == 1 and use.able.superiorBattlePotionOfAgility() and not buff.superiorBattlePotionOfAgility.exists() then
                use.superiorBattlePotionOfAgility()
                return true
            elseif br.getOptionValue("Potion") == 2 and use.able.potionOfUnbridledFury() and not buff.potionOfUnbridledFury.exists() then
        ...
        if useCDs() and br.isChecked("Trinkets") and ((cd.vendetta.remain() <= 1 and (not talent.subterfuge or debuff.garrote.applied() > 1)) or cd.vendetta.remain() > 45 or not br.isChecked("Vendetta")) and targetDistance < 5 and ttd("target") > br.getOptionValue("CDs TTD Limit") then
            if br.canUseItem(13) and not (hasEquiped(169311, 13) or hasEquiped(169314, 13) or hasEquiped(159614, 13)) then
                br.useItem(13)
            end
            if br.canUseItem(14) and not (hasEquiped(169311, 14) or hasEquiped(169314, 14) or hasEquiped(159614, 13)) then
                br.useItem(14)
            end
        end
    
        -- # Specific trinktes
        if br.isChecked("Trinkets") and not stealthedRogue then
        -- # Razor Coral
            if hasEquiped(169311, 13) and br.canUseItem(13) and (not debuff.razorCoral.exists(units.dyn5) or debuff.vendetta.remain("target") > 5 or fightRemain < 20 or (br.isBoss() and ttd("target") < 20)) then
                br.useItem(13)
            elseif hasEquiped(169311, 14) and br.canUseItem(14) and (not debuff.razorCoral.exists(units.dyn5) or debuff.vendetta.remain("target") > 5 or fightRemain < 20 or (br.isBoss() and ttd("target") < 20)) then
                br.useItem(14)
            end
        -- # Pop Razor Coral right before Dribbling Inkpod proc to increase it's chance to crit (at 31% of HP)
            if hasEquiped(169311, 13) and br.canUseItem(13) and hasEquiped(169319, 14) and debuff.conductiveInk.exists("target") and br.getHP("target") < 31 then
                br.useItem(13)
            elseif hasEquiped(169311, 14) and br.canUseItem(14) and hasEquiped(169319, 13) and debuff.conductiveInk.exists("target") and br.getHP("target") < 31 then
                br.useItem(14)
            end
        -- # Galecallers Boon
        -- actions.cds+=/use_item,name=galecallers_boon,if=(debuff.vendetta.up|(!talent.exsanguinate.enabled&cooldown.vendetta.remains>45|talent.exsanguinate.enabled&(cooldown.exsanguinate.remains<6|cooldown.exsanguinate.remains>20&fight_remains>65)))&!exsanguinated.rupture
            if hasEquiped(159614, 13) and br.canUseItem(13) and (debuff.vendetta.exists("target") or (not talent.exsanguinate and cd.vendetta.remain() > 45 or talent.exsanguinate and (cd.exsanguinate.remain() < 6 or cd.exsanguinate.remain() > 20) and fightRemain > 65)) and not debuff.garrote.exsang("target") then
                br.useItem(13)
            elseif hasEquiped(159614, 14) and br.canUseItem(14) and (debuff.vendetta.exists("target") or (not talent.exsanguinate and cd.vendetta.remain() > 45 or talent.exsanguinate and (cd.exsanguinate.remain() < 6 or cd.exsanguinate.remain() > 20) and fightRemain > 65)) and not debuff.garrote.exsang("target") then
                br.useItem(14)
            end
        end
        ...
        if useCDs() and br.isChecked("Racial") and debuff.vendetta.exists("target") and ttd("target") > 5 and targetDistance < 5  then
        ...
        if useCDs() and ttd("target") > br.getOptionValue("CDs TTD Limit") and targetDistance < 5 then
            ...
            if br.isChecked("Vendetta") and not stealthedRogue and not debuff.vendetta.exists("target") then
                if br.isChecked("Hold Vendetta") and (not talent.subterfuge or sSActive == 0 or (debuff.garrote.applied("target") > 1
                ...
                if not br.isChecked("Hold Vendetta") and (not talent.nightstalker or not talent.exsanguinate 
                 or (talent.exsanguinate and cd.exsanguinate.remain() < (5-2*dSEnabled))) and debuff.rupture.exists("target") then
                    if cast.vendetta("target") then return true end
                end
            end
            if mode.vanish == 1 and #br.friend > 1 and not stealthedRogue and gcd < 0.2 and br.getSpellCD(spell.vanish) == 0 then
                ...
                if talent.nightstalker and not talent.exsanguinate and comboDeficit >= comboMax and (debuff.vendetta.exists("target") or not br.isChecked("Vendetta") or essence.visionOfPerfection.active) then
                    if cast.vanish("player") then return true end
                end
                ...
                if talent.subterfuge and enemies10 == 1 and br.getSpellCD(spell.garrote) == 0 and (not debuff.garrote.exists("target") or (sSActive == 0 and not debuff.vendetta.exists("target") and debuff.garrote.refresh("target")) or (sSActive ~= 0 and debuff.garrote.remain("target") < 18 and not debuff.vendetta.exists("target"))) and comboDeficit >= (1+2*sSActive) then
                ...
                if talent.exsanguinate and talent.subterfuge and enemies10 == 1 and comboDeficit >=(1+2*sSActive) and cd.exsanguinate.remain() < 1 and debuff.garrote.refresh("target") and not debuff.garrote.exsang("target") and br.getSpellCD(spell.garrote) == 0 then
                    if cast.pool.garrote() then return true end
                    if cast.vanish("player") then return true end
                end
                ...
                if talent.subterfuge and (not talent.exsanguinate or enemies10 > 1) and not stealthedRogue and br.getSpellCD(spell.garrote) == 0 and debuff.garrote.refresh("target") and not debuff.garrote.exsang("target") and ((enemies10 <= 3 and comboDeficit >= 1 + enemies10) or (enemies10 >= 4 and comboDeficit >= 4)) then
                    if cast.pool.garrote(nil, nil, 2) then return true end
                    if cast.vanish("player") then return true end
                end
            ...
            if br.isChecked("Essences") and debuff.rupture.exists("target") and not stealthedRogue and not IsMounted() then
                --Worldvein Resonance
                if not buff.masterAssassin.exists() then
                    if cast.worldveinResonance("player") then return true end
                end
                --Memory of lucid Dreams
                if energy < 50 and energyDeficit > (25 + energyRegenCombined) and (not br.isChecked("Vendetta") or (cd.vendetta.exists() and cd.vendetta.remain() < 115)) and not hasBloodLust() then
        ...
        if cast.able.reapingFlames() and br.isChecked("Essences") and not stealthedRogue and not IsMounted() and inCombat then
            ...
            if mob_count == 1 then
                if ((br.player.essence.reapingFlames.rank >= 2 and br.getHP(enemies.yards30[1]) > 80) or br.getHP(enemies.yards30[1]) <= 20 or br.getTTD(enemies.yards30[1], 20) > 30) then
                    reapTarget = enemies.yards30[1]
                end
            elseif mob_count > 1 then
                for i = 1, mob_count do
                    thisUnit = enemies.yards30[i]
                    local reapTTD = br.getTTD(thisUnit)
                    if br.getTTD(thisUnit) ~= 999 then
                        thisHP = br.getHP(thisUnit)
                        thisABSHP = br._G.UnitHealth(thisUnit)
                        thisABSHPmax = UnitHealthMax(thisUnit)
                        reapingPercentage = round2(reapingDamage / UnitHealthMax(thisUnit), 2)
                        if br._G.UnitHealth(thisUnit) <= reapingDamage or reapTTD < 2 then
                            reap_execute = thisUnit
                            break
                        elseif br.getTTD(thisUnit, reapingPercentage) < 29 or br.getTTD(thisUnit, 20) > 30 and (br.getTTD(thisUnit, reapingPercentage) < 44) then
                            reap_hold = true
                        elseif (thisHP > 80 or thisHP <= 20) or br.getTTD(thisUnit, 20) > 30 then
            ...
            if reapTarget ~= nil and not isExplosive(reapTarget) and br.getFacing("player",reapTarget) then
        ...
        if mode.exsang == 1 and talent.exsanguinate and br.getSpellCD(spell.exsanguinate) == 0 and debuff.rupture.remain("target") > 25 and 
        ...
        if mode.exsang == 1 and enemies10 > 1 and talent.exsanguinate and br.getSpellCD(spell.exsanguinate) == 0 and debuff.rupture.remain("target") > 15 and (not debuff.garrote.refresh("target") or garroteCheck == false) and ttd("target") > 4 then
            if cast.exsanguinate("target") then return true end
        end
        -- actions.cds+=/toxic_blade,if=dot.rupture.ticking
        if talent.toxicBlade and mode.tb == 1 and ttd("target") > 3 and br.getSpellCD(spell.toxicBlade) == 0 and debuff.rupture.exists("target") 
        ...
        if debuff.rupture.exists("target") and not stealthedRogue then
            -- Concentrated Flame
            if br.isChecked("Essences") and ttd("target") > 3 then
                if cast.concentratedFlame("target") then return true end
            end
            --Beamers
            if (standingTime > 1 or (essence.essenceOfTheFocusingIris and essence.essenceOfTheFocusingIris.rank >= 3)) and br.isChecked("Focused Azerite Beam") then
                if castBeam(br.getOptionValue("Focused Azerite Beam"), true, 3) then return true end
            end
        end
    end
    ...
    local function actionList_Direct()
        ...
        if combo >= (4 + dSEnabled) and ((debuff.vendetta.exists("target") or not useCDs() or ttd("target") < br.getOptionValue("CDs TTD Limit")) or debuff.toxicBlade.exists("target") or buff.masterAssassin.exists() or energyDeficit <= (25 + energyRegenCombined) or enemies10 > 1) 
        and (not talent.exsanguinate or cd.exsanguinate.remain() > 2 or (not debuff.vendetta.exists() and comboDeficit >= comboMax) or mode.exsang == 2) and br.getFacing("player","target")
        then
            if cast.envenom("target") then return true end
        end
        ...
        if useFiller and enemies10 == 2 then
            if (br.getOptionValue("Poison") == 1 and not debuff.deadlyPoison.exists("target")) or (br.getOptionValue("Poison") == 2 and not debuff.woundPoison.exists("target")) and br.getFacing("player","target") then
                if cast.mutilate("target") then return true end
            end
            for i = 1, #enemyTable5 do
                local thisUnit = enemyTable5[i].unit
                if (br.getOptionValue("Poison") == 1 and not debuff.deadlyPoison.exists(thisUnit)) or (br.getOptionValue("Poison") == 2 and not debuff.woundPoison.exists(thisUnit)) and br.getFacing("player",thisUnit) then
        ...
        if br.isChecked("Poisoned Knife out of range") and not stealthedRogue and #enemyTable5 == 0 and energy >= br.getOptionValue("Poisoned Knife out of range") and inCombat  then
            for i = 1, #enemyTable30 do
                ...
                if br.getFacing("player",thisUnit) then
        ...
        if useFiller and br.getFacing("player","target") then
            if cast.mutilate("target") then return true end
        end
        --evis low level
    ...
    local function actionList_Dot()
        ...
        if useCDs() and ttd("target") > br.getOptionValue("CDs TTD Limit") then
        ...
        if (not talent.subterfuge or not (vanishCheck and vendettaCheck)) and comboDeficit >= 1 and garroteCheck and br.getSpellCD(spell.garrote) == 0 then
            if garroteCount <= br.getOptionValue("Multidot Limit") and (not talent.masterAssassin or buff.masterAssassin.remain() <= 0.8) then
                for i = 1, #enemyTable5 do
                    local thisUnit = enemyTable5[i].unit
        ...
        if combo >= 4 and br.getSpellCD(spell.rupture) == 0 and (mode.focus == 1 or energyDeficit > (25 + energyRegenCombined)) then
            for i = 1, #enemyTable5 do
                local thisUnit = enemyTable5[i].unit
    ...
    local function actionList_Stealthed()
        ...
        if talent.subterfuge and sSActive == 1 and not debuff.rupture.exists("target") and #enemies.yards30 == 1 then
            if cast.rupture("target") then return true end
        end
        ...
        if talent.subterfuge and sSActive == 1 then
            local garroteTable = {}
            local addUnit = {}
            for i = 1, #enemyTable5 do
                local thisUnit = enemyTable5[i].unit
...
-----------------
    ...
    --     local _, param, _, source, _, _, _, destination, _, _, _, spell, _, _, spellType = CombatLogGetCurrentEventInfo()
    --     if inCombat and param == "SPELL_CAST_START" then
    --         print("Spell Cast Detected")
    --         if not UnitIsPlayer(source)  then
    --         --and br.rogueTables.someone_casting ~= true then
    ...
    if inCombat then
        ...
        -- if br.rogueTables.someone_casting == true then
        if actionList_Interrupts() then end
        -- end
    end

    --tricks
    if SpecificToggle("Tricks Key") then
        if cast.tricksOfTheTrade(tricksUnit) then end
    end
    if br.isChecked("Auto Tricks") then
        if tricksUnit ~= nil then
            for i = 1, #enemies.yards30 do
            local thisUnit = enemies.yards30[i]
                if (br._G.UnitTarget(thisUnit) ~= "TANK" or UnitGroupRolesAssigned(br._G.UnitTarget(thisUnit)) ~= "TANK") and UnitThreatSituation(tricksUnit, thisUnit) ~= nil and UnitThreatSituation(tricksUnit, thisUnit) <= 2 then
                    if cast.tricksOfTheTrade(tricksUnit) then end
                end
            end
        end
...
--- In Combat - Rotations ---
-----------------------------
        if (inCombat or (not br.isChecked("Disable Auto Combat") and (cast.last.vanish(1) or cast.last.vanish(2) or (validTarget and targetDistance < 15)))) and opener == true then
            ...
            if useCDs() and br.isChecked("Racial") and targetDistance < 5 then
