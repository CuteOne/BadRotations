...
local function createToggles()
    -- Rotation Button
    local RotationModes = {
        [1] = { mode = "On", value = 1, overlay = "Rotation Enabled", tip = "Enable Rotation", highlight = 1, icon = br.player.spells.sinisterStrike },
        [2] = { mode = "Off", value = 4, overlay = "Rotation Disabled", tip = "Disable Rotation", highlight = 0, icon = br.player.spells.sinisterStrike }
    };
    ...
    local InterruptModes = {
        [1] = { mode = "On", value = 1, overlay = "Interrupts Enabled", tip = "Use Interrupt Abilities", highlight = 1, icon = br.player.spells.kick },
        [2] = { mode = "Off", value = 2, overlay = "Interrupts Disabled", tip = "Do Not Use Interrupt Abilities", highlight = 0, icon = br.player.spells.kick }
    };
...
actionList.Extras = function()
    ...
    -- Stealth
    if ui.checked("Stealth") and cast.able.stealth() and not buff.stealth.exists()
        and not unit.casting() and not unit.inCombat() and not (unit.mounted() or unit.flying()) and not unit.resting()
    then
        if cast.stealth() then
            ui.debug("Casting Stealth")
            return true
        end
    end
    -- Sprint
...
actionList.Combat = function()
    ...
    --Eviscerate
    if cast.able.eviscerate() and (combo() > 4 or unit.hp(units.dyn5) < 20) then
        if cast.eviscerate() then
            ui.debug("Casting Eviscerate")
            return true
        end
    ...
    if cast.able.sinisterStrike() then
        if cast.sinisterStrike() then
            ui.debug("Casting Sinister Strike")
            return true
        end
    end
...
local function runRotation()
    ...
    buff        = br.player.buff
    cast        = br.player.cast
    combo       = br.player.power.comboPoints
